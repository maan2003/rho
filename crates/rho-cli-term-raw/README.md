...
Terminal prompt with async output support for rho.
...
- **Resize clears pre-rho scrollback history.** Any terminal output from
  *before* rho started (shell commands, etc.) is lost on the first resize.
  rho's configured replay window is rebuilt by the full re-render, but older
  clipped rho rows remain only in rho's logical history, and the pre-rho
