...
/// External-program pause must release terminal feature modes that rho enabled
...
/// Cursor shape settings should use steady crossterm styles so rho does not
...
/// Basic append operations should keep rho's retained model and vt100's
...
fn accepting_completion_refreshes_next_candidates_immediately() {
    ...
    term.set_completion_source(Some(Box::new(
        |buffer: &str, _cursor: usize| match buffer {
            ...
            "./crates/" => vec![Candidate {
                label: "./crates/rho-cli-term-raw/".to_owned(),
                description: "directory".to_owned(),
                replacement: "./crates/rho-cli-term-raw/".to_owned(),
            }],
            _ => Vec::new(),
        },
    )));
    ...
    assert_eq!(completion.candidates[0].label, "./crates/rho-cli-term-raw/");
}
