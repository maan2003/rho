...
pub fn write_installation(profile: &Path, extension: &Path, executable: &Path) -> Result<()> {
    ...
    fs::write(
        hosts.join(format!("{HOST_NAME}.json")),
...
mod tests {
    ...
    fn installation_uses_the_fixed_extension_identity() {
        ...
        write_installation(&profile, &extension, &executable).unwrap();
