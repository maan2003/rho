//! The desk, as the client holds it: one replica per host, the handshake
//! that keeps it level with the host's copy, the map drawn from its facts,
//! and the writes every verb makes.
//!
//! Nothing here draws. The words of a note are the one thing kept
//! elsewhere: they live in editor buffers, which belong to the window, so
//! this hands over what arrived and the window folds it in.
...
/// How many source entries the desk has examined since last asked, on
/// this thread.
#[cfg(any(test, feature = "test-support"))]
pub fn take_source_scans() -> usize {
...
/// What a sync answer leaves for the caller.
pub struct Synced {
    /// What goes back to the host: this client's half of the sync, and a
    /// further `Sync` when a poke is still owed.
    pub back: Vec<DeskClientFrame>,
    /// What moved on the map.
    pub delta: DeskDelta,
    /// The cells were counted in another store, so everything held for
    /// the host went, and the words of its notes go with it.
    pub reset: bool,
    /// The words that arrived, for the note buffers.
    pub bodies: Vec<BodySnapshot>,
}

impl Synced {
    fn quiet(reset: bool) -> Self {
        Self {
            back: Vec::new(),
            delta: DeskDelta::quiet(),
            reset,
            bodies: Vec::new(),
        }
    }
...
impl Desk {
    ...
    pub fn device(&mut self) -> DeviceId {
        *self.device.get_or_insert_with(crate::cache::device)
    }
    ...
    /// one was in flight and the answer does not already cover it. The
    /// words that arrived go back to the caller for the note buffers.
    pub fn synced(
        &mut self,
        host: HostId,
        store: DeviceId,
    ...
    /// How much of each note's text arrived, said before any of it is
    /// applied: what the next sync asks for is what arrived, whether or not
    /// this build could decode every operation in it.
    fn hold_bodies(&mut self, host: HostId, bodies: &[BodySnapshot]) {
        let Some(desk) = self.hosts.get_mut(&host) else {
            return;
        };
        for body in bodies {
...
impl Desk {
    ...
    /// True while the map draws a row for the id.
    pub fn has_node(&self, host: HostId, id: &Id) -> bool {
        self.hosts
            .get(&host)
            .is_some_and(|desk| desk.node_at.contains_key(id))
    }
...
/// the caller sends them through [`Desk::apply`] so one keypress is one
/// mutation.
impl Desk {
