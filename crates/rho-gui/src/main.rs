...
fn run() -> Result<()> {
    ...
    let browser_socket = match local_socket {
        Some(socket) => rho_rpc::protocol::RuntimePaths::new(Some(socket.clone()))?,
        None => rho_rpc::protocol::RuntimePaths::new(None::<PathBuf>)?,
    }
    .browser_socket();
...
fn editor_stage_name(kind: gpui::profiler::EditorTimingKind) -> &'static str {
    match kind {
        ...
        gpui::profiler::EditorTimingKind::HighlightedChunks => "highlighted_chunks",
    }
}
