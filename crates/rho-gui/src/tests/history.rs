...
/// A dealt note's surface is the dashboard's editor with the point on the
/// node, so stepping back to one has to put the point back. Without this
/// the title bar names the surface the reader stepped to while the rows
/// under it never move: the rig showed frames that differed only in the
/// title row.
#[gpui::test]
fn stepping_back_to_a_dealt_note_puts_the_point_on_it(cx: &mut TestAppContext) {
    let mut desk = super::DeskFixture::new();
    let first = desk.due_note(None, "The first card");
    let second = desk.due_note(None, "The second card");
    ...
    let dealt = workspace
        .update(cx, |workspace, _, cx| {
            workspace.dashboard.tree_node_at_cursor(cx)
        })
        .unwrap();
    assert!(
        dealt
            .as_ref()
            .is_some_and(|(_, id)| *id == first || *id == second),
        "dealing put the point on the card it opened, not {dealt:?}"
    );
    ...
    assert_ne!(
        back, dealt,
        "up moved the point off the card the reader was on"
    );
}
