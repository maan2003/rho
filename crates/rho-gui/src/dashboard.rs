...
pub use rho_desk_client::protocol::cells::SlackUnit;

use crate::workspace::Workspace;

type DraftTopic = Option<(HostId, rho_desk_client::protocol::cells::Id)>;
...
pub struct DealCard {
    ...
    pub topic_node_id: rho_desk_client::protocol::cells::Id,
...
pub struct DeskRoom {
    pub host: HostId,
    pub node_id: rho_desk_client::protocol::cells::Id,
    pub name: String,
}
...
struct HeadingContext {
    ...
    heading: rho_desk_client::protocol::cells::Id,
    breadcrumb: String,
    room: Option<String>,
...
struct DealerSet {
    ...
    of_heading: HashMap<(HostId, rho_desk_client::protocol::cells::Id), HashSet<AgentId>>,
...
pub enum DealScope<'a> {
    ...
    /// of the hand with it.
    Nodes(&'a [rho_desk_client::protocol::cells::Id]),
}
...
pub enum CardCursor {
    ...
    Slack(rho_desk_client::protocol::cells::SlackTs),
    ...
    Desk(DeskMark, rho_desk_client::protocol::cells::Timestamp),
}
...
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
enum LineKey {
    NewDraft(Option<(HostId, rho_desk_client::protocol::cells::Id)>),
}
...
pub enum RowTarget {
    ...
    TreeAgent {
        host: HostId,
        node_id: rho_desk_client::protocol::cells::Id,
        topic_node_id: rho_desk_client::protocol::cells::Id,
        agent_id: AgentId,
    },
    TreePage {
        host: HostId,
    ...
    NewDraft,
    NewTreeDraft((HostId, rho_desk_client::protocol::cells::Id)),
}
...
pub struct Dashboard {
    ...
    tree_new_draft_parent: Option<(HostId, rho_desk_client::protocol::cells::Id)>,
...
impl Dashboard {
    ...
    fn heading_context(
        ...
        if heading.state != rho_desk_client::protocol::cells::State::Open {
            return None;
        }
    ...
    fn agent_card(
        ...
        let node_id = source
            ...
            .unwrap_or(rho_desk_client::protocol::cells::Id::Agent(agent_id));
        Some(RankedDealCard {
            priority,
    ...
    fn thread_card(
        ...
        if node.state != rho_desk_client::protocol::cells::State::Open
        ...
        Some(RankedDealCard {
            ...
            virtual_reply: false,
            order,
            cursor: CardCursor::Slack(rho_desk_client::protocol::cells::SlackTs(
                thread.latest.clone(),
            )),
    ...
    fn loose_agent_card(
        ...
        let node_id = node
            .map(|node| node.id.clone())
            .unwrap_or(rho_desk_client::protocol::cells::Id::Agent(agent.agent_id));
        let identity = DealCardId {
            host: agent.host,
    ...
    pub fn card_for_node(
        ...
        let agent_id = match node {
            ...
            None => match &node_id {
                rho_desk_client::protocol::cells::Id::Agent(agent_id) => Some(*agent_id),
                _ => return None,
            },
        };
    ...
    fn room_for_node(
        &self,
        host: HostId,
        mut node_id: rho_desk_client::protocol::cells::Id,
    ) -> Option<DeskRoom> {
        let source = self.deal_hosts.get(&host)?;
        loop {
            let node = source.node(&node_id)?;
    ...
    fn filed_node(
        &self,
        find: impl Fn(&crate::candidates::HostNodes) -> Option<rho_desk_client::protocol::cells::Id>,
    ...
    fn page_node_id(
        ...
        let page = rho_desk_client::protocol::PageId(*page_id.0.as_bytes());
    ...
    pub fn first_agent_for_topic(
        &self,
        topic: (HostId, rho_desk_client::protocol::cells::Id),
    ) -> Option<AgentId> {
        self.deal_hosts
            .get(&topic.0)
    ...
    pub(crate) fn patch_deal_source(
        &mut self,
        host: HostId,
        touched: &BTreeSet<rho_desk_client::protocol::cells::Id>,
        nodes: &[rho_desk_client::desk::DeskNode],
    ) -> bool {
        #[cfg(test)]
        {
    ...
    pub fn card_target(&self, card: DealCardId) -> CardTarget {
        ...
        match &node.id {
            rho_desk_client::protocol::cells::Id::Agent(_) => {
                node.agent().map_or(CardTarget::Missing, CardTarget::Agent)
            }
            rho_desk_client::protocol::cells::Id::Page(_) => {
                node_page(node).map_or(CardTarget::Missing, CardTarget::Page)
            }
            rho_desk_client::protocol::cells::Id::Slack(_) => {
        ...
        self.deal_hosts
            ...
            .flat_map(|(host, source)| {
                source
                    .nodes()
                    .iter()
                    .filter(|node| node.state == rho_desk_client::protocol::cells::State::Open)
    ...
    pub fn node_is_open(&self, card: DealCardId) -> bool {
        self.deal_hosts
            .get(&card.host)
            .and_then(|source| source.node(&card.node_id))
            .is_some_and(|node| node.state == rho_desk_client::protocol::cells::State::Open)
    }
    ...
    pub fn node_defer_until(
        &self,
        card: DealCardId,
    ) -> Option<rho_desk_client::protocol::cells::Timestamp> {
        self.deal_hosts
            .get(&card.host)
            .and_then(|source| source.node(&card.node_id))
    ...
    pub fn open_new_draft(
        &mut self,
        topic: Option<(HostId, rho_desk_client::protocol::cells::Id)>,
        window: &mut Window,
        cx: &mut Context<Workspace>,
    ) {
    ...
    pub fn new_draft_topic(&self) -> Option<(HostId, rho_desk_client::protocol::cells::Id)> {
...
fn desk_time(at: rho_desk_client::protocol::cells::Timestamp) -> Option<chrono::NaiveDateTime> {
...
fn desk_elapsed(
    at: rho_desk_client::protocol::cells::Timestamp,
    now: chrono::NaiveDateTime,
) -> Option<f64> {
    let time = desk_time(at)?;
    Some(
        if at.precision == rho_desk_client::protocol::cells::TimestampPrecision::Day {
...
fn desk_marks(
    node: &rho_desk_client::desk::DeskNode,
) -> Vec<(DeskMark, rho_desk_client::protocol::cells::Timestamp)> {
    if node.state != rho_desk_client::protocol::cells::State::Open {
        return Vec::new();
    }
...
fn node_closed(
    ...
    node.state != rho_desk_client::protocol::cells::State::Open
...
struct RankedDealCard {
    ...
    heading: Option<rho_desk_client::protocol::cells::Id>,
}
