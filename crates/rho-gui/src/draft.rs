...
//! to create the agent.
//!
//! Entirely separate from the agent transcript model — there is no
...
impl DraftModel {
    ...
    fn refresh_attachment_blocks(&mut self, cx: &mut Context<Self>) {
        ...
        let block = style::attachment_block(anchor, attachment_labels(&self.attachments));
        for editor in self.live_editors() {
...
fn attachment_labels(attachments: &[ContentPart]) -> Vec<String> {
    attachments
        .iter()
