...
mod tests {
    ...
    #[test]
    fn capped_log_evicts_oldest_and_renders_time() {
        let timestamp = chrono::DateTime::parse_from_rfc3339("2026-08-23T09:05:00+00:00").unwrap();
        let entry = |text: &str| Entry {
            timestamp,
            class: StyleClass::SystemInfo,
            text: text.to_owned(),
        };
        let mut log = Log(VecDeque::new(), 2);
        assert!(!log.push(entry("first")));
        assert!(!log.push(entry("second")));
        assert!(log.push(entry("third")));
        assert_eq!(
            log.0
                .iter()
        ...
        assert_eq!(render(&entry("third")), "09:05  third\n");
    }
}
