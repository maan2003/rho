...
/// Async output invalidates rho-owned rows; the next prompt render must redraw
...
fn all_three_zones_together() {
    let mut t = TestTerm::new(24, 40);
    let (lines, cursor) = build_prompt_layout("rho v0.1", "$ ", "ls", "[main]", 40);
    let mut buf = Vec::new();
    t.screen.update(&mut buf, &lines, cursor).expect("ok");
    t.term.process(&buf);

    assert_eq!(t.row_text(0), "rho v0.1");
