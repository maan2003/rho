...
mod tests {
    ...
    /// An agent label is a workset to join, not a revision to clone from.
    /// The mismatched workdir value proves the target's place wins for Join.
    #[test]
    fn existing_workset_requires_join_and_keeps_its_place() {
        let hosts = Hosts::new(std::sync::Arc::new(rho_agent_hosts::DroppedSink));
        let place = rho_agent_types::Place {
            workset: "another-workset".into(),
            cwd: "/src/agent/repo".into(),
            mode: WorksetMode::Exposed,
            origin: Some("/src/original".into()),
        };
        let base = StartBase {
            host: Some(HostId(7)),
            workspace: Some(WorkspaceInfo::Workset(place.clone())),
        };
        let workdir = Some(HostPath {
            host: HostId(7),
            path: "/src/other-repo".into(),
        });
        assert!(
            parse_start(
                &hosts,
        ...
        assert_eq!(
            ...
            (
                HostId(7),
                StartMode::Join(JoinTarget::Workspace(WorkspaceInfo::Workset(place)))
            )
        );
    }
