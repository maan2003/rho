...
impl RealtimeSession {
        ...
        F: Future<Output = anyhow::Result<SdpAnswer>>,
    {
        ...
        tokio::time::timeout(Duration::from_secs(15), open_rx)
            .await
            .context("timed out opening realtime data channel")??;
...
fn start_microphone(
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let Ok(mut encoder) = OpusEncoder::new(SAMPLE_RATE as i32, 1, Application::Voip) else {
                return;
            };
            ...
            loop {
                ...
                let Ok(packet_len) = encoder.encode(&frame, FRAME_SAMPLES, &mut packet) else {
                    return;
                };
