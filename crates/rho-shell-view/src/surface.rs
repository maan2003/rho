...
use crate::channel::{ShellChannel, ShellSubmission};
use crate::protocol::{
...
impl ShellModel {
    ...
    fn apply(&mut self, frame: ShellServerFrame, cx: &mut Context<Self>) {
        match frame {
            ...
            ShellServerFrame::ExecutionStarted {
                ...
                {
                    block.state = crate::protocol::ShellExecutionState::Running;
                    block.prompt = prompt;
                    block.cwd = cwd;
                }
            }
            ...
            ShellServerFrame::PagerPaused { execution, pager } => {
                if pager.execution != execution
                    ...
                    || pager.page == 0
                    || pager.bytes == 0
                    || pager.bytes > crate::protocol::MAX_PAGER_BYTES
                    || pager.lines > crate::protocol::MAX_PAGER_LINES
                {
                    self.mark_disconnected(cx);
                    return;
                }
                ...
                {
                    *existing = pager;
                } else if self.shell_state.pagers.len() < crate::protocol::MAX_ACTIVE_PAGERS {
            ...
            ShellServerFrame::ExecutionFinished { execution, status } => {
                if let Some(block) = self
                    ...
                    .find(|block| block.execution == execution)
                {
                    block.state = crate::protocol::ShellExecutionState::Finished { status };
                }
            }
            ShellServerFrame::ExecutionFailed { execution } => {
                if let Some(block) = execution.and_then(|execution| {
                    ...
                    block.state = crate::protocol::ShellExecutionState::Failed;
                }
            }
    ...
    fn render_transcript(&mut self, cx: &mut Context<Self>) {
        ...
        for execution in &self.shell_state.executions {
            if matches!(
                execution.state,
                crate::protocol::ShellExecutionState::Queued
            ) {
                continue;
            }
...
mod tests {
    ...
    fn shell_snapshot_rejects_invalid_pagers() {
        let pager = crate::protocol::ShellPager {
            execution: 1,
            pager: 1,
            page: 1,
            lines: 24,
        ...
        let mut state = crate::protocol::ShellState {
        ...
        state.pagers[0].bytes = crate::protocol::MAX_PAGER_BYTES + 1;
        assert!(!shell_state_styles_valid(&state));
    }
