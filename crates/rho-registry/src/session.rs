...
pub fn recent_agent_roots(
    ...
    let mut roots = agents
        .iter()
        .filter(|agent| !agent.hidden && agent.parent_agent.is_none())
        .collect::<Vec<_>>();
    roots.sort_by_key(|agent| (std::cmp::Reverse(agent.last_active), agent.agent_id));
    ...
    if let Some(selected) = selected
        && agents
            .iter()
            .any(|agent| agent.agent_id == selected && !agent.hidden)
    {
