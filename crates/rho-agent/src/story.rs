...
pub fn from_raw_event(event: &crate::AgentEvent<'_>, at: UnixMillis) -> Vec<StoryEvent> {
    use crate::{AgentEvent, InputKind, QueuedInput, QueuedItem, QueuedItemKind};

    let message = |sender: &rho_core::MessageSender, content: &[rho_core::ContentPart]| {
    ...
    match event {
        ...
        // A reply is one response block; what else rides in it is not
        // the model speaking.
        AgentEvent::Replied { blocks, .. } => blocks
            .iter()
            .flat_map(|block| match block {
                rho_core::ContextBlock::InferenceResponse { items, .. } => {
                    from_inference_items(items, at)
                }
                _ => Vec::new(),
            })
            .collect(),
