...
#[cfg(test)]
use rho_agent_types::StreamingContextItem;
...
pub(in crate::agent) mod tests {
    use rho_agent_types::{AppendString, ContextItemEvent, ProviderResponseItemId, ToolType};
            ...
            agent
                .persist(AgentEvent::Native(
                    crate::native::NativeEvent::RequestStarted {
                        input: vec![rho_agent_types::ContextBlock::DeveloperMessage {
            ...
            agent
                .persist(AgentEvent::Native(
                    crate::native::NativeEvent::ResponseFinished {
                        output: vec![rho_agent_types::ContextBlock::InferenceResponse {
                            items: Vec::new(),
                            provider_response_id: None,
                        }],
                        context_used: None,
    ...
    async fn failure_discards_suffix_and_retry_boundary_drains_fresh_command_output() {
        ...
        assert!(!agent.provider_input().await.unwrap().iter().any(|block| matches!(&**block,
            ContextBlock::UserMessage { content, .. } if rho_agent_types::text_content(content).contains("stream disconnected"))));
