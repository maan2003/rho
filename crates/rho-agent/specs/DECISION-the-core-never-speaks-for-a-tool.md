...
- `PythonCell::first_output` is required, because a provider rejects a call no
...
- `PythonCell::more_output` is entirely the tool's choice, including whether to
...
- Source facts are input to `boundary`, not transcript content. Completion
  ...
  rejected whole.
- A call is forgotten when `PythonCell::done` says so, after its output has
