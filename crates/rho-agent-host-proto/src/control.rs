...
//!
//! Opened by [`crate::Open::Control`], one per iroh connection. The host
//! speaks first with [`ServerFrame::Ready`] and then pushes whatever changes
//! about itself; nothing on this stream answers a request, because requests
//! have streams of their own ([`crate::Open::Request`]).

use senax_encoder::{Decode, Encode, Pack, Unpack};

use crate::{AgentId, AuthState, DesktopSession, GitTransportRequest, QuotaSummary};
...
pub enum ServerFrame {
    ...
    /// provides one; each answers with [`crate::Open::GitProvide`].
