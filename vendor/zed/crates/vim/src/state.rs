...
pub struct VimGlobals {
    ...
    pub marks: Option<Entity<MarksState>>,
}
...
mod zed_marks {
    ...
    pub struct MarksState {
        ...
        pub(super) multibuffer_marks: HashMap<EntityId, HashMap<String, Vec<Anchor>>>,
        pub(super) buffer_marks: HashMap<BufferId, HashMap<String, Vec<text::Anchor>>>,
        watched_buffers: HashMap<BufferId, (MarkLocation, Subscription, Subscription)>,

        pub(super) serialized_marks: HashMap<Arc<Path>, HashMap<String, Vec<Point>>>,
        pub(super) global_marks: HashMap<String, MarkLocation>,
    ...
    impl MarksState {
        pub fn new(workspace: &Workspace, cx: &mut App) -> Entity<MarksState> {
            cx.new(|cx| {
                ...
                let subscription =
                    cx.subscribe(&buffer_store, move |this: &mut Self, _, event, cx| {
        ...
        fn load(&mut self, cx: &mut Context<Self>) {
            cx.spawn(async move |this, cx| {
                let Some(workspace_id) = this.update(cx, |this, cx| this.workspace_id(cx)).ok()?
                else {
                    return None;
                };
            ...
            for (name, path) in global_mark_paths {
                ...
                let project_path = project
                    ...
                    .filter_map(|worktree| {
                        ...
                        let path =
                            RelPath::new(relative, worktree.read(cx).path_style()).log_err()?;
                        Some(ProjectPath {
        ...
        pub fn watch_buffer(
            ...
            let on_change =
                cx.subscribe(buffer_handle, move |this, buffer, event, cx| match event {
                    BufferEvent::Edited { .. } => {
        ...
        pub fn set_mark(
            ...
            let Some(abs_path) = abs_path else {
                ...
                if let Some(buffer) = buffer {
                    ...
                    if !self.watched_buffers.contains_key(&buffer_id) {
                        self.watch_buffer(
                            MarkLocation::Buffer(multibuffer.entity_id()),
                            &buffer,
                            cx,
                        )
                    }
            ...
            self.buffer_marks.entry(buffer_id).or_default().insert(
                ...
                anchors
                    .into_iter()
                    .filter_map(|anchor| {
                        Some(multibuffer_snapshot.anchor_to_buffer_anchor(anchor)?.0)
                    })
                    .collect(),
            );
            if !self.watched_buffers.contains_key(&buffer_id) {
        ...
        pub fn get_mark(
            ...
            if !self.is_global_mark(name)
                || target.is_some_and(|t| self.points_at(t, multi_buffer, cx))
            {
        ...
        pub fn delete_mark(
            ...
            if let Some(workspace_id) = self.workspace_id(cx) {
                let db = VimDb::global(cx);
                cx.background_spawn(
                    async move { db.delete_mark(workspace_id, path, mark_name).await },
                )
                .detach_and_log_err(cx);
            }
        }
...
#[cfg(feature = "zed-workspace")]
use zed_marks::MarksState;
#[cfg(feature = "zed-workspace")]
pub(crate) use zed_marks::{Mark, MarkLocation};
...
mod editor_marks {
    ...
    impl MarksState {
        ...
        pub fn delete_mark(
            ...
            if let Some(marks) = self.multibuffer_marks.get_mut(&multibuffer.entity_id()) {
                marks.remove(&name);
            }
        }
    }
...
#[cfg(not(feature = "zed-workspace"))]
pub(crate) use editor_marks::Mark;
#[cfg(not(feature = "zed-workspace"))]
use editor_marks::MarksState;
...
impl VimGlobals {
    pub(crate) fn register(cx: &mut App) {
        ...
        #[cfg(not(feature = "zed-workspace"))]
        {
            let marks = MarksState::new(cx);
            cx.global_mut::<VimGlobals>().marks = Some(marks);
        }
        ...
        cx.observe_global::<SettingsStore>(move |cx| {
            ...
            if is_enabled {
                ...
                #[cfg(not(feature = "zed-workspace"))]
                {
                    let marks = Vim::globals(cx).marks.clone();
                    *Vim::globals(cx) = VimGlobals::default();
                    Vim::globals(cx).marks = marks;
                }
                #[cfg(feature = "zed-workspace")]
                {
...
mod zed_workspace {
    ...
    impl RegistersView {
        pub(super) fn register(workspace: &mut Workspace, _window: Option<&mut Window>) {
        ...
        fn render_match(
            ...
            Some(
                h_flex()
                    ...
                    .px_2()
                    .child(
                        StyledText::new(left_output)
                            .with_default_highlights(&text_style, left_runs),
                    )
                    .child(
                        StyledText::new(right_output)
                            .with_default_highlights(&text_style, right_runs),
                    ),
            )
        }
    }
    ...
    pub(super) struct SerializedMark {
    ...
    impl VimDb {
        ...
        pub(super) fn get_marks(&self, workspace_id: WorkspaceId) -> Result<Vec<SerializedMark>> {
...
#[cfg(feature = "zed-workspace")]
use zed_workspace::{MarksView, RegistersView, SerializedMark, VimDb};
