...
use crate::coding::{BufMutExt, Decodable, Encodable, UnexpectedEnd};
use crate::config::{ClientConfig, EndpointConfig, ServerConfig};
use crate::connection::{Connection, ConnectionError, SideArgs};
use crate::crypto::{self, Keys, UnsupportedVersion};
use crate::packet::{
...
use crate::token::{IncomingToken, InvalidRetryTokenError, Token, TokenPayload};
...
use crate::{
    ...
    RESET_TOKEN_SIZE, ResetToken, Side, Transmit, TransportConfig, TransportError, frame,
};

/// The main entry point to the library
///
/// This object performs no I/O whatsoever. Instead, it consumes incoming
/// packets and connection-generated events via `handle` and `handle_event`.
pub struct Endpoint {
    rng: StdRng,
...
impl Endpoint {
    ...
    ///
    /// `allow_mtud` enables path MTU detection when requested by `Connection`
    /// configuration for better performance. This requires that outgoing
    /// packets are never fragmented, which can be achieved via e.g. the
    ...
    ///
    /// In turn, processing this event may return a `ConnectionEvent` for the
    /// same `Connection`.
    pub fn handle_event(
        &mut self,
        ch: ConnectionHandle,
    ...
    fn stateless_reset(
        ...
        /// Minimum amount of padding for the stateless reset to look like a
        /// short-header packet
        const MIN_PADDING_LEN: usize = 5;

        // Prevent amplification attacks and reset loops by ensuring we pad to at most 1
        // byte smaller than the inciting packet.
        ...
        // Resets with at least this much padding can't possibly be distinguished from
        // real packets
    ...
    /// Check if we should refuse a connection attempt regardless of the
    /// packet's contents
    fn early_validate_first_packet(
        ...
        // RFC9000 §7.2 dictates that initial (client-chosen) destination CIDs must be
        // at least 8 bytes. If this is a Retry packet, then the length must
        // instead match our usual CID length. If we ever issue non-Retry
        // address validation tokens via `NEW_TOKEN`, then we'll also need to
        // validate CID length for those after decoding the token.
    ...
    pub fn retry(&mut self, incoming: Incoming, buf: &mut Vec<u8>) -> Result<Transmit, RetryError> {
        ...
        // First Initial
        // The peer will use this as the DCID of its following Initials. Initial DCIDs
        // are looked up separately from Handshake/Data DCIDs, so there is no
        // risk of collision with established connections. In the unlikely event
        // that a collision occurs between two connections in the initial phase,
        // both will fail fast and may be retried by the application layer.
    ...
    /// response
    ///
    /// Doing this actively, rather than merely dropping the [`Incoming`], is
    /// necessary to prevent memory leaks due to state within [`Endpoint`]
    /// tracking the incoming connection.
    ...
    fn initial_close(
        ...
        // We don't need to worry about CID collisions in initial closes because the
        // peer shouldn't respond, and if it does, and the CID collides, we'll
        // just drop the unexpected response.
    ...
    /// in the buffers for Initial and 0-RTT messages for pending incoming
    /// connections
    ...
    pub(crate) fn known_connections(&self) -> usize {
        ...
        // Not all connections have unique remotes, and 0-length CIDs might not be in
        // use.
    ...
    ///
    /// We leave some space unused so that `new_cid` can be relied upon to
    /// finish quickly. We don't bother to check when CID longer than 4
    /// bytes are used because 2^40 connections is a lot.
    fn cids_exhausted(&self) -> bool {
        ...
        // Keep this architecture-independent: on 32-bit targets, 2usize.pow(32)
        // overflows.
...
struct ConnectionIndex {
    ...
    ///
    /// We don't yet support explicit source addresses for client connections,
    /// and zero-length CIDs require a unique 4-tuple, so at most one client
    /// connection with zero-length local CIDs may be established per
    /// remote. We must omit the local address from the key because we don't
    /// necessarily know what address we're sending from, and hence receiving
    /// at.
    ///
    /// Uses a standard `HashMap` to protect against hash collision attacks.
    ...
    /// Reset tokens provided by the peer for the CID each connection is
    /// currently sending to
    ///
    /// Incoming stateless resets do not have correct CIDs, so we need this to
    /// identify the correct recipient, if any.
...
impl ConnectionIndex {
    ...
    /// Associate a connection with its first locally-chosen destination CID if
    /// used, or otherwise its current 4-tuple
    fn insert_conn(
        &mut self,
        network_path: FourTuple,
        dst_cid: ConnectionId,
...
pub(crate) struct ConnectionMeta {
    ...
    ///
    /// Only needed to support connections with zero-length CIDs, which cannot
    /// migrate, so we don't bother keeping it up to date.
    network_path: FourTuple,
    side: Side,
    /// Reset tokens provided by the peer for CIDs we're currently sending to
    ///
    /// Since each reset token is for a CID, it is also for a fixed remote
    /// address which is also stored. This allows us to look up which reset
    /// tokens we might expect from a given remote address, see
    /// [`ResetTokenTable`].
    ///
    /// Each path has its own active CID. We use the [`PathId`] as a unique
    /// index, allowing us to retire the reset token when a path is
    /// abandoned.
...
struct PathLocalCids {
    /// Number of connection IDs that have been issued in
    /// (PATH_)NEW_CONNECTION_ID frames
    ///
    /// Another way of saying this is that this is the next sequence number to
    /// be issued.
    issued: u64,
...
impl Incoming {
    ...
    /// Whether the socket address that is initiating this connection has been
    /// validated
    ///
    /// This means that the sender of the initial packet has proved that they
    /// can receive traffic sent to `self.remote_address()`.
    ///
    ...
    /// Can be used to extract information from the TLS ClientHello without
...
pub enum ConnectError {
    ...
    ///
    /// Indicates that a necessary component of the endpoint has been dropped or
    /// otherwise disabled.
    ...
    ///
    /// Examples include attempting to connect to port 0, or using an
    /// inappropriate address family.
    ...
    /// The local endpoint does not support the QUIC version specified in the
...
/// Error for attempting to retry an [`Incoming`] which already bears a token
