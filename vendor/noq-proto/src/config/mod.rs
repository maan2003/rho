...
use crate::cid_generator::{ConnectionIdGenerator, HashedConnectionIdGenerator};
#[cfg(all(feature = "rustls", any(feature = "aws-lc-rs", feature = "ring")))]
use crate::crypto::rustls::{QuicServerConfig, configured_provider};
use crate::crypto::{self, HandshakeTokenKey, HmacKey};
...
impl EndpointConfig {
    ...
    ///
    /// Called once by each `Endpoint` constructed from this configuration to
    /// obtain the CID generator which will be used to generate the CIDs
    /// used for incoming packets on all connections involving that
    /// `Endpoint`. A custom CID generator allows applications to embed
    /// information in local connection IDs, e.g. to support stateless
    ...
    /// Private key used to send authenticated connection resets to peers who
    /// were communicating with a previous instance of this endpoint.
    pub fn reset_key(&mut self, key: Arc<dyn HmacKey>) -> &mut Self {
        self.reset_key = key;
        self
    }

    /// Maximum UDP payload size accepted from peers (excluding UDP and IP
    /// overhead).
    ///
    /// Must be greater or equal than 1200.
    ///
    /// Defaults to 1472, which is the largest UDP payload that can be
    /// transmitted in the typical 1500 byte Ethernet MTU. Deployments on
    /// links with larger MTUs (e.g. loopback or Ethernet with jumbo frames)
    /// can raise this to improve performance at the cost of a linear increase
    /// in datagram receive buffer size.
    ...
    //
    // While most parameters don't need to be readable, this must be exposed to
    // allow higher-level layers, e.g. the `noq` crate, to determine how large a
    // receive buffer to allocate to support an externally-defined
    // `EndpointConfig`.
    //
    // While `get_` accessors are typically unidiomatic in Rust, we favor concision
    // for setters, which will be used far more heavily.
    ...
    ///
    /// Enabled by default. Helps protect against protocol ossification and
    /// makes traffic less identifiable to observers. Disable if helping
    /// observers identify this traffic as QUIC is desired.
    ...
    /// Minimum interval between outgoing stateless reset packets
    ///
    /// Defaults to 20ms. Limits the impact of attacks which flood an endpoint
    /// with garbage packets, e.g. [ISAKMP/IKE amplification]. Larger values
    /// provide a stronger defense, but may delay detection of some error
    /// conditions by clients. Using a [`ConnectionIdGenerator`] with a low
    /// rate of false positives in [`validate`](ConnectionIdGenerator::validate)
    /// reduces the risk incurred by a small minimum reset interval.
    ///
    /// [ISAKMP/IKE
    ...
    /// Optional seed to be used internally for random number generation
    ///
    /// By default, noq will initialize an endpoint's rng using a platform
    /// entropy source. However, you can seed the rng yourself through this
    /// method (e.g. if you need to run noq deterministically or if you are
    /// using noq in an environment that doesn't have a source of
    /// entropy available).
...
pub struct ServerConfig {
    ...
    ///
    /// Improves behavior for clients that move between different internet
    /// connections or suffer NAT rebinding. Enabled by default.
...
impl ServerConfig {
    ...
    /// The preferred IPv4 address that will be communicated to clients during
    /// handshaking
    ///
    ...
    /// The preferred IPv6 address that will be communicated to clients during
    /// handshaking
    ///
    /// If the client is able to reach this address, it will switch to it.
    ...
    /// time
    ///
    /// An [`Incoming`][crate::Incoming] comes into existence when an incoming
    /// connection attempt is received and stops existing when the
    /// application either accepts it or otherwise disposes of it. While
    /// this limit is reached, new incoming connection attempts are immediately
    /// refused. Larger values have greater worst-case memory consumption, but
    /// accommodate greater application latency in handling incoming
    /// connection attempts.
    ///
    /// The default value is set to 65536. With a typical Ethernet MTU of 1500
    /// bytes, this limits memory consumption from this to under 100 MiB--a
    /// generous amount that still prevents memory exhaustion in most
    /// contexts.
    ...
    /// An [`Incoming`][crate::Incoming] comes into existence when an incoming
    /// connection attempt is received and stops existing when the
    /// application either accepts it or otherwise disposes of it. This
    /// limit governs only packets received within that period, and does not
    /// include the first packet. Packets received in excess of this limit
    /// are dropped, which may cause 0-RTT or handshake data to have to be
    /// retransmitted.
    ///
    /// The default value is set to 10 MiB--an amount such that in most
    /// situations a client would not transmit that much 0-RTT data faster
    /// than the server handles the corresponding
    /// [`Incoming`][crate::Incoming].
    ...
    /// retransmitted.
    ///
    /// The default value is set to 100 MiB--a generous amount that still
    /// prevents memory exhaustion in most contexts.
    ...
    ///
    /// This exists to allow system time to be mocked in tests, or wherever else
    /// desired.
    ///
    /// Defaults to [`StdSystemTime`], which simply calls
...
#[cfg(all(feature = "rustls", any(feature = "aws-lc-rs", feature = "ring")))]
impl ServerConfig {
    /// Create a server config with the given certificate chain to be presented
    /// to clients
    ///
...
/// Configuration for sending and handling validation tokens in incoming
/// connections
///
/// Default values should be suitable for most internet applications.
///
/// ## QUIC Tokens
///
/// The QUIC protocol defines a concept of "[address validation][1]".
/// Essentially, one side of a QUIC connection may appear to be receiving QUIC
/// packets from a particular remote UDP address, but it will only consider that
/// remote address "validated" once it has convincing evidence that the address
/// is not being [spoofed][2].
///
/// Validation is important primarily because of QUIC's "anti-amplification
/// limit." This limit prevents a QUIC server from sending a client more than
/// three times the number of bytes it has received from the client on a given
/// address until that address is validated. This is designed to mitigate the
/// ability of attackers to use QUIC-based servers as reflectors in
/// [amplification attacks][3].
///
/// A path may become validated in several ways. The server is always considered
/// validated by the client. The client usually begins in an unvalidated state
/// upon first connecting or migrating, but then becomes validated through
/// various mechanisms that usually take one network round trip. However, in
/// some cases, a client which has previously attempted to connect to a server
/// may have been given a one-time use cryptographically secured "token" that it
/// can send in a subsequent connection attempt to be validated immediately.
///
/// There are two ways these tokens can originate:
///
/// - If the server responds to an incoming connection with `retry`, a "retry
///   token" is minted and sent to the client, which the client immediately uses
///   to attempt to connect again. Retry tokens operate on short timescales,
///   such as 15 seconds.
/// - If a client's path within an active connection is validated, the server
///   may send the client one or more "validation tokens," which the client may
///   store for use in later connections to the same server. Validation tokens
///   may be valid for much longer lifetimes than retry token.
///
/// The usage of validation tokens is most impactful in situations where 0-RTT
/// data is also being used--in particular, in situations where the server sends
/// the client more than three times more 0.5-RTT data than it has received
/// 0-RTT data. Since the successful completion of a connection
/// handshake implicitly causes the client's address to be validated,
/// transmission of 0.5-RTT data is the main situation where a server might be
/// sending application data to an address that could be validated by token
/// usage earlier than it would become validated without token usage.
///
...
///
/// These tokens should not be confused with "stateless reset tokens," which are
/// similarly named but entirely unrelated.
#[derive(Clone)]
...
impl ValidationTokenConfig {
    /// Duration after an address validation token was issued for which it's
    /// considered valid
    ///
    /// This refers only to tokens sent in NEW_TOKEN frames, in contrast to
    /// retry tokens.
    ///
    /// Defaults to 2 weeks.
    ...
    ///
    /// If the `bloom` feature is enabled (which it is by default), defaults to
    /// a default [`BloomTokenLog`][crate::BloomTokenLog], which is suitable
    /// for most internet applications.
    ///
    /// If the `bloom` feature is disabled, defaults to
    /// [`NoneTokenLog`][crate::NoneTokenLog], which makes the server ignore
    /// all address validation tokens (that is, tokens originating
    ...
    /// Number of address validation tokens sent to a client when its path is
    /// validated
    ///
    /// This refers only to tokens sent in NEW_TOKEN frames, in contrast to
    /// retry tokens.
...
impl ClientConfig {
    ...
    /// Configure how to populate the destination CID of the initial packet when
    /// attempting to establish a new connection
    ///
    /// By default, it's populated with random bytes with reasonable length, so
    /// unless you have a good reason, you do not need to change it.
    ///
    /// When prefer to override the default, please note that the generated
    /// connection ID MUST be at least 8 bytes long and unpredictable, as
    /// per section 7.2 of RFC 9000.
    ...
    ///
    /// Defaults to [`TokenMemoryCache`], which is suitable for most internet
    /// applications.
