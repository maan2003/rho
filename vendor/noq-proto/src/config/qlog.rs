...
///
/// This struct is returned from [`QlogFactory::for_connection`] if qlog logging
/// should be enabled for a connection. It allows to set metadata for the qlog
/// trace.
///
/// The trace will be written to the provided writer in the [`JSON-SEQ format`]
/// defined in the qlog spec.
///
...
#[cfg(feature = "qlog")]
impl QlogConfig {
    /// Creates a new [`QlogConfig`] that writes a qlog trace to the specified
    /// `writer`.
...
impl QlogFileFactory {
    /// Creates a new qlog factory that writes files into the specified
    /// directory.
    ...
    ///
    /// If the environment variable `QLOGDIR` is set, qlog traces for all
    /// connections handled by this endpoint will be written into that
    ...
    ///
    /// If not set, events will be recorded relative to the start of the
    /// connection.
