...
use crate::crypto::{HandshakeTokenKey, HmacKey};
use crate::packet::InitialHeader;
use crate::shared::ConnectionId;
use crate::{Duration, RESET_TOKEN_SIZE, ServerConfig, SystemTime, UNIX_EPOCH};
...
/// > attacks. To protect
/// > against such attacks, servers MUST ensure that replay of tokens is
/// > prevented or limited.
/// > Servers SHOULD ensure that tokens sent in Retry packets are only accepted
/// > for a short time,
/// > as they are returned immediately by clients. Tokens that are provided in
/// > NEW_TOKEN frames
/// > (Section 19.7) need to be valid for longer but SHOULD NOT be accepted
/// > multiple times.
/// > Servers are encouraged to allow tokens to be used only once, if possible;
/// > tokens MAY include
/// > additional information about clients to further narrow applicability or
/// > reuse.
///
/// `TokenLog` pertains only to tokens provided in NEW_TOKEN frames.
pub trait TokenLog: Send + Sync {
    /// Record that the token was used and, ideally, return a token reuse error
    /// if the token may have been already used previously
    ///
    /// False negatives and false positives are both permissible. Called when a
    /// client uses an address validation token.
    ///
    /// Parameters:
    /// - `nonce`: A server-generated random unique value for the token.
    /// - `issued`: The time the server issued the token.
    /// - `lifetime`: The expiration time of address validation tokens sent via
    ///   NEW_TOKEN frames, as configured by
    ///   [`ServerValidationTokenConfig::lifetime`][1].
    ///
    /// [1]: crate::ValidationTokenConfig::lifetime
    ///
    /// ## Security & Performance
    ///
    /// To the extent that it is possible to repeatedly trigger false negatives
    /// (returning `Ok` for a token which has been reused), an attacker
    /// could use the server to perform [amplification attacks][2]. The QUIC
    /// specification requires that this be limited, if not prevented fully.
    ///
    /// A false positive (returning `Err` for a token which has never been used)
    /// is not a security vulnerability; it is permissible for a `TokenLog`
    /// to always return `Err`. A false positive causes the token to be
    /// ignored, which may cause the transmission of some 0.5-RTT data to be
    /// delayed until the handshake completes, if a sufficient amount of 0.5-RTT
    /// data it sent.
    ///
...
/// Responsible for storing validation tokens received from servers and
/// retrieving them for use in subsequent connections
pub trait TokenStore: Send + Sync {
    ...
    ///
    /// The same token must never be returned from `take` twice, as doing so can
    /// be used to de-anonymize a client's traffic.
    ///
    /// Called when trying to connect to a server. It is always ok for this to
    /// return `None`.
...
impl IncomingToken {
    /// Construct for an `Incoming` given the first packet header, or error if
    ...
    pub(crate) fn from_header(
        ...
        // In cases where a token cannot be decrypted/decoded, we must allow for the
        // possibility that this is caused not by client malfeasance, but by the
        // token having been generated by an incompatible endpoint, e.g. a
        // different version or a neighbor behind the same load balancer. In
        // such cases we proceed as if there was no token.
        //
        // [_RFC 9000 § 8.1.3:_](https://www.rfc-editor.org/rfc/rfc9000.html#section-8.1.3-10)
        //
        // > If the token is invalid, then the server SHOULD proceed as if the client
        // > did not have
...
pub(crate) struct Token {
    ...
    /// Randomly generated value, which must be unique, and is visible to the
    /// client
    nonce: u128,
}
...
impl Token {
    ...
    fn decode(key: &dyn HandshakeTokenKey, raw_token_bytes: &[u8]) -> Option<Self> {
        ...
        if !data.is_empty() {
            // Consider extra bytes a decoding error (it may be from an incompatible
            // endpoint)
            return None;
        }
...
pub(crate) enum TokenPayload {
    ...
    Retry {
        ...
        address: SocketAddr,
        /// The destination connection ID set in the very first packet from the
        /// client
    ...
    Validation {
        /// The client's IP address (its port is likely to change between
        /// sessions)
        ip: IpAddr,
        /// The time at which this token was issued
        issued: SystemTime,
...
mod test {
    ...
    fn retry_token_sanity() {
        ...
        use crate::{Duration, MAX_CID_SIZE, UNIX_EPOCH};
