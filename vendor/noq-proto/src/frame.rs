...
use crate::coding::{self, BufExt, BufMutExt, Decodable, Encodable, UnexpectedEnd};
use crate::connection::PathId;
use crate::range_set::ArrayRangeSet;
use crate::shared::{ConnectionId, EcnCodepoint};
#[cfg(test)]
...
///
/// This includes some "encoder" types instead of the actual read frame, when
/// writing directly to a buffer is more efficient than building the Frame
/// itself.
...
impl Frame {
    ...
    pub(crate) fn is_1rtt(&self) -> bool {
        ...
        // > If an endpoint receives a multipath-specific frame in a different packet
        // > type, it MUST close the
...
impl RetireConnectionId {
    /// Maximum size of this frame when the frame type is
    ...
    /// Decode [`Self`] from the buffer, provided that the frame type has been
    /// verified (either [`FrameType::PathRetireConnectionId`], or
    /// [`FrameType::RetireConnectionId`])
    ...
    ///
    /// `path_retire_cid` determines whether this frame is a multipath frame.
    /// This is a rough upper estimate, does not squeeze every last byte
    /// out.
...
impl<'a> Encodable for PathAckEncoder<'a> {
    ...
    ///
    /// The [`FrameType`] will be either [`FrameType::PathAckEcn`] or
    /// [`FrameType::PathAck`] depending on whether [`EcnCounts`] are
    /// provided.
    ///
...
// NOTE: Due to lifetimes in fmt::Arguments it's not possible to make this a
// simple function that avoids allocations.
...
mod test {
    ...
    /// Test that encoding and decoding [`ObservedAddr`] produces the same
    /// result.
    #[test]
    ...
    /// Test that encoding and decoding [`RemoveAddress`] produces the same
    /// result
    #[test]
