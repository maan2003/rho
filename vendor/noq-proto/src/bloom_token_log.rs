...
///
/// Parameterizable over an approximate maximum number of bytes to allocate.
/// Starts out by storing used tokens in a hash set. Once the hash set becomes
/// too large, converts it to a bloom filter. This achieves a memory profile of
/// linear growth with an upper bound.
///
/// Divides time into periods based on `lifetime` and stores two filters at any
/// given moment, for each of the two periods currently non-expired tokens could
/// expire in. As such, turns over filters as time goes on to avoid bloom filter
/// false positive rate increasing infinitely over time.
pub struct BloomTokenLog(Mutex<State>);

impl BloomTokenLog {
    /// Construct with an approximate maximum memory usage and expected number
    /// of validation token usages per expiration period
    ///
    ...
    /// Construct with an approximate maximum memory usage and a [bloom filter k
    /// number][bloom]
    ///
    /// [bloom]: https://en.wikipedia.org/wiki/Bloom_filter
    ///
    /// If choosing a custom k number, note that `BloomTokenLog` always
    /// maintains two filters between them and divides the allocation budget
    /// of `max_bytes` evenly between them. As such, each bloom filter will
    /// contain `max_bytes * 4` bits.
...
impl TokenLog for BloomTokenLog {
    fn check_and_insert(
        ...
        let Ok(periods_forward) = expires_at
            ...
            // shouldn't happen unless time travels backwards or lifetime changes or the
            // current system time is before the Unix epoch
        ...
        //
        // the token's nonce needs to guarantee uniqueness because of the role it plays
        // in the encryption of the tokens, so it is 128 bits. but since the
        // token log can tolerate false positives, we trim it down to 64 bits,
        // which would still only have a small collision rate even at
        // significant amounts of usage, while allowing us to store twice as many in
        // the hash set variant.
        //
        // token nonce values are uniformly randomly generated server-side and
        // cryptographically integrity-checked, so we don't need to employ
        // secure hashing to trim it down to 64 bits, we can simply truncate.
        //
...
///
/// With the default validation token lifetime of 2 weeks, this corresponds to
/// one token usage per 1.21 seconds.
    ...
    fn check_and_insert(
        ...
        match self {
            Self::Set(hset) => {
                ...
                // convert to bloom
                // avoid panicking if user passed in filter_max_bytes of 0. we document that
                // this limit is approximate, so just fudge it up to 1.
...
/// Hasher that is the identity operation--it assumes that exactly 8 bytes will
/// be hashed, and the resultant hash is those bytes as a `u64`
#[derive(Default)]
...
fn optimal_k_num(num_bytes: usize, expected_hits: u64) -> u32 {
    // be more forgiving rather than panickey here. excessively high num_bits may
    // occur if the user wishes it to be unbounded, so just saturate.
    // expected_hits of 0 would cause divide-by-zero, so just fudge it up to 1
    // in that case.
    ...
    //
    // we also impose a minimum return value of 1, to avoid making the bloom filter
    // entirely useless in the case that the user provided an absurdly high
    // ratio of hits / bytes.
    (((num_bits as f64 / expected_hits as f64) * LN_2).round() as u32).max(1)
}

// remember to change the doc comment for `impl Default for BloomTokenLog` if
// these ever change
