...
use crate::{Duration, FourTuple, Instant, MAX_CID_SIZE, PathId, ResetToken};
...
impl EndpointEvent {
    ...
    /// Whether this is the event is the event indicating the start of the
    /// draining period.
...
pub(crate) enum EndpointEventInner {
    ...
    ///
    /// Whenever the connection switches to a new remote CID issued by the peer,
    /// it also switches the matching reset token that can be used to abort
    /// this connection. This event provides a new reset token for the
    /// active remote CID.
    ResetToken(PathId, SocketAddr, ResetToken),
    /// Retire the remotely issued reset token for a path, without replacing it
    /// with a new one
    ///
    /// This is like `ResetToken` above, but without replacing the `ResetToken`
    /// with a new one. See `ConnectionIndex::connection_reset_tokens`.
    RetireResetToken(PathId),
    /// The connection needs connection identifiers.
    ///
    /// The fields are:
    /// - The path ID for which the identifiers are needed.
    /// - The time when the identifiers were needed, not used to generate the
    ///   CIDs but sent back via the [`ConnectionEventInner::NewIdentifiers`] so
    ///   the connection can track the lifetime of when it needs to be rotated.
    ///   should be rotated.
...
impl EcnCodepoint {
    ...
    /// Returns whether the codepoint is a CE, signalling that congestion was
    /// experienced
