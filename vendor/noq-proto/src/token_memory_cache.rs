//! Storing tokens sent from servers in NEW_TOKEN frames and using them in
...
/// `TokenStore` implementation that stores up to `N` tokens per server name for
/// up to a limited number of server names, in-memory
#[derive(Debug)]
...
impl State {
    ...
    fn store(&mut self, server_name: &str, token: Bytes) {
        if self.max_server_names == 0 {
            // the rest of this method assumes that we can always insert a new entry so long
            // as we're willing to evict a pre-existing entry. thus, an entry
            // limit of 0 is an edge case we must short-circuit on now.
            return;
        }
        if self.max_tokens_per_server == 0 {
            // similarly to above, the rest of this method assumes that we can always push a
            // new token to a queue so long as we're willing to evict a
            // pre-existing token, so we short-circuit on the edge case of a
            // token limit of 0.
            return;
        }
