...
//!
//! noq-proto contains a fully deterministic implementation of QUIC protocol
//! logic. It contains no networking code and does not get any relevant
//! timestamps from the operating system. Most users may want to use the
//! futures-based noq API instead.
//!
//! The noq-proto API might be of interest if you want to use it from a C or C++
//! project through C bindings or if you want to use a different event loop than
//! the one tokio provides.
//!
//! The most important types are `Endpoint`, which conceptually represents the
//! protocol state for a single socket and mostly manages configuration and
//! dispatches incoming datagrams to the related `Connection`. `Connection`
//! types contain the bulk of the protocol logic related to managing a single
//! connection and all the related state (such as streams).
...
///
/// Including the local ensures good behavior when the host has multiple IP
/// addresses on the same subnet and zero-length connection IDs are in use or
/// when multipath is enabled and multiple paths exist with the same remote, but
/// different local IP interfaces.
///
/// `FourTuple` implements `From<SocketAddr>`, which expands to
/// [`Self::from_remote`].
...
impl FourTuple {
    ...
    /// Returns whether we think the other address probably represents the same
    /// path as ours.
    ///
    /// If we have a local IP set, then we're exact and only match if the
    /// 4-tuples are exactly equal.
    /// If we don't have a local IP set, then we only check the remote addresses
    /// for equality.
    ///
