...
use crate::connection::spaces::PendingReachOutFrames;
use crate::frame::{AddAddress, ReachOut, RemoveAddress};
use crate::{FourTuple, Side, VarInt};

/// Maximum number of times we send a NAT probe to the same remote address in a
/// round.
///
...
///
/// See [`State::retry_delay`] for the capped exponential backoff used. With
/// this we send probes for up to 4s by default.
...
impl CanonicalIpPort {
    ...
    ///
    /// Instead of using ipv4 and ipv6 addresses, this tries to match `ipv6`,
    /// which should indicate whether the local socket supports ipv6 or not.
    ///
    /// If ipv6 is supported, all ipv4 addresses are mapped using ipv6-mapped
    /// ipv4 addresses.
...
pub enum Error {
    /// An endpoint (local or remote) tried to add too many addresses to their
    /// advertised set
...
impl State {
    ...
    ///
    /// When this endpoint is the server within the connection, these addresses
    /// will be sent to the client in add address frames. For clients, these
    /// addresses will be sent in reach out frames when nat traversal
    /// attempts are initiated.
    ///
    ...
    ///
    /// When this endpoint is the server, removed addresses must be reported
    /// with remove address frames. Clients will simply stop reporting these
    /// addresses in reach out frames.
    ///
    /// If a frame should be sent, it is returned.
    ...
    /// Re-queues probes that have not yet succeeded or reached 0 remaining
    /// retries.
    ///
    ...
    ///
    /// Returns true if it was a response to one of the NAT traversal probes and
    /// a path needs to be opened. Note that the NAT probes are not padded
    /// to 1200 bytes so only the address is validated, but not the entire
    /// path.
    ...
    ///
    /// `initial_rtt` must be [`TransportConfig::initial_rtt`] so retries are
...
pub(crate) struct ClientState {
    ...
    ///
    /// These are addresses on which the server is potentially reachable, to use
    /// for NAT traversal attempts.
    ///
    /// They are indexed by their ADD_ADDRESS sequence id and stored in
    /// **canonical form**. Not in the socket-native form as usual. This
    /// because we need to store them so we have the correct sequence IDs.
    remote_addresses: FxHashMap<VarInt, (CanonicalIpPort, ProbeState)>,
    /// Candidate addresses for the local endpoint.
    ///
    /// These are addresses on which we are potentially reachable, to use for
    /// NAT traversal attempts.
    ///
    /// They are stored in **canonical form**, not in socket-native form as
    /// usual. We may nave a reflexive address that is IPv6 even if our
    /// local socket can only handle IPv4.
    local_addresses: FxHashSet<CanonicalIpPort>,
    /// Current nat traversal round.
    round: VarInt,
    /// The probing attempt in the round.
    ///
    /// Probes are sent to all remotes at the same time in a round, at intervals
    /// from [`State::retry_delay`]. This is the number of times probes have
    /// been sent.
    attempt: u8,
    /// The data of PATH_CHALLENGE frames sent in probes.
    ///
    /// These are cleared when a new round starts, so any late-arriving
    /// PATH_RESPONSEs will have no effect.
    ///
    ...
    ///
    /// They are stored in the usual socket-native form. Probes to address
    /// families not addressable by the family are never inserted.
    pending_probes: FxHashSet<IpPort>,
    /// Network paths that were successfully probed but not yet opened.
    ///
    /// When we do not have enough CIDs or free path IDs we might not have been
    /// able to open a new path. This allows us to try re-open the path when
    /// we get new CIDs or a new MAX_PATH_ID.
...
impl ClientState {
    ...
    /// cancelled.
    ///
    /// `ipv6` indicates if the connection runs on a socket that supports IPv6.
    /// If so, then all addresses returned [`PendingReachOutFrames`] will be
    /// IPv6 addresses (and IPv4-mapped IPv6 addresses if necessary).
    /// Otherwise they're all IPv4 addresses.  See
    /// also [`map_to_local_socket_family`].
    ///
    /// # Returns
    ///
    /// The REACH_OUT frames that need to be sent to the peer and the probed
    /// addresses. The probed addresses are only informational, the pending
    /// probes are stored in [`Self::pending_probes`].
    ///
    /// If the probed addresses are non-empty the `NatTraversalProbeRetry` timer
    /// needs to be set.
    ...
    ///
    /// On success returns the address if it was new to the set. It will error
    /// when the set has no capacity for the address.
    ///
    /// If this is called while a round is in progress this will effectively add
    /// the address to the current round. There is no guarantee however that
    /// the current round is still in progress however, if the last
    /// [`Self::queue_retries`] call returned `false` the round has stopped.
    // TODO(flub): probably should add an event to signal that the round is
    // finished, so    that the application knows to start a new round.
    pub(crate) fn add_remote_address(
        ...
        match self.remote_addresses.entry(seq_no) {
            Entry::Occupied(mut occupied_entry) => {
                ...
                // The value might be different. This should not happen, but we assume that the
                // new address is more recent than the previous, and thus worth
                // updating
    ...
    ///
    /// An address is valid as long as it does not change the value of a known
    /// address id.
    ...
    /// In that case [`Self::pop_pending_path_open`] should be called to
    /// open the next path.
...
pub(crate) struct ServerState {
    ...
    /// This is set by the remote endpoint.
    max_local_addresses: usize,
    /// Candidate addresses the server reports as potentially reachable, to use
    /// for nat traversal attempts.
    ///
    /// They are stored in **canonical form**, not in socket-native form as
    /// usual. We may nave a reflexive address that is IPv6 even if our
    ...
    /// Current nat traversal round
    ///
    /// Servers keep track of the client's most recent round and cancel probing
    /// related to previous rounds.
    round: VarInt,
    /// The probing attempt in the round.
    ///
    /// Probes are sent to all remotes at the same time in a round, at intervals
    ...
    /// Queued probes to be sent in the next [`poll_transmit`] call.
    ///
    /// At the beginning of a round this is populated from REACH_OUT frames and
    /// at every retry this is populated from [`Self::remotes`].
    ///
    /// [`poll_transmit`]: crate::connection::Connection::poll_transmit
    pending_probes: FxHashSet<IpPort>,
}
...
impl ServerState {
    ...
    ///
    /// This might ignore the reach out frame if it belongs to an older round or
    /// if the frame contains an IPv6 address while the local socket is
    /// IPv4-only.
    ///
    /// If a new round was started, the `NatTraversalProbeRetry` timer needs to
    /// be reset.
    ...
    /// Re-queues probes that have not yet succeeded or reached
    /// [`MAX_NAT_PROBE_ATTEMPTS`].
    ///
    /// Returns whether any probes are now queued to send. In this case the
    /// `NatTraversalProbeRetry` timer needs to be reset.
...
/// If the given address is an IPv6 address, but our local socket doesn't
/// support IPv6, then this returns `None`.
