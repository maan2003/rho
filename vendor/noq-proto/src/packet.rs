...
use crate::connection::{EncryptionLevel, SpaceKind};
use crate::{ConnectionId, PathId, crypto};
...
impl PartialDecode {
    /// Begin decoding a QUIC packet from `bytes`, returning any trailing data
    /// not part of that packet
    pub fn new(
        bytes: BytesMut,
...
///
/// This is commonly used for when a buffer is passed and the user may not write
/// past a given size. It allows the user of such a buffer to know the current
/// cursor position in the buffer. The maximum write size is usually passed in
/// the same unit as [`BufLen::len`]: bytes since the buffer start.
...
// TODO(flub): Would be grand to make this typesafe by adding a generic that
// indicates    whether the payload is encrypted or decrypted.
pub(crate) struct Packet {
    /// The decoded header.
    pub(crate) header: Header,
    /// The encoded header data, with header protection removed (i.e.
    /// decrypted).
...
impl PartialEncode {
    ...
    /// Creates a [`PartialEncode`] that has not encoded a header into the
    /// buffer.
    ///
    /// This is used exclusively for testing as such a type is otherwise
    /// invalid.
    #[cfg(test)]
...
/// A [`ConnectionIdParser`] implementation that assumes the connection ID is of
/// fixed length
