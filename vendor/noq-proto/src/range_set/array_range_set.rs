...
///
/// `Range<T>` only implements [`IntoIterator`] for types implementing
/// `std::iter::Step`, but that trait is unstable. We can work around this by
/// duplicating these functions for [`u32`] and [`u64`]. Only we don't currently
/// use the u32 version so it is u64-only for now.
...
impl proptest::arbitrary::Arbitrary for ArrayRangeSet {
    ...
    fn arbitrary_with(_: Self::Parameters) -> Self::Strategy {
        ...
        // Generate 1-8 ranges. Each range is defined by a gap from the previous and a
        // size. We use small values to keep encoding reasonable.
