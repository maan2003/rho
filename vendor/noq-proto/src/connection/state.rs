...
impl State {
    ...
    pub(super) fn move_to_draining(&mut self, error: Option<ConnectionError>) {
        ...
        // If no error is provided and we were in the `Closed` state with an unread
        // non-local error, preserve that error so `take_error` can still
        // surface it. Otherwise the `ConnectionLost` event would never be
        // emitted, breaking the invariant that a drained noq-proto connection
        // will always produce a `ConnectionLost` event.
    ...
    /// Enters the Closing connection state, due to changes in the
    /// [`Connection`] state.
    ///
    ...
    ///
    /// Crucially, this is to be used when internal state changes result in
    /// initiating an immediate close. The resulting error will be surfaced
    /// as a [`ConnectionLost`] event in [`Connection::poll`].
    ///
    /// # Panics
    ///
    ...
    /// having initiated immediate close.
    ///
    /// Crucially, this is to be used when immediate close is entered due to an
    /// API being called. It means the close will NOT surface as a
    /// [`ConnectionLost`] event in [`Connection::poll`].
    ///
    /// See [`Self::move_to_closed`] for when the internal state changes
    /// resulted in initiating an immediate close.
    ///
    /// # Panics
    ///
    /// Panics if the state is later than established.
...
pub(super) enum StateType {
    ...
    ///
    /// So the side that initiates an immediate close will stay in this state
    /// while it is waiting for the remote to also send a CONNECTION_CLOSE.
    /// The side that receives a connection close will skip straight to
    /// [`StateType::Draining`].
    Closed,
    /// The connection is draining, giving time to gracefully discard any
    /// in-flight packets.
    ///
    ...
    ///
    /// This is a terminal state in which the connection does nothing and can
    /// never do anything again. Waiting for the application to drop the
    /// [`Connection`] struct.
    ///
    /// [`Connection`]: super::Connection
    Drained,
...
enum InnerState {
    ...
    Closed {
        /// The reason the remote closed the connection, or the reason we are
    ...
    /// Waiting for application to call close so we can dispose of the
    /// resources.
    Drained {
        /// Why the connection was lost, if it has been.
        error: Option<ConnectionError>,
...
pub struct Handshake {
    ...
    ///
    /// We allow the server to migrate during the handshake as long as we have
    /// not received an authenticated handshake packet: it can send a
    /// response from a different address than we sent the initial to.  This
    /// allows us to send the initial packet over multiple paths - by means
    /// of an IPv6 ULA address that copies the packets sent to it to
    /// multiple destinations - and accept one response.
    ///
    /// This is only ever set to true if for a client which hasn't yet received
    /// an authenticated handshake packet.  It is set back to false in
    /// [`super::Connection::on_packet_authenticated`].
    ///
    /// THIS IS NOT RFC 9000 COMPLIANT!  A server is not allowed to migrate
    /// addresses, other than using the preferred-address transport
    /// parameter.
