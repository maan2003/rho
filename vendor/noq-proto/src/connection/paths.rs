...
use super::spaces::{PacketNumberSpace, SentPacket};
use super::{PathStats, SpaceKind};
use crate::coding::{self, Decodable, Encodable};
use crate::connection::{MAX_BACKOFF_EXPONENT, MAX_PTO_INTERVAL};
use crate::frame::ObservedAddr;
use crate::{
    ConnectionId, Duration, FourTuple, Instant, TIMER_GRANULARITY, TransportConfig,
    TransportErrorCode, VarInt, congestion,
};
...
///
/// A single path ID can migrate according to the rules in RFC9000 §9, either
/// voluntary or involuntary. We need to keep the [`PathData`] of the previously
/// used such path available in order to defend against migration attacks (see
/// RFC9000 §9.3.1, §9.3.2 and §9.3.3) as well as to support path probing
/// (RFC9000 §9.1).
#[derive(Debug)]
...
///
/// With QUIC-Multipath a path is identified by a [`PathId`] and it is possible
/// to have multiple paths on the same 4-tuple. Furthermore a single
/// QUIC-Multipath path can migrate to a different 4-tuple, in a similar manner
/// as an RFC9000 connection can use "path migration" to move to a different
/// 4-tuple. There are thus two states we keep for paths:
///
/// - [`PacketNumberSpace`]: The state for a single packet number space, i.e.
///   [`PathId`], which remains in place across path migrations to different
///   4-tuples.
///
///   This is stored in [`PacketSpace::number_spaces`] indexed on [`PathId`].
///
/// - [`PathData`]: The state we keep for each unique 4-tuple within a space. Of
///   note is that a single [`PathData`] can never belong to a different
///   [`PacketNumberSpace`].
///
///   This is stored in [`Connection::paths`] indexed by the current [`PathId`]
/// for which   space it exists. Either as the primary 4-tuple or as the
/// previous 4-tuple just after a   migration.
///
/// It follows that there might be several [`PathData`] structs for the same
/// 4-tuple if several spaces are sharing the same 4-tuple. Note that during the
/// handshake, the Initial, Handshake and Data spaces for [`PathId::ZERO`] all
/// share the same [`PathData`].
///
...
pub(super) struct PathData {
    ...
    /// Whether the last `poll_transmit_on_path` call yielded no data because
    /// there was no outgoing application data.
    ///
    /// The RFC writes:
    /// > When bytes in flight is smaller than the congestion window and sending
    /// > is not pacing limited,
    /// > the congestion window is underutilized. This can happen due to
    /// > insufficient application data
    /// > or flow control limits. When this occurs, the congestion window SHOULD
    /// > NOT be increased in
    ...
    /// Whether to trigger sending another PATH_CHALLENGE in the next
    /// poll_transmit.
    ///
    /// This is picked up by [`super::Connection::space_can_send`]. These are
    /// **not** retransmittable, which is why they are not part of the
    /// `PathRetransmits`.
    ///
    /// Only used for **on-path** challenges, like RFC9000-style path migration
    /// and multipath path validation (for opening).
    ///
    /// This is **not used** for n0 nat traversal challenge sending, which is
    /// off-path.
    pub(super) pending_challenge: bool,
    /// On-path path challenges sent that we didn't receive a path response for
    /// yet.
    ...
    ///
    /// Initially equal to `use_stateless_retry` for servers, and becomes false
    /// again on every migration. Always true for clients.
    ...
    /// Packet number of the first packet sent after an RTT sample was collected
    /// on this path
    ///
    ...
    /// Queue of data that must be sent over this specific
    /// [`PathData::generation`] path.
    pub(super) pending: PathRetransmits,
    /// Observed address frame with the largest sequence number received from
    ...
    ///
    /// With RFC9000 §9 style migration (i.e. not multipath) the PathId does not
    /// change and hence packet numbers continue. This is used to determine
    /// whether a packet was sent on such an earlier path. Insufficient to
    /// determine if a packet was sent on a later path.
    first_packet: Option<u64>,
    /// The number of times a tail-loss probe has been sent without receiving an
    /// ack.
    ///
    /// This is incremented by one every time the [`LossDetection`] timer fires
    /// because a tail-loss probe needs to be sent. Once an acknowledgement
    /// for a packet is received again it is reset to 0. Used to compute the
    /// PTO duration.
    ///
    ...
    ///
    /// If expired, the path will be abandoned.  This is different from the
    /// connection-wide idle timeout which closes the connection if expired.
    pub(super) idle_timeout: Option<Duration>,
    /// Keep alives to send on this path
    ///
    /// There is also a connection-level keep alive configured in the
    /// [`TransportParameters`].  This triggers activity on any path which can
    /// keep the connection alive.
    ///
    /// [`TransportParameters`]: crate::transport_parameters::TransportParameters
    pub(super) keep_alive: Option<Duration>,
    /// Whether to reset the idle timer when the next ack-eliciting packet is
    /// sent.
    ///
    /// Whenever we receive an authenticated packet the connection and path idle
    /// timers are reset if a maximum idle timeout was negotiated. However
    /// on the first ack-eliciting packet *sent* after this the idle timer
    /// also needs to be reset to avoid the idle timer firing while the sent
    /// packet is in-fight. See <https://www.rfc-editor.org/rfc/rfc9000.html#section-10.1>.
    pub(super) permit_idle_reset: bool,

    /// Whether we're currently draining the path after having abandoned it.
    ///
    /// This should only be true when a path discard timer is armed, and after
    /// the path was abandoned (and added to the abandoned_paths set).
    ///
    ...
    ///
    /// When a migration happens on the same [`PathId`] we still detect a change
    /// in the 4-tuple and generate a new [`PathData`] for it. Each such
    /// generation has a unique value to keep track of which 4-tuple a
    /// packet belonged to.
    generation: u64,
}
...
impl PathData {
    ...
    /// Whether we're in the process of validating this path with
    /// PATH_CHALLENGEs
    pub(super) fn is_validating_path(&self) -> bool {
        !self.unconfirmed_challenges.is_empty() || self.pending_challenge
    }

    /// Indicates whether we're a server that hasn't validated the peer's
    /// address and hasn't received enough data from the peer to permit
    /// sending `bytes_to_send` additional bytes
    ...
    /// The earliest time at which an on-path challenge we sent is considered
    /// lost.
    ...
    pub(super) fn on_path_response_received(
        ...
        // > contains the
        // > data that was sent in a previous PATH_CHALLENGE frame. A PATH_RESPONSE
        // > frame
        // > received on any network path validates the path on which the PATH_CHALLENGE
        // > was
        // > sent.
        //
        // At this point we have three potentially different network paths:
        // - current network path (`Self::network_path`)
        // - network path used to send the path challenge
        //   (`SentChallengeInfo::network_path`)
        // - network path over which the response arrived (not needed)
        //
        // As per above spec quote, this only validates the network path on which this
        // was *sent*, regardless of the path on which it was received in order
        // to protect against off-path packet forwarding attacks.
    ...
    /// Removes all on-path challenges we remember and cancels sending new
    /// on-path challenges.
    ...
    ///
    /// If the address was updated, it's returned to be informed to the
    /// application.
...
impl RttEstimator {
    ...
    ///
    /// This only resets the initial_rtt **if** no samples have been recorded
    /// yet. If there are any recorded samples the initial estimate can not
    /// be adjusted after the fact.
    ///
    /// This is useful when you receive a PATH_RESPONSE in the first packet
    /// received on a new path. In this case you can use the delay of the
    /// PATH_CHALLENGE-PATH_RESPONSE as the initial RTT to get a better
    /// expected estimation.
    ///
    /// A PATH_CHALLENGE-PATH_RESPONSE pair later in the connection should not
    /// be used explicitly as an estimation since PATH_CHALLENGE is an
    /// ACK-eliciting packet itself already.
...
impl PathResponses {
    pub(crate) fn push(&mut self, packet: u64, token: u64, network_path: FourTuple) {
        ...
        ///
        /// If we've negotiated the n0 NAT Traversal extension, and one user
        /// might have a lot of addresses, e.g. because of having lots
        /// of interfaces (we've seen >25 interfaces on Macs with docker
        /// and other things), then we need to be able to process at least
        /// as many PATH_CHALLENGE frames as there are interfaces.
        /// On top of that, there are retries, which make it possible that we
        /// need to process even more.
        ///
        /// Considering that there can be up to 2 `PathData`s per active
        /// `PathId`, and reasonable default values for maximum
        /// concurrent multipath paths are ~8 and each `PathResponse`
        ...
        if self.pending.len() < MAX_PATH_RESPONSES {
            ...
            // We don't expect to ever hit this with well-behaved peers, so we don't bother
            // dropping older challenges.
    ...
    pub(crate) fn pop_off_path(&mut self, network_path: FourTuple) -> Option<(u64, FourTuple)> {
        ...
        // We use an exact comparison here, because once we've received for the first
        // time, we really should either already have a local_ip, or we will
        // never get one (because our OS doesn't support it). And even if we get
        // it wrong we are only slightly less efficient and would not include
        // other on-path data in the packet.
        if response.network_path == network_path {
            // We don't bother searching further because we expect that the on-path response
            // will get drained in the immediate future by a call to
            // `pop_on_path`
            return None;
        }
        self.pending.pop();
    ...
    pub(crate) fn pop_on_path(&mut self, network_path: FourTuple) -> Option<u64> {
        ...
        if response.network_path != network_path {
            // We don't bother searching further because we expect that the off-path
            // response will get drained in the immediate future by a call to
            // `pop_off_path`
            return None;
        }
        self.pending.pop();
...
pub(super) struct PathStatusState {
    ...
    /// Local sequence number, for both PATH_STATUS_AVAILABLE and
    /// PATH_STATUS_BACKUP
    ///
...
pub enum PathStatus {
    ...
    ///
    /// If multiple paths are available, packets will be scheduled on whichever
    /// has capacity.
    #[default]
    Available,
    /// Paths marked as backup will only be used if there are no available paths
    ///
    /// If the max_idle_timeout is specified the path will be kept alive so that
    /// it does not expire.
    Backup,
}
...
pub enum PathEvent {
    ...
    /// before having been established, no [`Self::Established`] event is
    /// emitted.
    ///
    /// This event will always be followed by [`Self::Discarded`] after some
    /// time.
    #[non_exhaustive]
    Abandoned {
    ...
    ///
    /// This event is the last event for a path, and is always emitted after
    /// [`Self::Abandoned`].
    #[non_exhaustive]
    Discarded {
        /// Which path had its state dropped
        id: PathId,
        /// The final path stats, they are no longer available via
        /// [`Connection::stats`]
        ///
    ...
    ///
    /// The local status is not changed because of this event. It is up to the
    /// application to update the local status, which is used for packet
    /// scheduling, when the remote changes the status.
    #[non_exhaustive]
    RemoteStatus {
    ...
    #[non_exhaustive]
    ObservedAddr {
        /// Path over which the observed address was reported, [`PathId::ZERO`]
        /// when multipath is not negotiated
        id: PathId,
...
pub enum PathAbandonReason {
    ...
    ValidationFailed,
    /// We didn't receive any data from the remote within the path's idle
    /// timeout.
    TimedOut,
...
pub enum SetPathStatusError {
    /// Error indicating that a path has not been opened or has already been
    /// abandoned
    #[error("closed path")]
    ClosedPath,
    /// Error indicating that this operation requires multipath to be negotiated
    /// whereas it hasn't been
...
pub(super) struct PathRetransmits {
    ...
    ///
    /// This only happens if both peers agree to do so based on their transport
    /// parameters.
