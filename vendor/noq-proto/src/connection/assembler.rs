...
impl Assembler {
    ...
    ///
    /// This makes sure we're not unnecessarily holding on to many larger
    /// allocations. We merge contiguous chunks in the process of doing so.
    ...
    // Note: If a packet contains many frames from the same stream, the estimated
    // over-allocation will be much higher because we are counting the same
...
mod test {
    ...
    fn no_duplicate_after_mode_switch() {
        // Regression test: bytes read in ordered mode should not be returned again in
        // unordered mode
        let mut x = Assembler::new();
        x.insert(0, Bytes::from_static(b"a"), 1);
