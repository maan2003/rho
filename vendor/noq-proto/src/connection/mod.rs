...
use crate::connection::paths::PathRetransmits;
use crate::connection::qlog::{QlogRecvPacket, QlogSink};
use crate::connection::spaces::LostPacket;
use crate::connection::stats::PathStatsMap;
use crate::connection::timer::{ConnTimer, PathTimer};
...
use crate::token::{ResetToken, Token, TokenPayload};
...
use crate::{
    ...
    TransportError, TransportErrorCode, VarInt, n0_nat_traversal,
};
...
///
/// Objects of this type receive [`ConnectionEvent`]s and emit
/// [`EndpointEvent`]s and application [`Event`]s to make progress. To handle
/// timeouts, a `Connection` returns timer updates and expects timeouts through
/// various methods. A number of simple getter methods are exposed
...
///   convenience we refer to this as "performing I/O" below, however as per the
///   design of this library none of the functions actually perform system-level
///   I/O. For example, [`read`](RecvStream::read) and
///   [`write`](SendStream::write), but also things like
...
/// increasing time. Specifically, calling
/// [`handle_timeout`](Self::handle_timeout) with events of the same [`Instant`]
...
pub struct Connection {
    ...
    /// Counter to uniquely identify every [`PathData`] created in this
    /// connection.
    ///
    /// Each [`PathData`] gets a [`PathData::generation`] that is unique among
    /// all [`PathData`]s created in the lifetime of this connection. This
    /// helps identify the correct path when RFC9000-style migrations
    /// happen, even when they are aborted.
    ///
    /// Multipath does not change this, each path can also undergo RFC9000-style
    /// migrations. So a single multipath path ID could see several
    /// [`PathData`]s each with their unique [`PathData::generation].
    ...
    /// The value that the server included in the Source Connection ID field of
    /// a Retry packet, if one was received
    ...
    /// Active and surplus CIDs issued by the remote, for future use on new
    /// paths.
    ///
    /// These are given out before multiple paths exist, also for paths that
    /// will never exist.  So if multipath is supported the number of paths
    /// here will be higher than the actual number of paths in use.
    remote_cids: FxHashMap<PathId, CidQueue>,
    /// Attributes of CIDs generated by local endpoint
    ///
    /// Any path that is allowed to be opened is present in this map, as well as
    /// the already opened paths. However since CIDs are issued async by the
    /// endpoint driver via connection events it can not be used to know if
    /// CIDs have been issued for a path or
    ...
    ///
    /// The connection-level stats returned by [`Self::stats`] are the sum of
    /// the stats of all the paths. However once a path is discarded it gets
    /// added to this field instead so we do not have to keep an ever
    /// growing number of paths stats in memory.
    ...
    ///
    /// Initially set from the
    /// [`TransportConfig::max_concurrent_multipath_paths`]. Even
    /// when multipath is disabled this will be set to 1, it is not used in that
    /// case though.
    max_concurrent_paths: NonZeroU32,
    /// Local maximum [`PathId`] to be used
    ///
    /// This is initially set to [`TransportConfig::get_initial_max_path_id`]
    /// when multipath is negotiated, or to [`PathId::ZERO`] otherwise. This
    /// is essentially the value of the highest MAX_PATH_ID frame sent.
    ///
    ...
    ///
    /// This is initially set to the peer's
    /// [`TransportParameters::initial_max_path_id`] when multipath is
    /// negotiated, or to [`PathId::ZERO`] otherwise. A peer may increase this
    /// limit by sending [`Frame::MaxPathId`] frames.
    remote_max_path_id: PathId,
    /// The greatest [`PathId`] we have issued CIDs for
    ///
    /// CIDs are only issued for `min(local_max_path_id, remote_max_path_id)`.
    /// It is not possible to use [`Connection::local_cid_state`] to know if
    /// CIDs have been issued since they are issued asynchronously by the
    /// endpoint driver.
    max_path_id_with_cids: PathId,
    /// The paths already abandoned
    ///
    /// They may still have some state left in [`Connection::paths`] or
    /// [`Connection::local_cid_state`] since some of this has to be kept around
    /// for some time after a path is abandoned.
...
impl Connection {
    ...
    /// Opens a new path only if no path on the same network path currently
    /// exists.
    ///
    /// Returns `(path_id, true)` if the path already existed, or `(path_id,
    /// false)` if was opened.
    ///
    ...
    /// the remote, the OS is likely to use the same interface to talk to said
    /// remote.
    ///
    /// See also [`open_path`].
    ///
    ...
    /// Opens a new path.
    ///
    /// Further errors might occur and they will be emitted in
    /// [`PathEvent::Abandoned`] events with this path id.  Once the path is
    /// opened and can carry application data it will be reported using a
    /// [`PathEvent::Established`] event.
    pub fn open_path(
        &mut self,
        network_path: FourTuple,
        initial_status: PathStatus,
    ...
    /// It does allow closing paths which have not yet been opened, as e.g. is
    /// the case when receiving a PATH_ABANDON from the peer for a path that
    ...
    ///
    /// Other than [`Self::close_path`] this allows to specify the reason for
    /// the path being closed. Internally, this should be used over
    /// [`Self::close_path`].
    ...
    fn abandon_path(&mut self, now: Instant, path_id: PathId, reason: PathAbandonReason) {
        ...
        // Cleanup retransmits across ALL paths (CIDs for path_id may have been
        ...
        // Note: remote CIDs are NOT removed here. They are removed when the
        // PATH_ABANDON frame is actually written to a packet (in
        // populate_packet). This allows sending PATH_ABANDON on the abandoned
        // path itself when no other path exists (#509).
    ...
    // TODO(flub): Probably should also be some kind of path event?  Not even sure
    // if I like    this as an API, but for now it allows me to write a test
    // easily. TODO(flub): Technically this should be a
    // Result<Option<PathSTatus>>?
    ...
    ///
    /// The PathIdle timer is immediately re-armed accounting for
    /// already-elapsed idle time. Setting `None` disables the timeout and
    /// stops the timer.
    ///
    ...
    /// Find an open, validated path that's on the same network path as the
    /// given network path.
    ///
    ...
    fn find_validated_path_on_network_path(
        ...
        // TODO(@divma): we might want to ensure the path has been recently
        // active to consider the address validated
    ...
    fn ensure_path(
        ...
        // To open a path locally we need to send a packet on the path. Sending a
        // challenge guarantees this.
        ...
        // If the remote opened this path we may not have CIDs for it. For locally
        // opened paths the caller should have already made sure we have CIDs
        // and refused to open it if there were none.
        if !self.remote_cids.contains_key(&path_id) {
            ...
            self.spaces[SpaceId::Data]
                ...
                .path_cids_blocked
                .insert(path_id, VarInt(0));
            // Do not abandon this path right away. CIDs might be in-flight
            // still and arrive soon. It is up to the remote to
    ...
    pub fn poll_transmit(
        ...
        // Each call to poll_transmit can only send datagrams to one destination,
        // because all datagrams in a GSO batch are for the same destination.
        // Therefore only datagrams for one destination address are produced for
        // each poll_transmit call.
    ...
    ///
    /// While this information is only returned for a single path, it is
    /// important to know that this information remains static for the
    /// entire span of a single [`Connection::poll_transmit`] call. In other
    /// words, the return value is purely functional and only depends on the
    /// [`PathId`] **during a single** `poll_transmit` call. It can be
    /// computed up-front for all paths but we don't do that because it
    ...
    /// # Panics
    ///
    /// This will panic if called for a path for which we do not have any
    /// [`PathData`], like so many other functions we have. But this is the
    /// only one to document this in its doc comment. Maybe that should
    /// change. Eventually we'll refactor things for this panic to go away.
    fn scheduling_info(&self, path_id: PathId) -> PathSchedulingInfo {
        ...
        // PATH_ABANDON is normally sent together with other SpaceKind::Data frames. But
        // if there is no other validated space to send it on, it can be sent on
        // the path to be abandoned itself if that was validated.
    ...
    ///
    /// This is not quite the same as for a multipath packet space, since
    /// [`PathId::ZERO`] has 3 packet spaces, which this handles.
    ///
    ...
    fn poll_transmit_on_path(
        ...
        // Whether the last packet in the datagram must be padded so the datagram takes
        // up an exact size. An earlier space can decide to not fill an entire
        // datagram and require the next space to fill it further. But may need
        // a specific size of the datagram containing the packet. The final
        // packet built in the datagram must pad to this size.
        ...
        // If we end up not sending anything, we need to know if that was because there
        // was nothing to send or because we were congestion blocked.
        ...
        for space_id in SpaceId::iter() {
            ...
            match self.poll_transmit_path_space(
                ...
                PollPathSpaceStatus::NothingToSend {
                    ...
                    congestion_blocked |= cb;
                    // Continue checking other spaces, tail-loss probes may need
                    // to be sent in all spaces.
                }
    ...
    /// poll_transmit logic for a QUIC-MULTIPATH packet number space (PathID +
    /// SpaceId).
    #[must_use]
    fn poll_transmit_path_space(
        ...
        // - If coalescing, finish packet without padding to leave space in the
        //   datagram.
        ...
        loop {
            ...
            // ACK-only packets should only be sent when explicitly allowed. If we write
            // them due to any other reason, there is a bug which leads to one
            // component announcing write readiness while not writing any data.
            // This degrades performance. The condition is only checked if the
            // full MTU is available and when potentially large fixed-size
            // frames aren't queued, so that lack of space in the datagram isn't the reason
            // for just writing ACKs.
            debug_assert!(
    ...
    /// [`Connection::space_can_send`] issued will return some frames to be
    /// sent. Including having to predict which packet number space it will
    /// be invoked with. This depends on
    /// how both [`Connection::poll_transmit_on_path`] and
    /// [`Connection::poll_transmit_path_space`] behave.
    ///
    /// This is needed to determine if packet coalescing can happen. Because the
    /// last packet in a datagram may need to be padded and thus we must
    /// know if another packet will follow or not.
    ///
    /// The next packet can be either in the same space, or in one of the
    /// following spaces on the same path. Because a 0-RTT packet can be
    /// coalesced with a 1-RTT packet and both are in the Data(PathId::ZERO)
    /// space. Previous spaces are not checked, because packets are built
    /// from Initial to Handshake to Data spaces.
    ...
    fn path_congestion_check(
        ...
        // Tail loss probes must not be blocked by congestion, or a deadlock could
        // arise.
    ...
    fn send_prev_path_challenge(
        ...
        // don't bother accounting for possible retirement of that prev_cid because this
        // is sent once, immediately after migration, when the CID is known to
        // be valid. Even if a post-migration packet caused the CID to be
        // retired, it's fair to pretend this is sent first.
    ...
    fn send_off_path_path_response(
        ...
        //
        // If we are a client doing NAT traversal, always include a PATH_CHALLENGE with
        // any off-path PATH_RESPONSE. No need to schedule any retries for this,
        // if NAT traversal is taking place then this remote already is being
        // probed with retries, this only speeds up a successful traversal.
        if self
    ...
    fn send_nat_traversal_path_challenge(
        ...
        // TODO: Using the active CID here makes the paths linkable. This is a violation
        // of    RFC9000 but something we want to accept in the short term.
        // Eventually we aim    to fix up the supply of CIDs sufficiently so
        // that we can keep paths unlinkable    again.
        let Some(cid) = self
            .remote_cids
            .get(&path_id)
    ...
    /// Indicate what types of frames are ready to send for the given packet
    /// number space.
    ///
    /// Only for on-path data.
    ///
    /// *packet_size* is the number of bytes available to build the next packet.
    /// *connection_close_pending* indicates whether a CONNECTION_CLOSE frame
    /// needs to be sent.
    fn space_can_send(
        &mut self,
        space_id: SpaceId,
    ...
    ///
    /// Will execute protocol logic upon receipt of a connection event, in turn
    /// preparing signals (including application `Event`s, `EndpointEvent`s
    /// and outgoing datagrams) that should be extracted through the
    /// relevant methods.
    ...
    ///
    /// Packets sent on the wrong network path can be entirely ignored, saving
    /// further processing.
    ///
    /// Returns true if a packet coming in for this `path_id` over given
    /// `network_path` should be discarded.
    ...
    ///
    /// RFC9000 §9 and QNT both have probing packets which may arrive from new
    /// paths. This indicates whether these are allowed or not. This is a
    /// strict superset from [`Self::peer_may_migrate`]: every network path that
    /// may be migrated to, may also be probed. But e.g. servers may not
    /// migrate, but can be allowed to probe.
    // TODO(flub): In RFC9000 the server is allowed to send off-path probing packets
    //    once the client has been probing such a 4-tuple. These probes are
    // currently    not yet recognised and will end up being discarded because
    // of this.    See https://github.com/n0-computer/noq/issues/607.
    ...
    ///
    /// QUIC relies on stable endpoints during the handshake. So other than the
    /// server's preferred_address transport parameter no side may migrate
    /// before the handshake is completed.
    ///
    /// It is noteworthy that for iroh we allow server migrations during the
    /// handshake when [`state::Handshake::allow_server_migration`] is
    /// enabled, but that is handled earlier in [`Self::handle_packet`] and
    /// without probing the current and previous paths.
    ...
    /// Whether our local IP address is allowed to change with new incoming
    /// packets.
    ///
    /// Incoming packets show us the local IP address we received a packet on,
    /// which could be different from what we thought due to e.g. NAT
    /// rebinding or moving from mobile data to WiFi without being notified
    /// of the network change.
    ///
    /// This is only allowed to happen after the handshake is confirmed and when
    /// we are the client. Unless QNT is negotiated in which case the server
    /// is also allowed to migrate.
    ///
    /// Be aware that probing packets, which do not exist in Multipath without
    /// QNT, are exempt from this.
    ...
    ///
    /// Executes protocol logic, potentially preparing signals (including
    /// application `Event`s, `EndpointEvent`s and outgoing datagrams) that
    /// should be extracted through the relevant methods.
    ///
    /// It is most efficient to call this immediately after the system clock
    /// reaches the latest `Instant` that was output by `poll_timeout`;
    /// however spurious extra calls will simply no-op and therefore are
    /// safe.
    ...
    /// communications have been completed, e.g. by calling
    /// [`SendStream::finish`] on outstanding streams and waiting for the
    /// corresponding [`StreamEvent::Finished`] event.
    ///
    /// If [`Streams::send_streams`] returns 0, all outstanding stream data has
    /// been delivered. There may still be data from the peer that has not
    /// been received.
    ///
    ...
    /// [`Connection::poll`] call, because the API call already propagated the
    /// error to the user.
    ///
    /// Not to be used when entering immediate close due to an internal state
    /// change based on an event. See [`State::move_to_closed_local`] for
    /// details.
    ///
    ...
    pub fn ping(&mut self) {
        // TODO(flub): This is very brute-force: it pings *all* the paths.  Instead it
        // would    be nice if we could only send a single packet for this.
    ...
    ///
    /// This can be useful for testing key updates, as they otherwise only
    /// happen infrequently.
    pub fn force_key_update(&mut self) {
        ...
        if self.crypto_state.prev_crypto.is_some() {
            // We already just updated, or are currently updating, the keys. Concurrent key
            // updates are illegal.
    ...
    ///
    /// If this returns `false`, the connection may be either established or
    /// closed, signaled by the emission of a
    /// [`Connected`](Event::Connected) or
    /// [`ConnectionLost`](Event::ConnectionLost) event respectively. Note
    /// that locally-initiated closes via [`close()`](Self::close) do not
    /// emit a `ConnectionLost` event.
    ///
    /// For an established connection this essentially means the handshake is
    /// **completed**, but not necessarily yet confirmed.
    ...
    ///
    /// Closed connections cannot transport any further data. A connection
    /// becomes closed when either peer application intentionally closes it,
    /// or when either transport layer detects an error such as a time-out
    /// or certificate validation failure.
    ///
    /// A [`ConnectionLost`](Event::ConnectionLost) event is emitted with
    /// details when the connection is closed by the peer or due to an
    /// error. When the local application closes the connection via
    /// [`close()`](Self::close), no `ConnectionLost` event is emitted;
    /// instead, pending operations fail with
    ...
    ///
    /// Closed connections become drained after a brief timeout to absorb any
    /// remaining in-flight packets from the peer. All drained connections
    /// have been closed.
    ...
    /// Current state of this connection's congestion controller, for debugging
    /// purposes
    pub fn congestion_state(&self, path_id: PathId) -> Option<&dyn Controller> {
        self.path(path_id).map(|d| d.congestion.as_ref())
    }

    /// Modify the number of remotely initiated streams that may be concurrently
    /// open
    ///
    ...
    pub fn set_max_concurrent_streams(&mut self, dir: Dir, count: VarInt) {
        ...
        // If the limit was reduced, then a flow control update previously deemed
        // insignificant may now be significant.
    ...
    ///
    /// When reducing the number of concurrent paths this will only affect
    /// delaying sending new MAX_PATH_ID frames until fewer than this number
    /// of paths are possible.  To actively reduce paths they must be closed
    /// using [`Connection::close_path`], which can also be used to close
    /// not-yet-opened paths.
    ///
    /// If multipath is not negotiated (see the [`TransportConfig`]) this can
    /// not enable multipath and will fail.
    ...
    /// If needed, issues a new MAX_PATH_ID frame and new CIDs for any newly
    /// allowed paths
    ...
    /// Current number of remotely initiated streams that may be concurrently
    /// open
    ///
    /// If the target for this limit is reduced using
    /// [`set_max_concurrent_streams`](Self::set_max_concurrent_streams), it
    /// will not change immediately, even if fewer streams are open. Instead, it
    /// will decrement by one for each time a remotely initiated stream of
    /// matching directionality is closed.
    ...
    ///
    /// Multipath is only enabled after the handshake is completed and if it was
    ...
    fn inner_on_ack_received(
        ...
        // Avoid DoS from unreasonably huge ack ranges by filtering out just the new
        // acks.
        ...
        for packet in newly_acked.elts() {
            if let Some(info) = self.spaces[space].for_path(path).take(packet) {
                ...
                // Notify MTU discovery that a packet was acked, because it might be an MTU
                // probe
        ...
        if new_largest_pn.is_some() && ack_eliciting_acked {
            ...
            let path_data = self.path_data_mut(path);
            // TODO(@divma): should be a method of path, should be contained in a single
            // place
        ...
        // If the peer did not complete the handshake address validation the ACK could
        // be spoofed, e.g. in the Initial space. Setting the pto_count back to
        // 0 removes the exponential backoff from the PTO timer and would result
        // in too many tail-loss probes being sent.
        if self.peer_completed_handshake_address_validation() {
            self.path_data_mut(path).pto_count = 0;
        }

        // Explicit congestion notification
        // TODO(@divma): this code is a good example of logic that should be contained
        // in a single place but it's split between the path data and the packet
        // number space data, we should find a way to make this work without two
        // lookups
        if self.path_data(path).sending_ecn {
            if let Some(ecn) = ack.ecn {
                // We only examine ECN counters from ACKs that we are certain we received in
                // transmit order, allowing us to compute an increase in ECN
                // counts to compare against the number of newly acked packets
                // that remains well-defined in the presence of arbitrary packet
                // reordering.
    ...
    // Not timing-aware, so it's safe to call this for inferred acks, such as arise
    // from high-latency handshakes
    ...
    ///
    /// The latter means we have not received an ACK for an ack-eliciting packet
    /// we sent within the PTO time-window. We need to schedule a tail-loss
    /// probe, an ack-eliciting packet, to try and elicit new
    /// acknowledgements. These new acknowledgements will indicate whether
    /// the previously sent packets were lost or not.
    ...
    /// - The [`PathTimer::LossDetection`] timer expired. So there is an
    ///   un-acknowledged packet that was followed by an acknowledged packet.
    ///   The loss timer for this un-acknowledged packet expired and we need to
    ///   detect that packet as lost.
    ///
    ...
    ///   - Having a packet number [`TransportConfig::packet_threshold`] lower
    ///     then the last acknowledged packet.
    ...
    fn detect_lost_packets(
        ...
        // InPersistentCongestion: Determine if all packets in the time period before
        // the newest lost packet, including the edges, are marked lost. PTO
        // computation must always include max ACK delay, i.e. operate as if in
        // Data space (see RFC9001 §7.6.1).
    ...
    fn discard_path(&mut self, path_id: PathId, now: Instant) {
        ...
        // Before removing the path, we fetch the final path stats via
        // `Self::path_stats`. This ensures snapshot values (like rtt) are
        // properly updated.
    ...
    /// Returns the earliest time packets should be declared lost for all spaces
    /// on a path.
    ///
    /// If a path has an acknowledged packet with any prior un-acknowledged
    /// packets, the earliest un-acknowledged packet can be declared lost
    /// after a timeout has elapsed. The time returned is when this packet
    /// should be declared lost.
    ...
    ///
    /// This needs to be fully deterministic because it is also used to
    /// determine the PTO that fired, not just to set the next timer. So if
    /// it fired in the past it needs to return the time from the past at
    /// which it fired.
    ///
    ...
    fn peer_completed_handshake_address_validation(&self) -> bool {
        ...
        // handshake or 1-RTT packets are acknowledged or we've seen
        // HANDSHAKE_DONE and discarded handshake keys.
    ...
    /// Resets the the [`PathTimer::LossDetection`] timer to the next instant it
    /// may be needed
    ///
    ...
    fn max_pto_for_space(&self, space: SpaceKind) -> Duration {
        self.paths
            ...
            .max()
            .unwrap_or_else(|| {
                // No paths remain (e.g. last path was abandoned and the NoAvailablePath grace
                // timer fired before any new path was opened). Fall back to a
                // PTO derived from the configured initial RTT, matching RFC
                // 9002 §6.2.2 initial values.
    ...
    /// Probe Timeout
    ///
    /// The PTO is logically the time in which you'd expect to receive an
    /// acknowledgement for a packet. So approximately RTT + max_ack_delay.
    ...
    fn on_packet_authenticated(
        ...
        // During the handshake we already have discarded packets that do not match the
        // path remote. So any off-path packet here is either a probing packet
        // or a migration. Handling probing packets here means that the path's
        // idle timeout will be reset and will delay detecting the path as idle.
        // However tail-loss probes would still not get acknowledged if the path
        // was broken so eventually the path would still become idle.
        let is_on_path = self
            .path_data(path_id)
            .network_path
    ...
    ///
    /// Without multipath there is only the connection-wide idle timeout. When
    /// multipath is enabled there is an additional per-path idle timeout.
    ...
    /// Resets both the [`ConnTimer::KeepAlive`] and
    /// [`PathTimer::PathKeepAlive`] timers
    ...
    ///
    /// Decrypting the first packet in the `Endpoint` allows stateless packet
    ...
    fn read_crypto(
        ...
        let expected = if !self.state.is_handshake() {
            ...
            // On the server, self.highest_space can be Data after receiving the client's
            // first flight, but we expect Handshake CRYPTO until the handshake
            // is complete.
            SpaceId::Handshake
        };
        // We can't decrypt Handshake packets when highest_space is Initial, CRYPTO
        // frames in 0-RTT packets are illegal, and we don't process 1-RTT
        // packets until the handshake is complete. Therefore, we will never see
    ...
    ///
    /// Processes the entire packet, starting with removing header protection,
    /// then handling a stateless reset if needed, and decrypting and
    /// processing the frames in the payload if not a stateless reset.
    fn handle_decode(
        &mut self,
        now: Instant,
        network_path: FourTuple,
    ...
    ///
    /// If the datagram was a stateless reset we may have failed to remove
    /// header protection and thus `packet` may be `None`.
    fn handle_packet(
        &mut self,
        now: Instant,
        network_path: FourTuple,
        ...
        //
        // If we received a valid packet and we are in the closed state we should
        // respond with a CONNECTION_CLOSE frame.
    ...
    fn process_decrypted_packet(
        ...
        if !self.paths.contains_key(&path_id) {
            // There is a chance this is a server side, first (for this path) packet, which
            // would be a protocol violation. It's more likely, however, that
            // this is a packet of a pruned path
    ...
    fn process_payload(
        ...
        let mut ack_eliciting = false;
        // if this packet triggers a path migration and includes a observed address
        // frame, it's stored here
                ...
                Frame::NewConnectionId(frame) => {
                    ...
                    match remote_cids.insert(frame) {
                        ...
                        Ok(Some((retired, reset_token))) => {
                            ...
                            /// Ensure `pending_retired` cannot grow without
                            /// bound. Limit is
                ...
                Frame::ObservedAddr(observed) => {
                    ...
                    if path.network_path.remote == network_path.remote {
                            ...
                            && space_open_status == OpenStatus::Informed
                        {
                            ...
                            // otherwise the event is reported when the path is
                            // deemed open
                        }
                    } else {
        ...
        // Issue stream ID credit due to ACKs of outgoing finish/resets and incoming
        // finish/resets on stopped streams. Incoming finishes/resets on open
        // streams are not handled here as they are only freed, and hence only
        // issue credit, once the application has been notified during a read on
        // the stream.
        let pending = &mut self.spaces[SpaceId::Data].pending;
        self.streams.queue_max_stream_id(pending);
        ...
        // validate the new path, we only need to validate the path if the peer migrates
        // per RFC9000 §9: https://www.rfc-editor.org/rfc/rfc9000.html#section-9-4
    ...
    ///
    /// *path_id* and *network_path* are those on which the PATH_RESPONSE was
    /// received.
    ...
    ///
    /// This creates a new [`PathData`] for the migrated path and stores the
    /// previous [`PathData`] in [`PathState::prev`].
    fn migrate(
        &mut self,
        path_id: PathId,
        now: Instant,
        ...
        self.path_generation_counter = self.path_generation_counter.wrapping_add(1);
        // TODO(@divma): conditions for path migration in multipath are very specific,
        // check them again to prevent path migrations that should actually
        // create a new path

        // Reset rtt/congestion state for new path unless it looks like a NAT rebinding.
        // Note that the congestion window will not grow until validation terminates.
        // Helps mitigate amplification attacks performed by spoofing source
        // addresses.
        ...
        // Only store this as previous path if it was validated. For all we know there
        // could already be a previous path stored which might have been
        // validated in the past, which is more valuable than one that's not yet
        // validated.
        //
        // With multipath it is possible that there are no remote CIDs for the path ID
        // yet. In this case we would never have sent on this path yet and would not be
        // able to send a PATH_CHALLENGE either, which is currently a
        // fire-and-forget affair anyway. So don't store such a path either.
            ...
            && let Some(cid) = self.remote_cids.get(&path_id).map(CidQueue::active)
        {
            ...
            // We haven't updated the remote CID yet, this captures the remote CID we were
            // using on the previous path.
    ...
    ///
    /// In the general (non-multipath) case, paths will perform a RFC9000
    /// migration and be pinged for a liveness check. This is the behaviour
    /// of a path assumed to be recoverable, even if this is not the case.
    ///
    /// Clients in a connection in which multipath has been negotiated should
    /// migrate paths to new [`PathId`]s. For paths that are known to be
    /// non-recoverable can be migrated to a new [`PathId`] by closing the
    /// current path, and opening a new one to the same remote. Treating
    /// paths as non recoverable when necessary accelerates connectivity
    /// re-establishment, or might allow it altogether.
    ///
    /// The optional `hint` allows callers to indicate when paths are
    /// non-recoverable and should be migrated to new a [`PathId`].
    // NOTE: only clients are allowed to migrate, but generally dealing with RFC9000
    // migrations is lacking <https://github.com/n0-computer/noq/issues/364>
    pub fn handle_network_change(&mut self, hint: Option<&dyn NetworkChangeHint>, now: Instant) {
        ...
        // Paths that can't recover so a new path should be open instead. If multipath
        // is not negotiated, this will be empty.
        ...
        for (path_id, path) in self.paths.iter_mut() {
            ...
            // Clear the local address for it to be obtained from the socket again. This
            // applies to all paths, regardless of being considered recoverable
            // or not
            path.data.network_path.local_ip = None;
            let remote = network_path.remote;

            // Without multipath, the connection tries to recover the single path, whereas
            // with multipath, even in a single-path scenario, we attempt to
            // migrate the path to a new PathId.
        ...
        // This are handled first, so that in case the treatment intended for these
        // fails, we can go the recoverable route instead.

        // Decide if we need to close first or open first in the multipath case.
        // - Opening first has a higher risk of getting limited by the negotiated
        //   MAX_PATH_ID.
        ...
        for (path_id, remote, status) in non_recoverable_paths.into_iter() {
            ...
            if !open_first && let Err(e) = self.open_path(network_path, status, now) {
                // Path has already been closed if we got here. Since the path was not
                // recoverable, this might be desirable in any case, because
                // other paths exist (!open_first) and this was is considered
                // non recoverable
                debug!(%e,"Failed to open new path for network change");
            }
        }
    ...
    ///
    /// The endpoint needs to know the reset tokens issued by the peer, so that
    /// if the peer sends a reset token it knows to route it to this
    /// connection. See RFC 9000 section 10.3. Stateless Reset.
    ///
    /// Reset tokens are different for each path, the endpoint identifies paths
    /// by peer socket address however, not by path ID.
    fn set_reset_token(&mut self, path_id: PathId, remote: SocketAddr, reset_token: ResetToken) {
        ...
        //    shortly after this was called.  And then the params don't have this
        // anymore.
        if path_id == PathId::ZERO {
    ...
    /// Issues an initial set of CIDs for paths that have not yet had any CIDs
    /// issued
    ///
    ...
    ///
    /// *path_exclusive_only* means to only build frames which can only be sent
    /// on this *path.  This is used in multipath for backup paths while
    /// there is still an active *path.
    fn populate_packet<'a, 'b>(
            ...
            && builder.frame_space_remaining() > frame::Crypto::SIZE_BOUND
        {
            ...
            // Since the offset is known, we can reserve the exact size required to encode
            // it. For length we reserve 2bytes which allows to encode up to
            // 2^14, which is more than what fits into normally sized QUIC
            // frames.
    ...
    fn populate_acks<'a, 'b>(
        ...
        // 0-RTT packets must never carry acks (which would have to be of handshake
        // packets)
    ...
    fn set_close_timer(&mut self, now: Instant) {
        // QUIC-MULTIPATH § 2.6 Connection Closure: draining for 3*PTO using the max PTO
        // of all paths.
    ...
    ///
    /// *remote_cid* and *local_cid* are the source and destination CIDs
    /// respectively of the *packet into which the transport parameters
    /// arrived.
    ...
    ///
    /// According to the spec, this will result in an error if the remote
    /// endpoint does not support the Acknowledgement Frequency extension
    ...
    /// Decodes a packet, returning its decrypted payload, so it can be
    /// inspected in tests
    #[cfg(test)]
    ...
    /// The number of bytes of packets containing retransmittable frames that
    ...
    /// Whether no timers but keepalive, idle, rtt, pushnewcid, and key discard
    /// are running
    #[cfg(test)]
    ...
    /// Instruct the peer to replace previously issued CIDs by sending a
    /// NEW_CONNECTION_ID frame with updated `retire_prior_to` field set to
    /// `v`
    #[cfg(test)]
    ...
    /// - Pending PATH_CHALLENGE frames on the active and previous path if just
    ///   migrated.
    /// - Pending PATH_RESPONSE frames.
    /// - Pending data to send in STREAM frames.
    /// - Pending DATAGRAM frames to send.
    ///
    /// See also [`PacketSpace::can_send`] which keeps track of all other frame
    /// types that may need to be sent.
    fn can_send_1rtt(&self, path_id: PathId, max_size: usize) -> SendableFrames {
        ...
        // Stream control frames are checked in PacketSpace::can_send, only check data
        // here.
    ...
    fn kill(&mut self, reason: ConnectionError) {
        ...
        // never sent a `Drained` event before (it's illegal to send more events after
        // drained).
        self.endpoint_events.push_back(EndpointEventInner::Drained);
    }

    /// Storage size required for the largest packet that can be transmitted on
    /// all currently available paths
    ///
    /// Buffers passed to [`Connection::poll_transmit`] should be at least this
    /// large.
    ///
    /// When multipath is enabled, this value is the minimum MTU across all
    /// available paths.
    ...
    ///
    /// Quantifies space consumed by the QUIC header and AEAD tag. All other
    /// bytes in a packet are frames. Changes if the length of the remote
    /// connection ID changes, which is expected to be rare. If `pn` is
    /// specified, may additionally change unpredictably due to variations in
    ...
    fn tag_len_1rtt(&self) -> usize {
        ...
        // If neither Data nor 0-RTT keys are available, make a reasonable tag length
        // guess. As of this writing, all QUIC cipher suites use 16-byte tags.
        // We could return `None` instead, but that would needlessly prevent
        // sending datagrams during 0-RTT.
        packet_crypto.map_or(16, |x| x.tag_len())
    }

    /// Mark the path as validated, and enqueue NEW_TOKEN frames to be sent as
    /// appropriate
    ...
    /// Handle new path status information: PATH_STATUS_AVAILABLE,
    /// PATH_STATUS_BACKUP
    ...
    /// Returns the maximum [`PathId`] to be used for sending in this
    /// connection.
    ///
    /// This is calculated as minimum between the local and remote's maximums
    /// when multipath is enabled, or `None` when disabled.
    ///
    /// For data that's received, we should use [`Self::local_max_path_id`]
    /// instead. The reasoning is that the remote might already have updated
    /// to its own newer [`Self::max_path_id`] after sending out a
    /// `MAX_PATH_ID` frame, but it got re-ordered.
    ...
    ///
    /// TODO(matheus23): This is related to noq endpoint state's `ipv6` bool. We
    /// should move that info here instead of trying to hack around not
    /// knowing it exactly.
    ...
    /// Add addresses the local endpoint considers are reachable for nat
    /// traversal.
    ...
    /// Removes an address the endpoing no longer considers reachable for nat
    /// traversal
    ///
    ...
    ///
    /// A nat traversal round involves advertising the client's local addresses
    /// in `REACH_OUT` frames, and initiating probing of the known remote
    /// addresses. When a new round is initiated, the previous one is
    /// cancelled.
    ///
    /// For all probes that succeed, if any, a new path will be opened on the
    /// successful 4-tuple.
    ///
    /// Returns the server addresses that are now being probed. If addresses
    /// fail due to spurious errors, these might succeed later and not be
    /// returned in this set.
    ...
    /// confirmed when you know the peer successfully received and successfully
    /// processed your TLS Finished message.
    ///
    /// Implementation-wise this is the point at which the handshake crypto keys
    /// are discarded. So we can use this to know if the handshake is
    /// confirmed.
...
///
/// Implementation based on ArrayRangeSet to share more code. The memory space
/// is proportional to the number of concurrently open paths allowed. So does
/// not grow unbounded.
...
///
/// A range is 2 u32's, so this is 16 * (4 + 4) = 128 bytes. A good size for
/// inline data, with plenty of ranges for common multipath use.
...
pub trait NetworkChangeHint: std::fmt::Debug + 'static {
    ...
    ///
    /// After network changes, paths may not be recoverable. In this case,
    /// waiting for the path to become idle may take longer than what is
    /// desirable. If [`Self::is_path_recoverable`] returns `false`, a
    /// multipath-enabled, client-side connection will establish a new path to
    /// the same remote, closing the current one, instead of migrating the path.
    ///
    /// Paths that are deemed recoverable will simply be sent a PING for a
    /// liveness check.
...
enum PollPathSpaceStatus {
    /// Nothing to send in the space, nothing was written into the
    /// [`TransmitBuf`].
    NothingToSend {
        /// If true there was data to send but congestion control did not allow
        /// so.
    ...
    WrotePacket {
        ...
        ///
        /// When packets in Initial, 0-RTT or Handshake packet do not fill the
        /// entire datagram they may decide to coalesce with the next
        /// packet from a higher encryption level on the same path. But
        /// the earlier packet may require specific size requirements
        /// for the datagram they are sent in.
        ///
        /// If a space did not complete the datagram, they use this to request
        /// the correct padding in the final packet of the datagram so
        /// that the final datagram will have the correct size.
        ///
        /// If a space did fill an entire datagram, it leaves this to the
        /// default of [`PadDatagram::No`].
        pad_datagram: PadDatagram,
    },
    /// Send the contents of the transmit immediately.
    ///
    /// Packets were written and the GSO batch must end now, regardless from
    /// whether higher spaces still have frames to write. This is used when
    /// the last datagram written would require too much padding to continue
    /// a GSO batch, which would waste space on the wire.
    Send {
...
///
/// Primarily used by [`Connection::poll_transmit_on_path`] and the functions
/// that help building packets for it: [`Connection::poll_transmit_path_space`]
/// and [`Connection::populate_packet`].
#[derive(Debug, Copy, Clone)]
struct PathSchedulingInfo {
    /// Whether the path is abandoned.
    ///
    /// Note that a path that is abandoned but still has CIDs can still send a
    /// packet. After sending that packet the CIDs issued by the remote have
    /// to be considered retired as well.
    is_abandoned: bool,
    /// Whether the path may send [`SpaceKind::Data`] frames.
    ///
    /// Some paths should only send frames from
    /// [`SendableFrames::space_specific`]. All other frames are essentially
    /// frames that can be sent on any [`SpaceKind::Data`] space. For
    /// those we want to respect packet scheduling rules however.
    ///
    /// Roughly speaking data frames are only sent on spaces that have CIDs, are
    /// not abandoned and have no *better* spaces. However see to comments
    /// where this is populated for the exact packet scheduling
    /// implementation.
    ///
    /// This essentially marks this paths as the best validated space ID. Except
    /// during the handshake in which case it does not need to be validated.
    /// Several paths could be equally good and all have this set to `true`,
    /// in that case packet scheduling can choose which path to use.
    /// Currently it chooses the lowest path that is not congestion blocked.
    ///
    ...
    ///
    /// This essentially marks this path as the best validated space ID with a
    /// fallback to unvalidated spaces if there are no validated spaces.
    /// Like for [`Self::may_send_data`] other paths could be equally good.
    may_send_close: bool,
...
enum ConnectionSide {
    Client {
        /// Sent in every outgoing Initial packet. Always empty after Initial
        /// keys are discarded
        token: Bytes,
...
/// Parameters to `Connection::new` specific to it being client-side or
/// server-side
...
/// Error when the multipath extension was not negotiated, but attempted to be
/// used.
...
pub enum Event {
    ...
    ///
    /// Emitted when the connection is closed due to an error, a timeout, or the
    /// peer closing it. This is **not** emitted when the local application
    /// closes the connection via
    /// [`Connection::close()`](crate::Connection::close). In that case, pending
    /// operations will fail with [`ConnectionError::LocallyClosed`].
    ConnectionLost {
...
///
/// If the path or connection idle time is less than [`MIN_IDLE_FOR_FAST_PTO`]
/// then we use this value to ensure we have plenty of retransmits before we
/// reach the idle time.
...
///
/// This is RTT time above which 1.5 * RTT > [`MAX_PTO_INTERVAL`], for these
/// links we want to extend the interval between tail-loss probes to not fill
/// the entire pipe with them.
const SLOW_RTT_THRESHOLD: Duration =
    Duration::from_millis((MAX_PTO_INTERVAL.as_millis() as u64 * 2) / 3);

/// Minimal remaining size to allow packet coalescing, excluding cryptographic
/// tag
///
/// This must be at least as large as the header for a well-formed empty packet
/// to be coalesced, plus some space for frames. We only care about handshake
/// headers because short header packets necessarily have smaller headers, and
/// initial packets are only ever the first packet in a datagram (because we
/// coalesce in ascending packet space order and the only reason to split a
/// packet is when packet space changes).
const MIN_PACKET_SPACE: usize = MAX_HANDSHAKE_OR_0RTT_HEADER_SIZE + 32;

/// Largest amount of space that could be occupied by a Handshake or 0-RTT
/// packet's header
///
...
/// Compute the negotiated idle timeout based on local and remote
/// max_idle_timeout transport parameters.
///
...
/// endpoints have opted out of idle timeout.
