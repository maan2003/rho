...
/// Implements Datagram Packetization Layer Path Maximum Transmission Unit
/// Discovery
///
...
impl MtuDiscovery {
    pub(crate) fn new(
        ...
        // We might be migrating an existing connection to a new path, in which case the
        // transport parameters have already been transmitted, and we already
        // know the value of `peer_max_udp_payload_size`
    ...
    /// MTU discovery will be disabled and the current MTU will be fixed to the
    /// provided value
    ...
    /// Notifies the [`MtuDiscovery`] that the peer's `max_udp_payload_size`
    /// transport parameter has been received
    pub(crate) fn on_peer_max_udp_payload_size_received(&mut self, peer_max_udp_payload_size: u16) {
        ...
        if let Some(state) = self.state.as_mut() {
            // It is possible for black hole detection to trigger before the connection has
            // been fully established, if the initial MTU is greater the minimum
            // MTU. We should never send probes before the connection has been
            // fully established and we have received the peer's transport
            // parameters though.
            debug_assert!(
    ...
    ///
    /// When done notifying of lost packets,
    /// [`MtuDiscovery::black_hole_detected`] must be called, to ensure the
    /// last loss burst is properly processed and to trigger black hole recovery
    /// logic if necessary.
    ...
    ///
    /// Calling this function will close the previous loss burst. If a black
    /// hole is detected, the current MTU will be reset to `min_mtu`.
...
impl EnabledMtuDiscovery {
    ...
    /// - The MTU discovery was completed but the
    ///   [`MtuDiscoveryConfig::interval`] expired, this re-starts a n MTU
    ///   search.
    ...
    ///
    /// Returns the new `current_mtu` if the packet number corresponds to the
    /// in-flight MTU probe
    ///
...
enum Phase {
    ...
    /// Searching has completed and will be triggered again at the provided
    /// instant
    Complete(Instant),
}
...
impl SearchState {
    /// Creates a new search state, with the specified lower bound (the upper
    /// bound is derived from the config and the peer's
...
///
/// Our MTU black hole detection scheme is a heuristic based on the order in
/// which packets were sent (the packet number order), their sizes, and which
/// are deemed lost.
///
/// First, contiguous groups of lost packets ("loss bursts") are aggregated,
/// because a group of packets all lost together were probably lost for the same
/// reason.
///
/// A loss burst is deemed "suspicious" if it contains no packets that are (a)
/// smaller than the minimum MTU or (b) smaller than a more recent acknowledged
/// packet, because such a burst could be fully explained by a reduction in MTU.
///
/// When the number of suspicious loss bursts exceeds [`BLACK_HOLE_THRESHOLD`],
/// we judge the evidence for an MTU black hole to be sufficient.
#[derive(Clone, Debug)]
struct BlackHoleDetector {
    ...
    /// Packet number of the biggest packet larger than `min_mtu` which we've
    /// received acknowledgment of more recently than any suspicious loss
    /// burst, if any
    largest_post_loss_packet: u64,
    /// The maximum of `min_mtu` and the size of `largest_post_loss_packet`, or
    /// exactly `min_mtu` if no larger packets have been received since the
    /// most recent loss burst.
    acked_mtu: u16,
...
impl BlackHoleDetector {
    ...
    fn on_probe_acked(&mut self, pn: u64, len: u16) {
        // MTU probes are always larger than the previous MTU, so no previous loss
        // bursts are suspicious. At most one MTU probe is in flight at a time,
        // so we don't need to worry about reordering between them.
        self.suspicious_loss_bursts.clear();
        self.acked_mtu = len;
        // This might go backwards, but that's okay: a successful ACK means we haven't
        // yet judged a more recently sent packet lost, and we just want to
        // track the largest packet that's been successfully delivered more
    ...
    fn on_non_probe_acked(&mut self, pn: u64, len: u16) {
        if len <= self.acked_mtu {
            // We've already seen a larger packet since the most recent suspicious loss
            // burst; nothing to do.
            return;
        }
        self.acked_mtu = len;
        ...
        self.largest_post_loss_packet = pn;
        // Loss bursts packets smaller than this are retroactively deemed
        // non-suspicious.
    ...
    fn on_non_probe_lost(&mut self, pn: u64, len: u16) {
        // A loss burst is a group of consecutive packets that are declared lost, so a
        // distance greater than 1 indicates a new burst
    ...
    /// Marks the end of the current loss burst, checking whether it was
    /// suspicious
    fn finish_loss_burst(&mut self) {
        let Some(burst) = self.current_loss_burst.take() else {
            return;
        };
        // If a loss burst contains a packet smaller than the minimum MTU or a more
        // recently transmitted packet, it is not suspicious.
        ...
        // A suspicious loss burst more recent than `largest_post_loss_packet`
        // invalidates it. This makes `acked_mtu` a conservative approximation.
        // Ideally we'd update `safe_mtu` and `largest_post_loss_packet` to
        // describe the largest acknowledged packet sent later than this burst,
        // but that would require tracking the size of an unpredictable number of
        // recently acknowledged packets, and erring on the side of false positives is
        // safe.
...
/// Maximum number of suspicious loss bursts that will not trigger black hole
/// detection
...
mod tests {
    ...
    fn mtu_discovery_after_complete_reactivates_when_interval_elapsed() {
        ...
        // Polling after the interval has passed does (taking the current mtu as lower
        // bound)
        assert_eq!(
    ...
    // Loss of packets larger than have been acknowledged should indicate a black
    // hole
    #[test]
    ...
    // Loss of packets followed in transmission order by confirmation of a larger
    // packet should not indicate a black hole
    #[test]
    ...
    // Loss of packets smaller than have been acknowledged previously should still
    ...
    // Bursts that are non-suspicious before a delivered packet become suspicious
    // past it
    #[test]
    ...
    // Acknowledgment of a packet marks prior loss bursts with the same packet size
    // as non-suspicious
    #[test]
