...
use super::PathId;
use crate::{Duration, FrameType};

/// Statistics about UDP datagrams transmitted or received on a connection.
///
/// All QUIC packets are carried by UDP datagrams. Hence, these statistics cover
/// all traffic on a connection.
#[derive(Default, Debug, Copy, Clone, PartialEq, Eq, derive_more::Add, derive_more::AddAssign)]
#[non_exhaustive]
pub struct UdpStats {
    /// The number of UDP datagrams observed.
    pub datagrams: u64,
    /// The total amount of bytes which have been transferred inside UDP
    /// datagrams.
    pub bytes: u64,
    /// The number of I/O operations executed.
    ///
    /// Can be less than `datagrams` when GSO, GRO, and/or batched system calls
    /// are in use.
    pub ios: u64,
}
...
pub struct PathStats {
    ...
    ///
    /// These are not included in [`Self::lost_packets`] and
    /// [`Self::lost_bytes`].
...
/// Connection statistics.
///
/// The fields here are a sum of the respective fields in the [`PathStats`] for
/// all the paths that exist as well as all paths that previously existed.
...
impl std::ops::Add<PathStats> for ConnectionStats {
    ...
    fn add(self, rhs: PathStats) -> Self::Output {
        // Be aware that Connection::stats() relies on the fact this function ignores
        // the rtt, cwnd and current_mtu fields.
        let PathStats {
            rtt: _,
            udp_tx,
            udp_rx,
...
///
/// This helper also helps with borrowing issues compared to having the
/// [`Self::for_path`] function as a helper directly on [`Connection`].
///
