...
use crate::{Duration, Instant, TransportError};
...
pub(super) struct CidState {
    ...
    ///
    /// Each entry indicates the expiration of all timestamps up to the sequence
    /// number in the entry.  This means one entry can expire multiple CIDs
    /// if the sequence number jumps by more than 1 between entries.
    retire_timestamp: VecDeque<CidTimestamp>,
    /// Number of local connection IDs that have been issued in
    ...
    /// Sequence number the peer has already retired all CIDs below at our
    /// request via `retire_prior_to`
    prev_retire_seq: u64,
    /// Sequence number to set in retire_prior_to field in NEW_CONNECTION_ID
    /// frame
    retire_seq: u64,
...
impl CidState {
    ...
    fn track_lifetime(&mut self, new_cid_seq: u64, now: Instant) {
        ...
        if let Some(last) = last_record {
            ...
            // Combine into a single batch if timestamp of current cid is same as the last
            // record
    ...
    ///
    /// Return whether a new CID needs to be pushed that notifies remote peer to
    /// respond `RETIRE_CONNECTION_ID`
    pub(crate) fn on_cid_timeout(&mut self) -> bool {
        ...
        if !unretired_ids_found {
            // All Cids are retired, `prev_retire_cid_seq` can be assigned to
            // `retire_cid_seq`
        ...
        //
        // If yes (return true), a new CID must be pushed with updated `retire_prior_to`
        // field to remote peer. If no (return false), it means CIDs that reach
        // the end of lifetime have been retired already. Do not push a new CID in order
        // to avoid violating above RFC.
    ...
    /// (PATH_)NEW_CONNECTION_ID frames in the next packet that is sent.  This
    /// records them and tracks their lifetime.
    ...
    ///
    /// Returns whether a new CID can be issued, or an error if the frame was
    /// illegal.
    pub(crate) fn on_cid_retirement(
        ...
        // Peer B first send a NEW_CONNECTION_ID with cid 3 and retire_prior_to set to
        // 1. Peer A processes this NEW_CONNECTION_ID frame; update remote cid
        // to 1,2,3 and meanwhile send a RETIRE_CONNECTION_ID to retire cid 0 to
        // peer B. If peer B doesn't check the cid limit here and send a new cid
        // again, peer A will then face CONNECTION_ID_LIMIT_ERROR
