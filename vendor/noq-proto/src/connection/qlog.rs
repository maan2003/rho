...
//!
//! This uses the [`qlog`] crate to emit qlog events. The n0-qlog crate, and
//! thus this implementation, is currently based on
//! [draft-ietf-quic-qlog-main-schema-13] an
...
use crate::connection::{EncryptionLevel, PathData, SentPacket, SpaceKind};
use crate::frame::EncodableFrame;
use crate::packet::Header;
use crate::transport_parameters::TransportParameters;
use crate::{Connection, ConnectionId, FourTuple, Frame, Instant, PathId};
#[cfg(feature = "qlog")]
use crate::{
...
/// A [`QlogStream`] that may be either dynamically disabled or compiled out
/// entirely
...
impl QlogSink {
    ...
    /// Emits a timer event.
    ///
    /// This function is not public: Instead, create a [`QlogSinkWithTime`] via
    /// [`Self::with_time`] and use its `emit_timer_` methods.
    #[cfg(feature = "qlog")]
    ...
    ///
    /// This may be used if you want to pass a [`QlogSink`] downwards together
    /// with the current `now` timestamp, to not have to pass the latter
    /// separately as an additional argument just for qlog support.
...
/// Info about a sent packet. Zero-sized struct if `qlog` feature is not
/// enabled.
#[derive(Default)]
...
impl QlogSentPacket {
    ...
    /// because constructing a [`QuicFrame`] may involve calculations which
    /// shouldn't be performed if the `qlog` feature is disabled.
    #[cfg(feature = "qlog")]
    fn frame_raw(&mut self, frame: QuicFrame) {
        self.inner.frames.get_or_insert_default().push(frame);
    }

    /// Finalizes the packet by setting the final packet length (after
    /// encryption).
...
/// Info about a received packet. Zero-sized struct if `qlog` feature is not
/// enabled.
