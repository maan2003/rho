...
use super::PathId;
use crate::Instant;
use crate::connection::qlog::{QlogSink, QlogSinkWithTime};
...
pub(crate) enum ConnTimer {
    ...
    ///
    /// Fires at initial PTO intervals to retransmit probes that got no
    /// PATH_RESPONSE.
...
pub(crate) enum PathTimer {
    ...
    /// 1. When opening a path we validate it according to RFC9000 §8.2 as
    ///    required by the multipath spec. This timer is armed to time-bound
    ///    that validation.
    /// 2. When validating an already opened multipath path for various reasons
    ///    in which we expect the path to either work or not work and want to
    ///    respond correspondingly in a timely manner.
...
/// For how many paths we keep the timers on the stack, before spilling onto the
/// heap.
