...
use super::PathId;
use super::paths::{PathResponses, PathRetransmits};
use crate::connection::StreamsState;
use crate::frame::{self, AddAddress, RemoveAddress};
...
pub(super) struct PacketSpace {
    ...
    ///
    /// Each [`PathId`] has it's own [`PacketNumberSpace`].  Only the
    /// [`SpaceId::Data`] can have multiple packet number spaces, the other
    /// spaces only have a number space for `PathId::ZERO`, which is
...
impl PacketSpace {
    ...
    ///
    /// When multipath is disabled use `PathId::ZERO`.
    // TODO(flub): Note that this only exists as `&mut self` because it creates a
    // new    [`PacketNumberSpace`] if one is not yet available for a path.
    // This forces a few    more `&mut` references to users than strictly
    // needed.  An alternative would be to    return an Option but that would
    // need to be handled for all callers.  This could be    worth exploring
    // once we have all the main multipath bits fitted.
    ...
    /// Queue data for a tail loss probe (or anti-amplification deadlock
    /// prevention) packet
    ///
    /// Probes are sent similarly to normal packets when an expected ACK has not
    /// arrived. We never deem a packet lost until we receive an ACK that
    /// should have included it, but if a trailing run of packets (or their
    /// ACKs) are lost, this might not happen in a timely fashion. We send
    /// probe packets to force an ACK, and exempt them from congestion control
    /// to prevent a deadlock when the congestion window is filled with lost
    /// tail packets.
    ///
    /// We prefer to send new data, to make the most efficient use of bandwidth.
    /// If there's no data waiting to be sent, then we retransmit in-flight
    /// data to reduce odds of loss. If there's no in-flight data either,
    /// we're probably a client guarding against a handshake
    ...
    pub(super) fn queue_tail_loss_probe(
        ...
        if request_immediate_ack {
            // The probe should be ACKed without delay (should only be used in the Data
            // space and when the peer supports the acknowledgement frequency
            // extension)
            ...
            .flat_map(|s| s.sent_packets.values_mut())
        {
            if !packet.retransmits.is_empty(streams) {
                // Remove retransmitted data from the old packet so we don't end up
                // retransmitting it *again* even if the copy we're sending now
                // gets acknowledged.
        ...
        // Nothing new to send and nothing to retransmit, so fall back on a ping. This
        // should only happen in rare cases during the handshake when the server
        // becomes blocked by anti-amplification.
    ...
    ///
    /// For the data space [`Connection::can_send_1rtt`] also needs to be
    /// consulted. Prefer to use [`Connection::space_can_send`] which
    /// handles this.
    ///
...
///
/// Unlike [`SpaceId`], this always has exactly three variants — it represents
/// the encryption level / space kind, not a specific packet number space
/// identity.
...
///
/// In RFC9000 there are 3 packet number spaces: Initial, Handshake and Data. In
/// QUIC Multipath there are multiple packet number spaces for Data, each
/// identified by a [`PathId`].
///
/// This contains the state for a packet number space which is not specific to
/// the 4-tuple this space is currently using. The 4-tuple specific state, like
/// congestion controller, pacing, ECN, MTU etc, is stored in [`PathData`].
///
/// Note that the `Initial`, `Handshake` and `Data(PathId(0))` space all share
/// the same [`PathData`].
///
...
pub(super) struct PacketNumberSpace {
    /// Whether the path has already been considered opened from an application
    /// perspective.
    ///
    /// This means, for paths other than the original [`PathId::ZERO`], a first
    /// path challenge has been responded to, regardless of the initial
    /// validation status of the path. This state is irreversible, since
    /// it's not affected by the path being closed.
    ///
    /// Sending a PATH_CHALLENGE and receiving a valid response before the
    /// application is informed of the path, is a way to ensure the path is
    /// usable before it is reported. This is not required by the spec, and
    /// in the future might be changed for simply requiring a first ack'd
    /// packet.
    pub(super) open_status: OpenStatus,

    /// Highest received packet number, if any
    pub(super) largest_received_packet_number: Option<u64>,
    /// The packet number of the next packet that will be sent, if any. In the
    /// Data space, the packet number stored here is sometimes skipped by
    /// [`PacketNumberFilter`] logic.
    ...
    /// Number of explicit congestion notification codepoints seen on incoming
    /// packets
    ...
    /// acked packet number. Stored per-space to simplify verification,
    /// which would otherwise have difficulty distinguishing between ECN
    /// bleaching and counts having been updated by a near-simultaneous
    ...
    /// An IMMEDIATE_ACK (draft-ietf-quic-ack-frequency) frame needs to be sent
    /// on this path.
    pub(super) pending_immediate_ack: bool,
    /// Responses to path challenges that need to be sent, on and off-path.
    ///
    /// Responses are only tied to the 4-tuple they were received on, not to a
    /// specific path generation. Whether the response is on or off-path
    /// only depends on the path's 4-tuple at sending time.
...
impl PacketNumberSpace {
    ...
    ///
    /// In the Data space, the connection's [`PacketNumberFilter`] must be used
    ...
    pub(super) fn detect_ecn(
        ...
        // If total_increase > newly_acked (which happens when ACKs are lost), this is
        // required by the draft so that long-term drift does not occur. If =,
        // then the only question is whether to count CE packets as CE or ECT0.
        // Recording them as CE is more consistent and keeps the congestion
        // check obvious.
    ...
    pub(super) fn sent(&mut self, number: u64, packet: SentPacket) -> Option<SentPacket> {
        // Retain state for at most this many non-ACK-eliciting packets sent after the
        // most recently sent ACK-eliciting packet. We're never guaranteed to
        // receive an ACK for those, and we can't judge them as lost without an
        // ACK, so to limit memory in applications which receive packets but
        // don't send ACK-eliciting data for long periods use we must eventually start
        // forgetting about them, although it might also be reasonable to just kill the
        // connection due to weird peer behavior.
        ...
        if packet.ack_eliciting {
            ...
            // non-ACK-eliciting packets must only contain PADDING, ACK, and
            // CONNECTION_CLOSE frames, which require no special handling on ACK
    ...
    /// Whether any congestion-controlled packets in this space are not yet
    /// acknowledged or lost
    pub(super) fn has_in_flight(&self) -> bool {
        // The number of non-congestion-controlled (i.e. size == 0) packets in flight at
        // a time should be small, since otherwise congestion control wouldn't
        // be effective. Therefore, this shouldn't need to visit many packets
        // before finishing one way or another.
...
pub(super) enum OpenStatus {
    ...
    #[default]
    Pending,
    /// The first packet has been sent using this [`PathId`]. However, it is not
    /// yet deemed good enough to be reported to the application.
    Sent,
...
pub(super) struct SentPacket {
    /// [`PathData::generation`](super::PathData::generation) of the path on
    ...
    /// overhead, but including QUIC framing overhead. Zero if this packet
    /// is not counted towards congestion control, i.e. not an "in flight"
    /// packet.
    pub(super) size: u16,
    ...
    ///
    /// These might be retransmitted over any available path in the same
    /// [`SpaceKind`].
...
///
/// Data in this queue must be retransmittable over any path in the same
/// [`SpaceKind`].
...
pub struct Retransmits {
    ...
    /// Whether we should inform the peer that their max [`PathId`] is blocking
    /// our attempt to open new paths.
    ///
    /// Stores the remote_max_path_id at the time this was generated.
    /// This frame is entirely informational, so when it's retransmitted, the
    /// remote_max_path_id is intentionally not updated to preserve the fact
    /// that this was the state of the client at some point.
    ...
    ///
    /// - If the path changes, NEW_TOKEN frames bound for the old path are not
    ///   retransmitted on the new path. That is why this field stores the
    ///   remote address: so that ones for old paths can be filtered out.
    /// - If a token is lost, a new randomly generated token is re-transmitted,
    ///   rather than the original. This is so that if both transmissions are
    ///   received, the client won't risk sending the same token twice. That is
    ///   why this field does _not_ store any actual token.
    ///
    /// It is true that a QUIC endpoint will only want to effectively have
    /// NEW_TOKEN frames enqueued for its current path at a given point in
    /// time. Based on that, we could conceivably change this from a vector
    /// to an `Option<(FourTuple, usize)>` or just a `usize` or something.
    /// However, due to the architecture of noq, it is considerably simpler to
    /// not do that; consider what such a change would mean for implementing
    /// `BitOrAssign` on Self.
    ...
    /// If a [`frame::PathStatusAvailable`] and [`frame::PathStatusBackup`] need
    /// to be sent for a path
    pub(super) path_status: BTreeSet<PathId>,
    /// Whether a PATH_CIDS_BLOCKED frame needs to be sent for a path.
    ///
    /// Stores the next_seq number for the blocked path. This number can be
    /// "outdated" at the time of sending when this is a retransmission.
    /// This is intentional, as this frame is purely informational, and this
...
impl ::std::ops::BitOrAssign for Retransmits {
    fn bitor_assign(&mut self, rhs: Self) {
        ...
        // We reduce in-stream head-of-line blocking by queueing retransmits before
        // other data for STREAM and CRYPTO frames.
...
///
/// This queue is always sorted, so that popping off the last item is always the
/// lowest sequence number of the lowest path ID. Which is the CID you want to
/// be issued next.
///
...
///
/// This keeps track of the highest round ID and automatically drops frames with
/// a lower round ID.
///
...
impl Dedup {
    ...
    fn smallest_missing_in_interval(&self, lower_bound: u64, upper_bound: u64) -> Option<u64> {
        ...
        // Since we already know the packets at the boundaries have been received, we
        // only need to check those in between them (this removes the necessity
        // of extra logic to deal with the highest packet, which is stored
        ...
        // The highest packet is not included in the bitfield, so we subtract 1 to
        // account for that
        let start_offset = (self.highest() - upper_bound).max(1) - 1;
        if start_offset >= BITFIELD_SIZE {
            // The start offset is outside of the window. All packets outside of the window
        ...
        // The range is clamped at the edge of the window, because any earlier packets
        // are considered to be received
        ...
        // Ensure the shift is within bounds (we already know start_offset <
        // BITFIELD_SIZE, because of the early return)
...
///
/// This applies to a particular space ID that was queried and all refers to
/// on-path data.
...
pub(super) struct SendableFrames {
    ...
    ///
    /// A space here in the sense of a QUIC Multipath packet number space:
    /// `Initial`, `Handshake` and all `Data(PathId)` spaces.
    ///
...
pub(super) struct PendingAcks {
    ...
    /// ACK frames only when there is other data to send, or when the
    /// `MaxAckDelay` timer expires.
    ...
    /// calculate the moment upon which `max_ack_delay` elapses
    earliest_ack_eliciting_since_last_ack_sent: Option<Instant>,
    /// Packet number ranges for which to still send acknowledgements.
    ///
    /// These are packet number ranges of ack-eliciting packets the peer has
    /// sent and which need to be acknowledged.  Packet numbers are only
    /// removed from here once the peer has acknowledged the ACKs for them.
...
impl PendingAcks {
    ...
    ///
    /// This is used in the top-level [`Connection::space_can_send`], so
    /// determines if a packet will be built. It is often possible to
    /// construct new ACK ranges to send before this returns `true`. This
    /// results in more ACK frames being sent, and processing those at the
    /// receiver costs CPU for very little improvements.
    ///
    /// [`Connection::space_can_send`]: super::Connection::space_can_send
    ...
    pub(super) fn acks_sent(&mut self) {
        // It is possible (though unlikely) that the ACKs we just sent do not cover all
        // the ACK-eliciting packets we have received (e.g. if there is not
        // enough room in the packet to fit all the ranges). To keep things
        // simple, however, we assume they do. If there are indeed some ACKs
        // that weren't covered, the packets might be ACKed later anyway, because
        // they are still contained in `self.ranges`. If we somehow fail to send the
        // ACKs at a later moment, the peer will assume the packets got lost and
        // will retransmit their frames in a new packet, which is suboptimal,
        // because we already received them. Our assumption here is
        // that simplicity results in code that is more performant, even in the presence
        // of occasional redundant retransmits.
    ...
    pub(super) fn maybe_ack_non_eliciting(&mut self) {
        // If we're going to send a packet anyway, and we've received a significant
        // number of non-ACK-eliciting packets, then include an ACK to help the
        // peer perform timely loss detection even if they're not sending any
        // ACK-eliciting packets themselves. Exact threshold chosen somewhat
        // arbitrarily.
...
///
/// A malicious peer could prompt the local application to begin a large data
/// transfer, and then send ACKs without first waiting for data to be received.
/// This could defeat congestion control, allowing the connection to consume
/// disproportionate resources. We therefore occasionally skip packet numbers,
/// and classify any ACK referencing a skipped packet number as a transport
/// error.
///
/// Skipped packet numbers occur only in the application data space (where
/// costly transfers might take place) and are distributed exponentially to
/// reflect the reduced likelihood and impact of bad behavior from a peer that
/// has been well-behaved for an extended period.
///
/// ACKs for packet numbers that have not yet been allocated are also a
/// transport error, but an attacker with knowledge of the congestion control
/// algorithm in use could time falsified ACKs to arrive after the packets they
/// reference are sent.
///
