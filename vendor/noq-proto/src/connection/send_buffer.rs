...
use crate::VarInt;
use crate::connection::streams::BytesOrSlice;
...
pub(super) struct SendBuffer {
    ...
    ///
    /// Only data up to the highest contiguous acknowledged offset can be
    /// discarded. We could discard acknowledged in this buffer, but it
    /// would require a more complex data structure. Instead, we track
    /// acknowledged ranges in `acks`.
    ///
    ...
    ///
    /// All ranges must be within `data.range().start..(data.range().end -
    /// unsent)`, since data that has never been sent can't be acknowledged.
    // TODO: Recover storage from these by compacting (#700)
    acks: ArrayRangeSet,
    /// Previously transmitted ranges deemed lost and marked for retransmission
    ///
    /// All ranges must be within `data.range().start..(data.range().end -
    /// unsent)`, since data that has never been sent can't be
    /// retransmitted.
    ///
    /// This should usually not overlap with `acks`, but this is not strictly
    /// enforced.
...
///
/// Any segment larger than this will be stored as-is, possibly triggering a
/// flush of the buffer.
const MAX_COMBINE: usize = 1452;

/// This is where the data of the send buffer lives. It supports appending at
/// the end, removing from the front, and retrieving data by range.
#[derive(Default, Debug)]
...
impl SendBufferData {
    ...
    ///
    /// Concatenates `segments` and `last_segment` so they can be handled
    /// uniformly
