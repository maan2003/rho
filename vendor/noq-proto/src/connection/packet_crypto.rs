...
use crate::{ConnectionId, Instant, RESET_TOKEN_SIZE, Side, TransportError};

/// Perform key updates this many packets before the AEAD confidentiality limit.
///
/// Chosen arbitrarily, intended to be large enough to prevent spurious
/// connection loss.
const KEY_UPDATE_MARGIN: u64 = 10_000;

pub(super) struct UnprotectHeaderResult {
    /// The packet with the now unprotected header (`None` in the case of
    /// stateless reset packets that fail to be decoded)
...
pub(super) struct PrevCrypto {
    /// The keys used for the previous key phase, temporarily retained to
    /// decrypt packets sent by the peer prior to its own key update.
    ...
    ///
    /// Incoming packets should be decrypted using these keys iff this is `None`
    /// or their packet number is lower. `None` indicates that we have not
    /// yet received a packet using newer keys, which implies that the
    /// update was locally initiated.
    pub(super) end_packet: Option<(u64, Instant)>,
    /// Whether the following key phase is from a remotely initiated update that
    /// we haven't acked
...
pub(super) struct CryptoState {
    ...
    /// Whether or not 0-RTT was enabled during the handshake. Does not imply
    /// acceptance.
    ...
    ///
    /// These are generated in advance to prevent timing attacks and/or DoS by
    /// third-party attackers spoofing key updates.
    ...
    /// How many packets are in the current key phase. Used only for `Data`
    /// space.
...
impl CryptoState {
    ...
    /// Removes header protection of a packet, or returns `None` if the packet
    /// was dropped.
    ...
    pub(super) fn decrypt_packet_body(
        ...
        // Packets that do not belong to known path ids are valid as long as they can be
        // decrypted. If we didn't have a path, that's for the purposes of this
        // function equivalent to not having received packets on that path yet.
        // So both of these cases are represented by `None`.
        ...
        let crypto = if packet.header.is_0rtt() {
            ...
            if crypto.end_packet.is_none_or(|(pn, _)| packet_number < pn) {
                ...
                // Otherwise, this must be a remotely-initiated key update, so fall through to
                // the final case.
                None
            }
        }) {
            ...
            // We're in the Data space with a key phase mismatch and either there is no
            // locally initiated key update or the locally initiated key update
            // was acknowledged by a lower-numbered packet. The key phase
            // mismatch must therefore represent a new remotely-initiated key
            // update.
            crypto_update = true;
        ...
        if crypto_update {
            ...
            // received any packets on this path yet. In that case, having the
            // first packet be a crypto update is fine.
    ...
    ///
    /// Use this only when sure the keys are allowed to be used.
    /// [`Self::encryption_keys`] should be preferred otherwise.
    ...
    /// Get local (sending) crypto keys and the actual encryption level for a
    /// given space.
    ///
    /// This method takes a [`SpaceKind`] and resolves the encryption level
    /// automatically: for the [`SpaceKind::Data`] space on the client side,
    /// it falls back to 0-RTT keys when 1-RTT keys are not yet available.
    /// Resolving the appropriate encryption keys makes this method
    ...
    pub(super) fn encryption_keys(
        ...
        // Clients use 0-RTT keys if 1-RTT keys are not available. Servers never encrypt
        // 0-RTT
    ...
    ///
    /// Generates the next set of keys, rotates current keys into previous, and
    /// installs the new keys. Updates `key_phase` and `key_phase_size`
    /// accordingly.
    ///
    ...
    ///
    /// For [`EncryptionLevel::OneRtt`], this counter resets to zero on every
    /// key update (see [`Self::update_keys`]).
    ...
    /// Number of packets that may still be sent before the AEAD confidentiality
    /// limit is reached at the given encryption level.
    ///
    /// For [`EncryptionLevel::OneRtt`] the effective limit is the minimum of
    /// the AEAD confidentiality limit and the current key-phase size. For
    /// all other levels the raw AEAD confidentiality limit is used.
    ///
