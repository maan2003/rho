...
impl Recv {
    ...
    pub(super) fn stop(&mut self) -> Result<(u64, ShouldTransmit), ClosedStream> {
        ...
        // it's a bit fiddly to distinguish that from the case where we've
        // received a FIN but are missing some data that the peer might still be
        // trying to retransmit, in which case a STOP_SENDING is still useful.
        Ok((read_credits, ShouldTransmit(self.is_receiving())))
    }

    /// Returns the window that should be advertised in a `MAX_STREAM_DATA`
    /// frame
    ///
    ...
    /// `false` the new window should only be transmitted if a previous
    /// transmission had failed.
    ...
    pub(super) fn can_send_flow_control(&self) -> bool {
        // Stream-level flow control is redundant if the sender has already sent the
        // whole stream, and moot if we no longer want data on this stream.
    ...
    pub(super) fn reset(
        ...
        // reads don't issue flow control credit redundant to that already
        // issued. We could instead special-case reset streams during read, but
        // it's unclear if there's any benefit to retaining data for
        // reset streams.
        self.assembler.clear();
        Ok(true)
    }
    ...
    /// Compute the amount of flow control credit consumed, or return an error
    /// if more was consumed than issued
...
///
/// It is recommended that you call [`Self::finalize()`] because it returns a
/// flag telling you whether reading from the stream has resulted in the need to
/// transmit a packet.
///
...
impl<'a> Chunks<'a> {
    ...
    /// a packet needs to be sent to the peer informing them that the stream is
    /// unblocked. This means that you should call
    /// [`Connection::poll_transmit()`][crate::Connection::poll_transmit]
    /// and send the returned packet as soon as is reasonable, to unblock the
    /// remote peer.
    ...
    fn finalize_inner(&mut self) -> ShouldTransmit {
        ...
        // We issue additional stream ID credit after the application is notified that a
        // previously open stream has finished or been reset and we've therefore
        // disposed of its state, as recorded by `stream_freed` calls in `next`.
        let mut should_transmit = self.streams.queue_max_stream_id(self.pending);

        // If the stream hasn't finished, we may need to issue stream-level flow control
        // credit
        ...
        // Issue connection-level flow control credit for any data we read regardless of
        // state
