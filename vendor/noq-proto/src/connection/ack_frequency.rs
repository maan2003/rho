...
use crate::{AckFrequencyConfig, Duration, TIMER_GRANULARITY, TransportError, VarInt};

/// State associated to ACK frequency
pub(super) struct AckFrequencyState {
    //
    // Sending ACK_FREQUENCY frames
    /// The path ID, packet number and value of the in-flight ACK_FREQUENCY
    /// frame
...
impl AckFrequencyState {
    ...
    /// Returns the `max_ack_delay` that should be requested of the peer when
    /// sending an ACK_FREQUENCY frame
    pub(super) fn candidate_max_ack_delay(
        ...
        // Use the peer's max_ack_delay if no custom max_ack_delay was provided in the
        // config
        let min_ack_delay =
    ...
    ///
    /// This `max_ack_delay` is defined as the maximum of the peer's current
    /// `max_ack_delay` and all in-flight `max_ack_delay`s (i.e. proposed
    /// values that haven't been acknowledged yet, but might be already in
    /// use by the peer).
    ...
    /// Notifies the [`AckFrequencyState`] that a packet containing an
    /// ACK_FREQUENCY frame was sent
    ...
    /// Notifies the [`AckFrequencyState`] that an ACK_FREQUENCY frame was
    /// received
    ///
    /// Updates the endpoint's params according to the payload of the
    /// ACK_FREQUENCY frame, or returns an error in case the requested
    /// `max_ack_delay` is invalid.
    ///
    /// Returns `true` if the frame was processed and `false` if it was ignored
    /// because of being stale.
...
/// Maximum proportion difference between the most recently requested max ACK
/// delay and the currently desired one before a new request is sent, when the
/// peer supports the ACK frequency extension and an explicit max ACK delay is
/// not configured.
const MAX_RTT_ERROR: f32 = 0.2;

/// Minimum value to request the peer set max ACK delay to when the peer
/// supports the ACK frequency extension and an explicit max ACK delay is not
/// configured.
