...
use super::{Connection, PathId, SentFrames, TransmitBuf};
use crate::coding::Encodable;
use crate::connection::qlog::QlogSentPacket;
use crate::connection::spaces::Retransmits;
use crate::connection::{ConnectionSide, EncryptionLevel};
use crate::frame::EncodableFrame;
use crate::packet::{
    FIXED_BIT, Header, InitialHeader, LongType, PacketNumber, PartialEncode, SpaceId,
};
use crate::{ConnectionId, FrameStats, Instant, MIN_INITIAL_SIZE, TransportError};

/// QUIC packet builder
///
/// This allows building QUIC packets: it takes care of writing the header,
/// allows writing frames and on [`PacketBuilder::finish`] (or
/// [`PacketBuilder::finish_and_track`]) it encrypts the packet so it is ready
/// to be sent on the wire.
///
/// The builder manages the write buffer into which the packet is written, and
/// directly implements [`BufMut`] to write frames into the packet.
pub(super) struct PacketBuilder<'a, 'b> {
    ...
    /// Smallest absolute position in the associated buffer that must be
    /// occupied by this packet's frames
...
impl<'a, 'b> PacketBuilder<'a, 'b> {
    /// Write a new packet header to `buffer` and determine the packet's
    /// properties
    ///
    /// Marks the connection drained and returns `None` if the confidentiality
        ...
        'b: 'a,
    {
        ...
        // combined lengths of the encoded packet number and protected payload
        // must be at least 4 bytes longer than the sample required for header
        // protection. Further, each packet should be at least tag_len + 6 bytes
        // larger than the destination CID on incoming packets so that the
        // peer may send stateless resets that are indistinguishable from regular
        // traffic.
    ...
    /// Append the minimum amount of padding to the packet such that, after
    /// encryption, the enclosing datagram will occupy at least `min_size`
    /// bytes
    pub(super) fn pad_to(&mut self, min_size: u16) {
        // The datagram might already have a larger minimum size than the caller is
        // requesting, if e.g. we're coalescing packets and have populated more
        // than `min_size` bytes with packets already.
    ...
    ///
    /// It will also:
    /// - Track the frame so that it's registered with the path once
    ///   [`Self::finish_and_track`] is called.
    ...
    ///
    /// The [`BufMut::remaining_mut`] call on the returned buffer indicates the
    /// amount of space available to write QUIC frames into.
    ...
    /// Encrypt packet, returning the length of the packet and whether padding
    /// was added
    pub(super) fn finish(
        mut self,
        conn: &mut Connection,
    ...
    /// The number of additional bytes the current packet would take up if it
    /// was finished now
    ///
    /// This will include any padding which is required to make the size large
    /// enough to be encrypted correctly.
    pub(super) fn predict_packet_end(&self) -> usize {
        self.buf.len().max(self.min_size) + self.tag_len - self.buf.len()
    }

    /// Returns the remaining space in the packet that can be taken up by QUIC
    /// frames
    ///
    /// This leaves space in the datagram for the cryptographic tag that needs
    /// to be written when the packet is finished.
