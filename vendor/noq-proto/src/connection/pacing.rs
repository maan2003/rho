...
impl Pacer {
    ...
    /// Obtains the `max_bytes_per_second` used when this [`Pacer`] was
    /// constructed.
    ...
    /// returns `Some(d)`, where `d` is the duration after which this
    /// function should be called again.
    ///
    /// The 5/4 ratio used here comes from the suggestion that N = 1.25 in the
    /// draft IETF RFC for QUIC.
    ///
    /// `capacity` (bytes) and `pacing_rate` (bytes/s) are optional overrides
    /// supplied by the congestion controller (e.g. BBRv3's `send_quantum` /
    /// `pacing_rate`). They take precedence over the window-derived
    /// defaults, but are still subject to the static `max_bytes_per_second`
    /// cap configured at construction time.
    pub(super) fn delay(
        &mut self,
        smoothed_rtt: Duration,
        bytes_to_send: u64,
        ...
        // In the unlikely event that we're getting polled faster than tokens are
        // generated, ensure that `elapsed_rtts` can grow until we make
        // progress.
        if new_tokens > 0 {
            self.prev = now;
        }
...
/// than 25%, we will lose that part of the congestion window since no
/// additional tokens for the extra-elapsed time can be stored.
///
...
fn optimal_capacity(smoothed_rtt: Duration, window: u64, mtu: u16) -> u64 {
    ...
    // Batch the greater of `TARGET_BURST_INTERVAL` or `MIN_BURST_SIZE` worth of
    // traffic at a time. To avoid inducing excessive latency, limit that result
    // to at most `MAX_BURST_INTERVAL` worth of traffic.
    Ord::min(
        max_capacity,
...
fn rate_limited_window(
    ...
    // the pacer refills tokens at x1.25 speed, so we shrink the window to cancel
    // out the speedup (otherwise the actual sending rate could be higher than
    // `max_bytes_per_second`)
...
/// Minimum number of datagrams to batch together, so long as we won't have to
/// wait for more than [`MAX_BURST_INTERVAL`]
const MIN_BURST_SIZE: u64 = 10;

/// Creating 256 packets took 1ms in a benchmark, so larger bursts don't make
/// sense.
