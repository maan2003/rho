...
use crate::connection::PacketBuilder;
use crate::frame::{Datagram, FrameStruct};
use crate::{FrameStats, TransportError};
...
impl Datagrams<'_> {
    ...
    ///
    /// If `drop` is true, previously queued datagrams which are still unsent
    /// may be discarded to make space for this datagram, in order of oldest
    /// to newest. If `drop` is false, and there isn't enough space due to
    /// previously queued datagrams, this function will return
    /// `SendDatagramError::Blocked`. `Event::DatagramsUnblocked` will be
    /// emitted once datagrams have been sent.
    ///
    ...
    ///
    /// When multipath is enabled, this is calculated using the smallest MTU
    /// across all available paths.
    pub fn max_size(&self) -> Option<usize> {
        // We use the conservative overhead bound for any packet number, reducing the
        // budget by at most 3 bytes, so that PN size fluctuations don't cause
        // users sending maximum-size datagrams to suffer avoidable packet loss.
    ...
    ///
    /// When greater than zero, [`send`](Self::send)ing a datagram of at most
    /// this size is guaranteed not to cause older datagrams to be dropped.
...
pub(super) struct DatagramState {
    /// Number of bytes of datagrams that have been received by the local
    /// transport but not delivered to the application
...
impl DatagramState {
    ...
    /// Discard outgoing datagrams with a payload larger than `max_payload`
    /// bytes
    ///
    /// Returns whether any datagrams were dropped.
    ///
    /// Used to ensure that reductions in MTU don't get us stuck in a state
    /// where we have a datagram queued but can't send it.
    ...
    /// Attempt to write a datagram frame into `buf`, consuming it from
    /// `self.outgoing`
    ///
    /// Returns whether a frame was written. At most `max_size` bytes will be
