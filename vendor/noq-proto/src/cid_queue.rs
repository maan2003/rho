...
///
/// This represents a circular buffer that can contain gaps due to packet loss
/// or reordering. The buffer has three regions:
/// - Exactly one active CID at `self.buffer[self.cursor]`.
/// - Zero to `Self::LEN - 1` reserved CIDs from `self.cursor` up to
///   `self.cursor_reserved`.
/// - More "available"/"ready" CIDs after `self.cursor_reserved`.
///
/// The range of reserved CIDs is grown by calling `CidQueue::next_reserved`,
/// which takes one of the available ones and returns the CID that was reserved.
///
...
pub(crate) struct CidQueue {
    ...
    ///
    /// The sequence number of the active CID; must be the smallest among CIDs
    /// in `buffer`.
    offset: u64,
    /// Circular index for the last reserved CID, i.e. a CID that is not the
    /// active CID, but was used for probing packets on a different remote
    /// address.
    ///
    /// When [`Self::cursor_reserved`] and [`Self::cursor`] are equal, no CID is
    /// considered reserved.
    ///
    /// The reserved CIDs section of the buffer, if non-empty will always be
    /// ahead of the active CID.
...
impl CidQueue {
    ...
    /// token of the new active CID iff any CIDs were retired.
    pub(crate) fn insert(
        &mut self,
        cid: NewConnectionId,
        ...
        // The active CID was retired. Find the first known CID with sequence number of
        // at least retire_prior_to, and inform the caller that all prior CIDs
        // have been retired, and of the new CID's reset token.
        ...
        // NEW_CONNECTION_ID frame, since having previously received them would violate
        // the connection ID limit we specified based on Self::LEN. If we do
        // receive a such a frame in the future, e.g. due to reordering, we'll
        // retire it then. This ensures we can't be made to buffer an
        // arbitrarily large number of RETIRE_CONNECTION_ID frames.
        Ok(Some((
    ...
    /// 1) the corresponding ResetToken and 2) a non-empty range preceding it to
    ///    retire
    ...
    /// Iterate CIDs in CidQueue that are not `None`, from
    /// [`Self::cursor_reserved`].
    ///
    /// The iterator will always have at least one item, as it will include the
    /// active CID when no CID has been reserved, or the last reserved CID
    /// otherwise.
    ///
    /// Along with the CID, it returns the offset counted from
    /// [`Self::cursor_reserved`] where the CID is stored.
    ...
    /// The length of the internal buffer's section of CIDs that are marked as
    /// reserved.
...
mod tests {
    ...
    #[test]
    fn retire_sparse() {
        // Retiring CID 0 when CID 1 is not known should retire CID 1 as we move to CID
        // 2
        let mut q = CidQueue::new(initial_cid());
