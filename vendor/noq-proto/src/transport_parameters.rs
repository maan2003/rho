...
// types, along with their codes and default values. Using this helps us avoid
// error-prone duplication of the contained information across decoding,
// encoding, and the `Default` impl. Whenever we want to do something with
// transport parameters, we'll handle the bulk of cases by writing a macro that
...
/// Such identifiers are reserved to exercise the requirement that unknown
/// transport parameters be ignored. The reserved transport parameter has no
/// semantics and can carry arbitrary values. It may be included in transport
/// parameters sent to the peer, and should be ignored when received.
///
...
pub(crate) struct ReservedTransportParameter {
    ...
    /// The number of bytes to include in the wire format from the `payload`
    /// buffer
    payload_len: usize,
}
...
impl ReservedTransportParameter {
    ...
    /// Generates a random reserved identifier of the form `31 * N + 27`, as
    /// required by RFC 9000. Reserved transport parameter identifiers are
    /// used to test compliance with the requirement that unknown transport
    /// parameters must be ignored by peers. See: <https://www.rfc-editor.org/rfc/rfc9000.html#section-18.1> and <https://www.rfc-editor.org/rfc/rfc9000.html#section-22.3>
    ...
    /// the limit used by other implementations of QUIC, e.g., quic-go and
    /// quiche.
...
mod test {
    ...
    fn read_semantic_validation() {
        ...
        let illegal_params_builders: Vec<Box<dyn FnMut(&mut TransportParameters)>> = vec![
            ...
            Box::new(|t| {
                // Preferred address can only be sent by senders (and we are reading the
                // transport params as a client)
