...
use crate::{ConnectError, ConnectionId, PathId, Side, TransportError, TransportErrorCode};
...
impl crypto::Session for TlsSession {
    ...
    /// For the rustls `TlsSession`, the `Any` type is
    ...
    fn read_handshake(&mut self, buf: &[u8]) -> Result<bool, TransportError> {
        ...
        if !self.got_handshake_data {
            // Hack around the lack of an explicit signal from rustls to reflect ClientHello
            // being ready on incoming connections, or ALPN negotiation
            // completing on outgoing connections.
...
pub struct HandshakeData {
    ...
    /// The key exchange group negotiated with the peer. Useful for logging and
    /// metrics.
    ///
...
///
/// noq implicitly constructs a `QuicClientConfig` with reasonable defaults
/// within [`ClientConfig::with_root_certificates()`][root_certs] and
/// [`ClientConfig::try_with_platform_verifier()`][platform]. Alternatively,
/// `QuicClientConfig`'s [`TryFrom`] implementation can be used to wrap around a
/// custom [`rustls::ClientConfig`], in which case care should be taken around
/// certain points:
///
/// - If `enable_early_data` is not set to true, then sending 0-RTT data will
///   not be possible on outgoing connections.
/// - The [`rustls::ClientConfig`] must have TLS 1.3 support enabled for
///   conversion to succeed.
///
/// The object in the `resumption` field of the inner [`rustls::ClientConfig`]
/// determines whether calling `into_0rtt` on outgoing connections returns `Ok`
/// or `Err`. It typically allows `into_0rtt` to proceed if it recognizes the
/// server name, and defaults to an in-memory cache of 256 server names.
///
...
impl QuicClientConfig {
    ...
    ///
    /// QUIC requires that TLS 1.3 be enabled. Advanced users can use any
    /// [`rustls::ClientConfig`] that satisfies this requirement.
    #[cfg(any(feature = "aws-lc-rs", feature = "ring"))]
    ...
    ///
    /// This is useful if you want to avoid the initial cipher suite for traffic
    /// encryption.
    pub fn with_initial(
...
/// [`CipherSuite::TLS13_AES_128_GCM_SHA256`]. When the cipher suite is derived
/// from a config's [`CryptoProvider`][provider], that provider must reference a
/// cipher suite with the same ID.
///
...
///
/// noq implicitly constructs a `QuicServerConfig` with reasonable defaults
/// within [`ServerConfig::with_single_cert()`][single]. Alternatively,
/// `QuicServerConfig`'s [`TryFrom`] implementation or `with_initial` method can
/// be used to wrap around a custom [`rustls::ServerConfig`], in which case care
/// should be taken around certain points:
///
/// - If `max_early_data_size` is not set to `u32::MAX`, the server will not be
///   able to accept incoming 0-RTT data. QUIC prohibits `max_early_data_size`
///   values other than 0 or `u32::MAX`.
/// - The `rustls::ServerConfig` must have TLS 1.3 support enabled for
///   conversion to succeed.
///
...
impl QuicServerConfig {
    ...
    ///
    /// QUIC requires that TLS 1.3 be enabled, and that the maximum early data
    /// size is either 0 or `u32::MAX`. Advanced users can use any
    /// [`rustls::ServerConfig`] that satisfies these requirements.
    #[cfg(any(feature = "aws-lc-rs", feature = "ring"))]
    pub(crate) fn inner(
        cert_chain: Vec<CertificateDer<'static>>,
        key: PrivateKeyDer<'static>,
...
impl crypto::ServerConfig for QuicServerConfig {
    fn start_session(
        ...
        // Safe: `start_session()` is never called if `initial_keys()` rejected
        // `version`
