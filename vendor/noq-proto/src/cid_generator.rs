...
pub trait ConnectionIdGenerator: Send + Sync {
    ...
    /// Quickly determine whether `cid` could have been generated by this
    /// generator
    ///
    /// False positives are permitted, but increase the cost of handling invalid
    /// packets.
...
///
/// Random CIDs can be smaller than those produced by
/// [`HashedConnectionIdGenerator`], but cannot be
...
///
/// This generator uses a non-cryptographic hash and can therefore still be
/// spoofed, but nonetheless helps prevents noq from responding to non-QUIC
/// packets at very low cost.
...
impl HashedConnectionIdGenerator {
    ...
    ///
    /// Allows [`validate`](ConnectionIdGenerator::validate) to recognize a
    /// consistent set of connection IDs across restarts
