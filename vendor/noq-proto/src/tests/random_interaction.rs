...
pub(super) enum TestOp {
    ...
    /// Move the first inbound packet to the back of the queue, simulating
    /// reordering.
    ReorderInbound { side: Side },
    /// Force a TLS key update on the connection belonging to `side`.
    ForceKeyUpdate { side: Side },
    /// Simulate a passive address migration by changing the address at
    /// `addr_idx` in the routing table for `side`.
    PassiveMigration {
        side: Side,
    ...
    /// Open a new network path from `side` to the remote's address at
    /// `addr_idx`.
    OpenPath {
        side: Side,
    ...
    /// Update the status of an existing path on the connection belonging to
    /// `side`.
    PathSetStatus {
        side: Side,
        /// Index into the connection's list of path IDs.
...
impl StreamOp {
    fn run(self, pair: &mut Pair, state: &mut State) -> Option<()> {
        let conn = state.conn(pair)?;
        // We generally ignore application-level errors. It's legal to call these APIs,
        // so we do. We don't expect them to work all the time.
        match self {
