...
use crate::{
    ClientConfig, ClosePathError, ConnectionId, ConnectionIdGenerator, Dir, Endpoint,
    EndpointConfig, Event, FourTuple, LOCAL_CID_COUNT, NetworkChangeHint, PathAbandonReason,
    PathEvent, PathId, PathStatus, RandomConnectionIdGenerator, ServerConfig, StreamEvent,
    TransportConfig, TransportErrorCode, n0_nat_traversal,
};
...
fn cid_issued_multipath() {
    ...
    // as they are all sent after the handshake is completed.
    ...
    // Multipath is only enabled *after* the handshake completes.  The first
    // server-CID is not issued but assigned by the client and changed by the
    // server.
    assert_eq!(
...
fn multipath_cid_rotation() {
    ...
    // The final round only pushes one set of CIDs with expires_before, the round is
    // not run to completion to wait for the expiry messages from the client.
...
fn open_path_validation_fails_client_side() -> TestResult {
    ...
    // Make sure the client's path open makes it through to the server and is
    // processed.
    pair.drive_client();
    pair.drive_server();
    ...
    // - path validation can't succeed because path responses don't make it through
    //   and
    // - the server needs to decide to close the path on its own, because path
    //   abandons don't make it through.
...
/// Regression: We never emit [`PathEvent::Established`] after
/// [`PathEvent::Abandoned`].
///
...
/// emit a [`PathEvent::Establish`] *after* [`PathEvent::Abandoned`], which
/// makes no sense. This was fixed, and this test ensures that this doesn't
/// happen again.
#[test]
...
/// Closing a path locally may be rejected if this leaves the endpoint without
/// validated paths. For paths closed by the remote, however, a `PATH_ABANDON`
/// frame must be accepted. In particular, it should not kill the connection.
///
...
/// With multipath and hint=None, the client defaults to non-recoverable: the
/// old path is closed with PATH_UNSTABLE_OR_POOR and a new path is opened. Data
/// still flows on the new path.
#[test]
...
/// With two paths open and a selective hint, only the non-recoverable path gets
/// replaced. The recoverable path is kept and pinged for liveness.
#[test]
...
fn network_change_server_two_paths_selective_hint() -> TestResult {
    ...
    // Signal network change without actually changing the server's local address.
    // This means the client will not see an actual network change and keep
    // accepting the packets from the server. If the server's address would
    // change it would discard the server's packets since the server may not
    // migrate.
    pair.handle_network_change(Server, Some(&SelectiveHint(second_path)));

    pair.drive();

    // The non-recoverable path is abandoned on the server. No replacement opens
    // because servers cannot call open_path.
    assert_matches!(
        pair.poll(Server),
        Some(Event::Path(PathEvent::Abandoned {
            id,
...
/// Server-side network change with no hint defaults to recoverable. Both paths
/// stay open.
#[test]
...
/// Checks that the deadline given before a path fails to be considered open
/// start only when the first packet is sent.
///
...
fn path_open_deadline_is_set_on_send() -> TestResult {
    ...
    // Fast-forward time well past 3×PTO without letting any transmit happen on the
    // new path.
...
fn server_abandon_last_verified_path() -> TestResult {
    // The client abandons the last verified path the server has. The server is
    // expected to send PATH_ABANDON on the abandoned path itself in this case.

    let _guard = subscribe();
    let mut pair = ConnPair::builder().enable_multipath().connect();

    // Passively migrate the client and immediately open a second path. This way the
    // client will assume the 2nd path is validated but to the server it will be
    // un-validated. Otherwise the client would not allow closing path 0 since there
    // would be no validated path left over.
    pair.routes.as_basic_mut().passive_migration(Client);
    let route = FourTuple {
        remote: pair.routes.public_server_addr(),
        local_ip: None,
    ...
    // We need to move past the Abandoned and Open events, we really only care about
    // getting the stats from the abandoned path.
...
/// Remote abandons the last path, no new path opened: connection closes after
/// grace period.
#[test]
...
/// Remote abandons the last path, client opens a new path within grace period:
/// connection survives.
#[test]
...
fn abandon_path_data_continues() -> TestResult {
    ...
    // Client abandons path 0 (picoquic: `picoquic_abandon_path(cnx_client, 0, 0,
    // "test", time)`)
    ...
    // Data should still flow on the remaining path (picoquic sends
    // test_scenario_multipath)
    let s = pair.streams(Client).open(Dir::Uni).unwrap();
...
fn abandon_cycle() -> TestResult {
    ...
    // Cycle: open a second path, abandon path 0, verify cleanup, repeat with new
    // paths. Each cycle uses a fresh pair of addresses.
...
fn test_simple_nat_traversal_challenge_with_response() -> TestResult {
    ...
    // Client sends probe (blocked) + REACH_OUT, server send probe. Both firewalls
    // open.
    pair.step();
    ...
    // Without the challenge-with-response only a PATH_RESPONSE would have been
    // sent.
    assert_eq!(
...
///
/// Here "very easy NAT" is an EIM+EIF NAT. The port is opened by the QAD probe.
/// "hard NAT" is an EDM NAT.
#[test]
fn test_hard_nat_client_opens_path() -> TestResult {
    let _guard = subscribe();
    let mut routing = SimpleFirewallRouting::new();
    // By configuring the server side to be open it emulates an EIM+EIF NAT: the QAD
    // probe already opened the firewall.
    ...
    info!("adding addrs");
    pair.add_nat_traversal_address(Server, SimpleFirewallRouting::SERVER_FW_ADDR)?;
    // By adding a dummy address the client can start NAT traversal but does not
    // advertise its real public interface, thus emulating a hard NAT. Choose
    // the last addr in the client subnet.
...
fn test_hard_nat_server_opens_path() -> TestResult {
    let _guard = subscribe();
    let mut routing = SimpleFirewallRouting::new();
    // By configuring the client side to be open it emulates an EIM+EIF NAT: the QAD
    // probe already opened the firewall.
    ...
    // By adding a dummy address the client can start NAT traversal but does not
    // advertise its real public interface, thus emulating a hard NAT. Choose
    // the last addr in the server subnet.
...
/// If the client is not allowed to migrate, it should still be allowed to send
/// NAT traversal probes and be able to hole-punch.
#[test]
...
fn on_path_challenge_lost_backoff() {
    ...
    // We use two helpers so we can "skip over" unrelated stopping points from
    // `next_wakeup`

    /// Attempts to drive the client until it has sent the expected amount of
    /// path challenges.
    ///
    ...
    /// Attempts to drive the server until it has received the expected amount
    /// of path challenges.
    ///
    /// Tries for a maximum of 10 iterations. Panics if that fails.
    ...
    // Kickstart the process once to get a reference point for the
    // last_challenge_sent
    ...
    // Now we stop dropping the client inbound once and the backoff should return
    // back to normal:
...
/// This test used to generate a PROTOCOL_VIOLATION error from just packet loss
/// and delayed packets.
///
/// The problem was receiving a PATH_CIDS_BLOCKED frame with path_id=1 and
/// next_seq=1 when the server side had already abandoned and discarded path 1.
/// In that case, we assumed the other side would violate the protocol, whereas
/// in reality it's just a (very) delayed packet.
///
/// The fix was to properly check if the path was already abandoned that the
/// PATH_CIDS_BLOCKED frame referred to.
#[test]
fn regression_delayed_path_cids_blocked() -> TestResult {
    ...
    // PATH_CIDS_BLOCKED frame on the next open_path call. This only works,
    // because the server's outbound packet is constructed inefficiently:
    // The PATH_NEW_CONNECTION_ID frames should *actually* be coaleced together with
    // the server's handshake response instead of being put into a separate datagram. See also <https://github.com/n0-computer/noq/issues/66>
    ...
    // Attempting to open a path on the client side will fail, because we're missing
    // the CIDs for path 1 and more
    ...
    // After the client has sent the PATH_CIDS_BLOCKED frame, we give it all the
    // server's CIDs.
    ...
    // This is now an encrypted datagram containing a PATH_CIDS_BLOCKED path_id=1
    // next_seq=1 frame.
    ...
    // Now we send the delayed PATH_CIDS_BLOCKED frame, and it'll trigger a protocol
    // violation
...
fn regression_discarded_path_stats_are_up_to_date() -> TestResult {
    ...
    // After a full handshake + MTU probing on the second path, these must be
    // non-zero.
