...
use crate::{
    ...
    SystemTime, TokenLog, TokenReuseError, Transmit, TransportConfig, VarInt, n0_nat_traversal,
};
...
pub(super) struct Pair {
    ...
    /// Simulates the maximum size allowed for UDP payloads by the link (packets
    ...
    pub(super) spins: u64,
    /// The routing table used for resolving addresses observed for incoming
    /// packets and determining whether they should get lost.
...
impl Pair {
    ...
    /// Creates an endpoint pair that'll run deterministically with hardcoded
    /// addresses.
    ...
    /// Drive both endpoints once, optionally preventing them from receiving
    /// traffic.
    ///
    ...
    /// Advance time until both connections are idle, or after 100 steps have
    /// been executed
    ///
    /// Returns true if the amount of steps exceeds the bounds, because the
    /// connections never became idle
    ...
    /// Like [`Self::connect_with`], but allows events not related to connection
    /// establishment.
    ///
    ...
    /// Asserts that client and server events lead to connection, allowing and
    /// accumulating unrelated events.
    ///
    /// The return type contains the client events and server events that were
    /// not related to connection establishment.
    ...
    /// Asserts connection events expected for a finished connection are
    /// emitted.
...
impl Connection {
    ...
    ///
    /// Returns all events found before a match is found. If no event matches
    /// `pattern` it panics.
...
impl ConnPairBuilder {
    ...
    ///
    /// This is useful if you want to construct a [`ConnPair`] while manually
    /// controlling the connection establishment. Otherwise
    /// [`Self::connect`] should be used.
    ...
    /// Builds the [`ConnPair`] and connects the two endpoints, accumulating
    /// events emitted by both endpoints that are not related to connection
    /// establishment.
...
/// Wrapper to a [`Pair`] which keeps handles to the client and server
/// connections.
...
impl ConnPair {
    ...
    /// Creates the [`ConnPair`] allowing events not related to connection
    /// establishment.
    ///
...
impl Routing {
    ...
    ///
    /// This is the address that can be used to establish a connection with the
    /// server.
...
pub(super) enum RoutingDecision {
    Deliver {
        ...
        ///
        /// In other words this becomes the [`FourTuple::remote`] for the
        /// receiver.
        src: SocketAddr,
        /// The destination IP address of the delivered packet.
        ///
        /// In other words this becomes the [`FourTuple::local_ip`] for the
        /// receiver.
        dst: Option<IpAddr>,
    },
    Drop,
...
///
/// Packets set to the wrong destination are still dropped however. But the
/// source IP they are sent from is ignored, so it is only a primitive kind of
/// routing.
///
/// You may change the addresses of either to make it look like they migrated to
/// a new address.
#[derive(Debug)]
...
///
/// Each entry on the client or server side represents a single interface in a
/// /32 subnet. Each interface has exactly one uni-directional outgoing link to
/// a peer interface. The destination interface is identified by the `usize`
/// index into the peer's interfaces `Vec`.
///
/// An interface may only appear once for a peer, so each interface only has a
/// single outgoing link. However interfaces can have multiple incoming links if
/// multiple interfaces of the peer have an outgoing link to it.
#[derive(Debug, Clone)]
...
impl ManyToManyRouting {
    ...
    fn route_client_to_server(&mut self, transmit: &Transmit) -> RoutingDecision {
        if let Some(client_interface_ip) = transmit.src_ip {
            // If we have a client interface IP, then we use that to build the packet coming
            // in on the other side. But we need to check if that is even
    ...
    fn route_server_to_client(&mut self, transmit: &Transmit) -> RoutingDecision {
        if let Some(server_interface_ip) = transmit.src_ip {
            // If we have a server interface IP, then we use that to build the packet coming
            // in on the other side. But we need to check if that is even
    ...
    /// Each interface has an outgoing link to the peer's interface with the
    /// same index.
    ///
    /// This produces a routing table where each link is bi-directional (or
    /// symmetric) and connected to the corresponding index of the peer
    /// interfaces.
    ///
    ...
    /// Adds a new route from an existing server address (identified by index)
    /// to a new client address.
    ...
    /// Adds a new route from an existing client address (identified by index)
    /// to a new server address.
...
///
/// This essentially behaves like a Destination Endpoint Independent Mapping NAT
/// with address and port filtering: EIM + ADF in [RFC4748] terminology. But
/// without having to simulate the public side of the network.
///
/// This same routing can be used for EIM + EIF: Endpoint-Independent filtering.
/// In this case the filtering is effectively opened by the QAD probe, so all
/// that is needed is configure it with [`Self::client_firewall_open`] and/or
/// [`Self::server_firewall_open`] from the start.
///
/// This is pretty simplistic, but tests the basics.
///
/// The client and server both have 2 interfaces:
///
/// 1. One "direct" interface, which has a link to the peer's "direct"
///    interface.
/// 2. One "nat" interface, which has a link to the peer's "nat" interface. This
///    link however does not allow an incoming packet unless it has seen an
///    outgoing packet first.
///
/// When an outgoing transmit has no `src_ip` is set, the source IP is set based
/// on the destination. This is the same as the kernel selecting the correct
/// outbound interface. If an outgoing transmit has an `src_ip` of an interface
/// that is not linked to the interface of the destination IP then a transmit is
/// dropped.
///
...
pub(super) struct SimpleFirewallRouting {
    ...
    ///
    /// If so packets from `server_nat` to `client_nat` will be allowed. If not
...
impl SimpleFirewallRouting {
    ...
    /// Routes a datagram from client to server.
    ///
    /// Returns the address the server will observe as the sender of the
    /// datagram. Or `None` if there is no open link.
    ///
    /// If there is no [`Transmit::src_ip`] then this routing table selects one
    /// based on the destination, if reachable. Otherwise if the `src_ip`
    /// does not match an open link the datagram is blocked.
    fn route_client_to_server(&mut self, transmit: &Transmit) -> RoutingDecision {
        // Find the address this datagram SHOULD have been sent on to be able to reach
        // the destination.
    ...
    /// Routes a datagram from server to client.
    ///
    /// Returns the address the client will observe as the sender of the
    /// datagram. Or `None` if there is no open link.
    ///
    /// If there is no [`Transmit::src_ip`] then this routing table selects one
    /// based on the destination, if reachable. Otherwise if the `src_ip`
