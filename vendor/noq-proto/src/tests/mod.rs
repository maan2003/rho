...
use crate::{
    ...
    SendDatagramError, ServerConfig, StreamEvent, Transmit, TransportConfig, TransportErrorCode,
    VarInt, WriteError,
};
...
// Unfortunately it's either-or: Enable this and you can run in the browser,
// disable to run in nodejs. #[cfg(all(target_family = "wasm", target_os =
// "unknown"))] wasm_bindgen_test::wasm_bindgen_test_configure!(run_in_browser);
...
fn version_negotiate_client() {
    ...
    // Configure client to use empty CIDs so we can easily hardcode a server version
    // negotiation packet
...
fn reject_self_signed_server_cert() {
    ...
    // Create a self-signed certificate with a different distinguished name than the
    // default one, such that path building cannot confuse the default root the
    // server is using and the one the client is trusting (in which case we'd
...
fn reject_missing_client_cert() {
    ...
    // `WebPkiClientVerifier` requires a non-empty store, so we stick our own
    // certificate into it because it's convenient.
...
fn congestion() {
    ...
    let s = pair.client_streams(client_ch).open(Dir::Uni).unwrap();
    // Send data without receiving ACKs until the congestion state falls below
    // target
...
fn zero_rtt_rejection() {
    ...
    // We want to have a TLS client config with the existing session cache (so
    // resumption could happen), but with different ALPN protocols (so that the
    // server must reject it). Reuse the existing `ClientConfig` and change the
    // ALPN protocols to make that happen.
    ...
    // `s2` was never successfully received by the server, so from its perspective
    // the stream has never been opened. We currently surface that as
    // `ClosedStream`.
...
fn test_zero_rtt_incoming_limit<F: FnOnce(&mut ServerConfig)>(configure_server: F) {
    ...
    // the next three are incoming-buffered, bringing the incoming buffer size to
    // 3600 bytes the last four are dropped due to the buffering limit and must
    // be retransmitted
...
fn streams_blocked_not_sent_under_limit() {
    ...
    // Default config allows many streams; opening one should not trigger
    // STREAMS_BLOCKED
    let s = pair
        .client_streams(client_ch)
        .open(Dir::Uni)
...
/// Client closes the connection at the same time it experience a passive
/// migration.
#[test]
fn close_from_migrated_address() {
    ...
    // Client closes connection from the NEW address.  The server will see the
    // migration and close the connection on the new address.
    // TODO(flub): Potentially the server should also send the CONNECTION_CLOSE to
    // the old    address. Because the new one has not yet been validated.
...
fn migration() {
    ...
    // Assert that the client's response to the PATH_CHALLENGE was an IMMEDIATE_ACK,
    // instead of a second ping
...
/// Regression test: handle sent challenges that are waiting for a response
/// while a passive migration occurs.
///
...
fn regression_path_validation_stale_local_after_passive_migration() {
    ...
    // Drop the response before it reaches the client. We want to simulate the
    // response arriving only *after* passive migration has changed the client's
    // observed local IP.
    pair.client.inbound.clear();

    pair.routes.as_basic_mut().passive_migration(Client);

    // Send a ping so the server detects the migration and starts routing to the new
    // ip.
    ...
    //   self.network_path.local_ip = Some(127.0.0.2)  (updated after passive
    // migration)
    //
    // Without the fix: challenge_A is never cleaned up, PathChallengeLost fires
    // forever.
    assert!(
...
fn handshake_1rtt_handling() {
    ...
    pair.drive_server();
    let server_ch = pair.server.assert_accept();
    // Server now has 1-RTT keys, but remains in Handshake state until the TLS CFIN
    // has authenticated the client. Delay the final client handshake flight so
...
// Ensure we can recover from loss of tail packets when the congestion window is
// full
#[test]
fn congested_tail_loss() {
    ...
    pair.server.inbound.clear();
    // Ensure that the congestion state recovers after retransmits occur and are
    // ACKed
    info!("recovering");
    pair.drive();
    assert!(pair.client_conn_mut(client_ch).congestion_window() > TARGET);
...
fn tail_loss_small_segment_size() {
    ...
    // Now we can send another batch of datagrams, so the PTO can send them instead
    // of sending a ping.  These are small enough that the segment_size is less
    // than the INITIAL_MTU.
    ...
    // If this succeeds the datagrams are received by the server and the client did
    // not crash.
    pair.drive();
...
#[test]
/// Ensure that we don't yield a finish event before the actual FIN is acked so
/// the peer isn't left hanging
fn finish_acked() {
    let _guard = subscribe();
    let mut pair = Pair::default();
    let (client_ch, server_ch) = pair.connect();
...
#[test]
/// Ensure that we don't yield a finish event while there's still unacknowledged
/// data
...
/// Ensures that exchanging data on a client-initiated bidirectional stream
/// works past the initial stream window.
#[test]
...
/// Ensures that the client sends an anti-deadlock probe after an incomplete
...
/// Ensures that the server can respond with 3 initial packets during the
/// handshake before the anti-amplification limit kicks in when MTUs are
/// similar.
#[test]
...
/// Generate a big fat certificate that can't fit inside the initial
...
#[test]
/// This is mostly a sanity check to ensure our testing code is correctly
/// dropping packets above the pmtu
...
fn migrate_detects_new_mtu_and_respects_original_peer_max_udp_payload_size() {
    ...
    // Set up a client with a max payload size of 1400 (and use the defaults for the
    // server)
    ...
    // Sanity check: MTUD ran to completion (the numbers differ because binary
    // search stops when changes are smaller than 20, otherwise both endpoints
    // would converge at the same MTU of 1300)
    ...
    // Sanity check: the client keeps the old MTU, because migration is triggered by
    // incoming packets from a different address
    assert_eq!(pair.client_conn_mut(client_ch).path_mtu(PathId::ZERO), 1293);
}
...
fn connect_runs_mtud_again_after_600_seconds() {
    ...
    // Note: we use an infinite idle timeout to ensure we can wait 600 seconds
    ...
    // Sanity check: the mtu does not change after the fact, even though the link
    // now supports a higher udp payload size
    pair.mtu = 1500;
    pair.drive();
...
fn blackhole_after_mtu_change_repairs_itself() {
    ...
    // The payload will be sent in a single packet, because the detected MTU was
    // 1444, but it will be dropped because the link no longer supports that
    // packet size!
...
fn packet_splitting_with_default_mtu() {
    ...
    // The payload needs to be split in 2 in order to be sent, because it is higher
    // than the max MTU
    let payload = vec![42; 1300];
...
fn single_ack_eliciting_packet_triggers_ack_after_delay() {
    ...
    // Sanity check: no loss probe was sent, because the delayed ACK was received on
    // time
    assert_eq!(
...
fn out_of_order_ack_eliciting_packet_triggers_ack() {
    ...
    // Send a packet that won't arrive right away (it will be dropped and be re-sent
    // later)
    pair.mtu = 0;
    pair.client_conn_mut(client_ch).ping();
    pair.drive_client();
    ...
    // Restore the default MTU and send another ping, which will arrive earlier than
    // the dropped one
    pair.mtu = default_mtu;
    pair.client_conn_mut(client_ch).ping();
    pair.drive_client();
...
/// Verify that an endpoint which receives but does not send ACK-eliciting data
/// still receives ACKs occasionally. This is not required for conformance, but
/// makes loss detection more responsive and reduces receiver memory use.
#[test]
...
fn stream_gso() {
    ...
    // Send 20KiB of stream data, which comfortably fits inside two
    // `tests::util::MAX_DATAGRAMS` datagram batches
    info!("sending");
    for _ in 0..20 {
...
fn datagram_gso() {
    ...
    // Send 10 datagrams above half the MTU, which fits inside a
    // `tests::util::MAX_DATAGRAMS` datagram batch
    info!("sending");
    ...
    // Expected overhead: flags + CID + PN + tag + frame type + frame length = 1 + 8
    // + 1 + 16 + 1 + 2 = 29
    assert_eq!(
...
fn gso_truncation() {
    ...
    // Send three application datagrams such that each is large to be combined with
    // another in a single MTU, and the second datagram would require an
    // unreasonably large amount of padding to produce a QUIC packet of the same
    // length as the first.
    info!("sending");
...
/// Verify that UDP datagrams are padded to MTU if specified in the transport
/// config.
#[test]
fn pad_to_mtu() {
    let _guard = subscribe();
    ...
    info!("sending");
    // Send two datagrams significantly smaller than MTU, but large enough to
    // require two UDP datagrams.
...
/// Verify that a large application datagram is sent successfully when an ACK
/// frame too large to fit alongside it is also queued, in exactly 2 UDP
/// datagrams.
#[test]
...
/// Verify that an ACK prompted by receipt of many non-ACK-eliciting packets is
/// sent alongside outgoing application datagrams too large to coexist in the
/// same packet with it.
#[test]
fn voluntary_ack_with_large_datagrams() {
    ...
    let initial_datagrams = pair.client_conn_mut(client_ch).stats().udp_tx.datagrams;
    // Send enough packets that we're confident some packet numbers will be skipped,
    // ensuring that larger ACKs occur
    ...
    let final_datagrams = pair.client_conn_mut(client_ch).stats().udp_tx.datagrams;
    // Failure may indicate `max_size` is too small and ACKs are reliably being
    // packed into the same datagram, which is reasonable behavior but makes
...
fn address_discovery() {
    ...
    // Create the connections accumulating events not related to connection
    // establishment.
    ...
    // Depending on handshake size, ObservedAddr reports might arrive before or
    // after the handshake is confirmed. Gather all events to account for that.
...
/// Test that a different address discovery configuration on 0rtt used by the
/// client is accepted by the server.
/// NOTE: this test is the same as zero_rtt_happypath, changing client transport
/// parameters on resumption.
#[test]
...
/// Test that a different address discovery configuration on 0rtt used by the
/// server is rejected by the client.
/// NOTE: the server MUST not change configuration on resumption. However, there
/// is no designed behaviour when this is encountered. noq chooses to accept and
/// then close the connection, which is what this test checks.
#[test]
fn address_discovery_zero_rtt_rejection() {
    ...
    // Changing address discovery configurations makes the client close the
    // connection
    pair.server
...
fn address_discovery_rebind_retransmission() {
    // NOTE: unlike `address_discovery_retransmission`, in which we drop packets
    // even if during the handshake, `address_discovery_rebind_retransmission`
    // needs to also migrate. Migrating and dropping packets causes any incoming
    // packets from the client to the server to be discarded, which ultimately
    // prevents the connection from being established.
    //
    // Instead, for this test, we drop in-flight data and migrate after the
    ...
    // This implementation sends address discovery reports with path challenges, so
    // start a path validation attempt that will produce the frame we will later
    // drop
...
/// Verify that dropping oversized datagrams will trigger a DatagramsUnblocked
/// event.
#[test]
fn oversized_datagrams_trigger_unblock() {
    ...
    // Drive the pair until black hole detection kicks in and the path MTU is
    // adjusted.
    while pair.step() {
        let err = loop {
            if let Err(e) = pair
...
/// Ensure that a connection can be made when a preferred address is advertised
/// by the server, regardless of whether the address is actually used.
#[test]
...
///
/// Downstream, this causes ConnectionClose frames to shift the "reason" they
/// encode too far back (by exactly 7 zeroes too much), in some cases, causing
/// the other side to misinterpret the reason bytes as other frames and erroring
/// out badly.
#[test]
...
/// Regression test simulating a situation that would trigger an unreachable! in
/// noq.
///
/// - noq expects that there always is a `ConnectionError` set at the end of
///   draining a connection.
/// - When the connection close is generated within noq-proto (or received
///   remotely), then this error in noq is populated via connection events.
...
///
/// The issue is a race condition between `move_to_closed` and
/// `move_to_draining(None)`. The latter overwrites the error from the former,
/// making it impossible to generate a `ConnectionLost` event, even though no
/// such event had previously made it out of noq-proto.
///
/// We simulate hitting this race condition by concurrently closing the
/// connection on both the client and server side. The client side closes it
/// from within noq-proto by simulating some arbitrary protocol violation.
#[test]
...
/// Ensures that the draining delay for the server is exactly 0.5 RTT and 1 RTT
/// for the client.
///
/// The draining delay is the time between the connection being closed and the
/// connection entering the "draining" state (either on the same or on the other
/// side).
///
/// We expect the side that *receives* the CONNECTION_CLOSE to immediately enter
/// the draining state. However in absolute terms, it'll be delayed by 0.5 RTT
/// (exactly the latency) compared to when `connection.close()` was called.
/// On the side that called `connection.close()` we first enter the "closed"
/// state, and only enter the "draining" state once we *receive* a "reciprocal"
/// CONNECTION_CLOSE from the other side. In the normal case this will be
/// exactly 1 RTT after calling `connection.close()` to account for the latency
/// of CONNECTION_CLOSE going one way and then coming back.
///
/// The "draining" state from noq-proto is observed by noq to enable `wait_idle`
/// waiting the ideal amount of time before allowing us to close the socket.
#[test]
fn timely_graceful_close() {
    ...
    // The server has now sent a CONNECTION_CLOSE back in response and the client
    // processes it. The client then enters the draining state once it processed
    // the response. already drove server
    pair.advance_time();
    let now = pair.time;
    pair.drive_client();
...
///
/// The server's first response during the handshake is lost. It should send 2
/// tail-loss probes correctly.
#[test]
fn initial_tail_loss_probe() {
    ...
    // Neither peers have anything to send right now, but steps time forward to next
    // timeout which is on the client-side.
    pair.step();

    // Client PTO fires: sends 2 tail-loss probes, padded because Initial and
    // ack-eliciting.
    ...
    // Server PTO fires: sends 2 tail-loss probes in the Initial space, padded
    // because Initial and ack-eliciting.
