...
use crate::tests::random_interaction::{TestOp, run_random_interaction};
use crate::tests::util::{
    ManyToManyRouting, Pair, Routing, client_config, server_config, subscribe,
};
use crate::{
...
/// - On a proptest failure, it is easy to see which minimal setup the proptest
///   fails with.
/// - The definition of the setup itself is concise, e.g. when copying it into
///   regression tests.
/// - We can be more precise/smaller in the "search space" and have more
///   efficient shrinking, see [`Seed`] or [`RoutingSetup`].
...
///
/// The advantage of using this is very efficient shrinking: The first attempt
/// at shrinking the routing setup will be to reduce the routing setup to
/// nothing or a simple symmetric one.
#[derive(Debug, test_strategy::Arbitrary)]
pub(super) enum RoutingSetup {
    /// Set [`Pair::routes`] to [`BasicRouting`]
    Basic,
    /// Use [`RoutingTable::simple_symmetric`] with the default [`CLIENT_ADDRS`]
    /// and [`SERVER_ADDRS`].
    SimpleSymmetric,
...
///
/// This structure has an advantage over a simple `[u8; 32]`, because on one
/// hand, we don't want to waste too much time shrinking the seed itself
/// (reducing individual values inside the array), but also we don't want to
/// disable shrinking altogether: when the seed shrinks to zero, this
...
/// `Generated(some_seed)`, shrinking will try `Zeroes` once, and if that fails,
/// fall back to using the generated seed and avoid doing any further shrinking
/// of `some_seed`.
#[derive(Debug, test_strategy::Arbitrary)]
enum Seed {
    /// The zero seed.
    ///
    /// If a test generates the zero seed, then it's likely that the seed
    /// doesn't have any effect on the test failure.
    Zeroes,
...
///
/// Client and server have multiple interfaces, but all outgoing links go to the
/// first interface of defined for the peer.
...
/// In proptests, we only allow connection errors that don't indicate erroring
/// out because we think we're working with another implementation that isn't
/// protocol- abiding. If we think that, clearly something is wrong, given we're
/// controlling both ends of the connection.
...
/// Regression test for the "invalid key" panic in
/// `noq-proto::Endpoint::handle_event`.
///
/// This test establishes this situation:
/// - There's a `Connection` in the `Drained` state (using close connection &
///   advance time).
/// - We try to generate another `EndpointEvent` from the connection via closing
///   the path.
///
/// Noq has an invariant that the last endpoint event allowed is
/// `EndpointEventInner::Drained`, but this invariant was violated in
/// `Connection::close_path_inner`, as that can be called on drained connections
/// via an API.
///
/// We fixed this bug by short-circuting in `close_path_inner` if the connection
/// is drained.
#[test]
...
///
/// - The server sees the remote as 1.1.1.0 and had previously sent and received
///   on that address in this connection and thus considers it valid, while
/// - the client side first sends to 2.2.2.1 but gets the response from the
///   remote 2.2.2.0, thus it fails validation on the client side and it ignores
///   the packet.
///
...
/// 2. The client opens another path on the same 4-tuple (thus that path is
///    immediately validated).
/// 3. It immediately closes path 0 afterwards.
///
/// At this point, the server is already fully checked out and not responding
/// anymore. The client however thinks the connection is still ongoing and
/// continues sending (that's fine). However, it never stops sending path
/// challenges, because of a bug where only when the path validation timer times
/// out, the path challenge lost timer was stopped. This means the client would
/// keep re-sending path challenges infinitely (never getting a response,
...
/// This test will loop forever, unless the loss detection timer is allowed to
/// back off infinitely beyond the idle timeout.
///
/// The situation this test creates is one where there are two paths between
/// client and server, and path 0 is fully broken, while path 1 is fully
/// working.
///
/// There is one packet with acknowledgements on path 0 that is sent but never
/// delivered, and the path will be broken forever due to the passive migration.
///
/// This will cause sending tail-loss probes (in practice that's an
/// IMMEDIATE_ACK frame) on path 1 forever, or at least until the loss detection
/// timer is bigger than the idle timeout, at which point the whole connection
/// times out instead of resending.
///
/// Changes to the backoff behavior that make the loss detection timer not
/// explode beyond the idle timeout will make this test fail.
///
/// We fix this test by fixing the test setup: If we allow path 0 to time out on
/// its own with its own idle timeout, then the path closes and we finally stop
/// resending tail loss probes.
#[test]
...
///
/// This test ended up severing the connection in an interesting way: It
/// generates a passive migration just before closing the path. The passive
/// migration breaks path 0.
/// That path used the network path (1.1.1.0, 2.2.2.0), but was changed to
/// (1.1.1.1, 2.2.2.0) by a middlebox without the server noticing, since the
/// client never ended up sending anything on that path.
/// This means that path 0 is effectively broken on the server side, but the
/// server still has path 0 verified.
/// When the server then wants to abandon path 1, it chooses path 0 as the path
/// to send the path abandon on, even though that path is "doomed forever".
/// No retransmits make it through, and the client keeps using path 1, thus
/// eventually the server thinks the client ignored the PATH_ABANDON frame,
/// although the client just never *received* that frame.
///
...
/// arriving means the client is not protocol compliant or just under bad
/// network.
///
/// To prevent memory accumulation due to malicious client behavior, we now
/// delay sending MAX_PATH_ID until the client reciprocated the PATH_ABANDON
/// instead.
#[test]
...
/// A regression test that used to put noq into a state of sending
/// PATH_CHALLENGE from the client side indefinitely.
///
/// The test uses passive migrations to establish a situation in which the
/// client expects the server to respond to a PATH_CHALLENGE that was sent on
/// the interface 1.1.1.1, but the server cannot respond to it anymore, because
/// the client was involuntarily migrated to path 1.1.1.2.
///
...
///
/// This issue was fixed by making sure to clear out challenges that were
/// probing 4-tuples that are different from the current network path.
#[test]
...
/// - Upon detecting loss, it would re-set the loss detection timer to `now`
///   (0ms delay) again
/// - Within a single `pair.step()` it would go back to the loss detection timer
///   firing (it expects timers to fire in the future eventually.)
///
/// The reason this tight loop existed was because the loss detection timer is
/// relative to the `time_of_last_ack_eliciting_packet`. If this becomes too old
/// (and we cap the PTO duration to a maximum), then this will always be in the
/// past, thus causing timers to be set to the past.
///
/// Usually, the timer would fire and then cause something to happen, e.g. we
/// send a tail-loss probe.
/// But in this case it didn't because the tail-loss probe was "scheduled" for
/// path 1, which in this example is already abandoned by the time the loss
/// detection timer fired.
///
/// This caused the conditions for the loss detection timer to never be cleared,
/// as in-flight packets would sit in that path indefinitely.
///
/// The fix was to only schedule loss detection timers for tail-loss probes when
/// the path is not yet abandoned.
#[test]
...
/// This test reproduced a situation in which a QNT-enabled connection sends
/// path challenges indefinitely.
///
/// In this test setup, we enable QNT, call the required functions for adding
/// addresses to holepunch, and then eventually initiate the first holepunching
/// round. Before that, we also trigger a passive migration on the server side,
/// effectively severing the connection in the server -> client direction on
/// path 0 (the only path at that time), because all packets are rejected on the
/// client side as coming from the wrong address.
///
/// What follows is that the server sends PATH_CHALLENGEs for path 0 (as that's
/// what we've added as the "holepunching address"), and initiating the
/// holepunching means that we reuse existing paths if we already have one on
/// the required address, but we do *revalidate* them (triggering new
/// PATH_CHALLENGEs).
///
/// However, in this code path, we didn't have anything that would prevent
/// re-validated path challenges to ever be stopped, so this revalidation would
/// keep the connection busy in the path challenge sent -> path challenge lost
/// -> path challenge sent loop.
///
/// We fixed this bug by introducing another `OpenState::Revalidating`, and
/// arming the `PathOpenFailed` timer when we start revalidating a path.
#[test]
...
/// This reproduced a never-idle infinite loop where both the client and server
/// would infinitely migration-probe each other.
///
/// This was fixed by disallowing migration probing on the client side (it was
/// accidentally enabled).
///
/// This test would have both client and server swap back and forth the 4-tuple
/// on one path between (1.1.1.2, 2.2.2.0) and (1.1.1.1, 2.2.2.2).
///
/// Each time the server detected the client's migration, it would switch its
/// 4-tuple and probe the previous path.
/// This would then trigger migration detection on the client side, making it
/// probe both 4-tuples itself, causing the server to detect yet another
/// migration and so on.
#[test]
...
/// Test for a case where we kept sending so many PATH_CHALLENGEs that we'd run
/// out of `drive_bounded` budget.
///
/// The original proptest found a situation in which the newly opened path was
/// working one-way. The client was able to send to the server, but the
/// responses from the server back to the client were not coming through.
///
/// This meant that PATH_CHALLENGEs were received, PATH_RESPONSEs were sent
/// back, but the client didn't receive them.
/// This in turn means that the client will re-send PATH_CHALLENGEs.
///
/// At the same time, the PATH_CHALLENGEs were acknowledged by the server and
/// the acknowledgements were sent over a different path.
///
/// This meant that the client had updated its RTT estimates for the path, even
/// though the path was not yet working.
/// Initially when the client opened the path, the RTT estimate was the initial
/// RTT estimate of 333ms. This resulted in the `AbandonFromPathValidation`
/// timer to be set at ~3s. After the first couple of ACKs, the path's RTT
/// estimate quickly updated to 1ms though.
///
/// When this test was failing, PATH_CHALLENGEs were re-sent after 1 PTO without
/// a response. This meant that it would re-send challenges every 2ms,
/// generating many hundreds of PATH_CHALLENGEs before the path would be
/// abandoned.
///
/// The fix was to implement PATH_CHALLENGE re-sending backoffs, similar to tail
/// loss probe backoff.
#[test]
