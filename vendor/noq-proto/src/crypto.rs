...
//!
//! The protocol logic in noq is contained in types that abstract over the
//! actual cryptographic protocol used. This module contains the traits used for
//! this abstraction layer as well as a single implementation of these traits
//! that uses *ring* and rustls to implement the TLS protocol support.
//!
...
use crate::{ConnectError, PathId, Side, TransportError};
...
pub trait Session: Send + Sync + 'static {
    /// Create the initial set of keys given the client's initial destination
    /// ConnectionId
    ...
    ///
    /// On the client side, this method can be used to see if 0-RTT key material
    /// is available to start sending data before the protocol handshake has
    /// completed.
    ///
    /// Returns `None` if the key material is not available. This might happen
    /// if you have not connected to this server before.
    ...
    ///
    /// This should be called with the contents of `CRYPTO` frames. If it
    /// returns `Ok`, the caller should call `write_handshake()` to check if
    /// the crypto protocol has anything to send to the peer. This method
    /// will only return `true` the first time that handshake data is
    /// available. Future calls will always return false.
    ///
    /// On success, returns `true` iff `self.handshake_data()` has been
    /// populated.
    fn read_handshake(&mut self, buf: &[u8]) -> Result<bool, TransportError>;

    /// The peer's QUIC transport parameters
    ///
    /// These are only available after the first flight from the peer has been
    /// received.
    fn transport_parameters(&self) -> Result<Option<TransportParameters>, TransportError>;

    /// Writes handshake bytes into the given buffer and optionally returns the
    /// negotiated keys
    ///
    /// When the handshake proceeds to the next phase, this method will return a
    /// new set of keys to encrypt data with.
...
pub trait PacketKey: Send + Sync {
    ...
    /// Maximum number of incoming packets that may fail decryption before the
...
/// The trait for encrypting tokens that power retry packets and NEW_TOKEN
/// frames.
