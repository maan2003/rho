    ...
    fn allocate_helper_retry(
        ...
        loop {
            let Some(candidate_region) = state.get_region_tracker_mut().find_free(required_order)
                ...
                // Files written by redb <= 4.1 can carry tracker bits that say
                // a region has no block of this order when it does (frees
                // only cleared bits up to the freed page's own order). Check
                // the regions themselves before giving up, lowest first.
                let regions = state.header.layout().num_regions();
                let found = (0..regions).find(|region| {
                    state
                        .get_region(*region)
                        .highest_free_order()
                        .is_some_and(|order| order >= required_order)
                });
                let Some(region) = found else {
                    return Ok(None);
                };
                let order = state.get_region(region).highest_free_order().unwrap();
                state.get_region_tracker_mut().mark_free(order, region);
                continue;
            };
