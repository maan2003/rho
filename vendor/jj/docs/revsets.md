...
workspace. Managed workspaces allocated by `jj workspace managed create` use
full `ws-<id>` handles. A repository-local prefix can be used in
`ws-<id prefix>@` (or as a plain revset symbol); if it matches multiple
generated ids, jj selects the first generated matching id. Managed handles are not user
workspace names and cannot be renamed. Use `<name>@<remote>` to refer to a remote-tracking tag / bookmark.
