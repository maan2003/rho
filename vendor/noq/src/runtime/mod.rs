...
pub trait AsyncUdpSocket: Send + Sync + Debug + 'static {
    /// Create a [`UdpSender`] that can register a single task for
    /// write-readiness notifications and send a transmit, if ready.
    ///
    /// A `poll_send` method on a single object can usually store only one
    /// [`Waker`] at a time, i.e. allow at most one caller to wait for an
    /// event. This method allows any number of interested tasks to
    /// construct their own [`UdpSender`] object. They can all then wait for the
    /// same event and be notified concurrently, because each [`UdpSender`] can
    /// store a separate [`Waker`].
    ///
    /// [`Waker`]: std::task::Waker
    fn create_sender(&self) -> Pin<Box<dyn UdpSender>>;

    /// Receive UDP datagrams, or register to be woken if receiving may succeed
    /// in the future
    fn poll_recv(
        &mut self,
        cx: &mut Context<'_>,
    ...
    ///
    /// Sockets should prevent this for best performance. See e.g. the
...
///
/// Any number of [`UdpSender`]s may exist for a single [`AsyncUdpSocket`]. Each
/// [`UdpSender`] is responsible for notifying at most one task for send
/// readiness.
pub trait UdpSender: Send + Sync + Debug + 'static {
    /// Send a UDP datagram, or register to be woken if sending may succeed in
    /// the future.
    ///
    /// Usually implementations of this will poll the socket for writability
    /// before trying to write to them, and retry both if writing fails.
    ///
    /// noq will create multiple [`UdpSender`]s, one for each task it's using it
    /// from. Thus it's important to poll the underlying socket in a way
    /// that doesn't overwrite wakers.
    ///
    /// A single [`UdpSender`] will be reused, even if `poll_send` returns
    /// `Poll::Ready` once, unlike [`Future::poll`], so calling it again
    /// after readiness should not panic.
    fn poll_send(
        self: Pin<&mut Self>,
        transmit: &Transmit<'_>,
        cx: &mut Context<'_>,
    ...
    WritableFut: Future<Output = io::Result<()>> + Send + Sync + 'static,
{
    fn poll_send(
        ...
        loop {
            ...
            // We're forced to `unwrap` here because `Fut` may be `!Unpin`, which means we
            // can't safely obtain an `&mut WritableFut` after storing it in
            // `self.writable_fut` when `self` is already behind `Pin`,
            ...
            // Polling an arbitrary `Future` after it becomes ready is a logic error, so
            // arrange for a new `Future` to be created on the next call.
...
/// Parts of the [`UdpSender`] trait that aren't asynchronous or require storing
/// wakers.
///
...
trait UdpSenderHelperSocket: Send + Sync + 'static {
    ...
    ///
    /// If not write-ready, this is allowed to return
    /// [`std::io::ErrorKind::WouldBlock`].
    ///
    /// The [`UdpSenderHelper`] will use this to implement
    /// [`UdpSender::poll_send`].
...
/// Automatically select an appropriate runtime from those enabled at compile
/// time
///
/// If `runtime-tokio` is enabled and this function is called from within a
/// Tokio runtime context, then `TokioRuntime` is returned. Otherwise, if
/// `runtime-smol` is enabled, `SmolRuntime` is returned. Otherwise, `None` is
/// returned.
#[cfg(any(feature = "runtime-tokio", feature = "runtime-smol"))]
