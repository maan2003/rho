...
impl OpenPath {
    ...
    ///
    /// If an error occurred before a path ID was allocated, `None` is returned.
    /// In this case the future is ready and polling it will immediately
    /// yield the error.
    ///
    /// The returned value remains the same for the entire lifetime of this
    /// future.
...
/// [`PathStats`] for this path are not dropped even after the path is
/// abandoned.
///
...
impl Path {
    /// Returns a [`Path`] for a path id, after checking that the path is not
    /// closed.
    ...
    /// Returns a [`Path`] for a path id without checking if the path exists or
    /// is closed.
    ...
    ///
    /// Holding a [`WeakPathHandle`] does not keep a connection alive, but
    /// ensures that the path's stats are not dropped until the underlying
    /// connection is dropped, even if the path is abandoned.
    ...
    pub fn stats(&self) -> PathStats {
        ...
        // - `Path` and its clones or `WeakPathHandle`s all increment the connection
        //   state's `path_ref` reference counter
        // - As long as a path is not abandoned, its stats are available from
        //   `proto::Connection`
        // - If a path is abandoned, the `crate::Connection` stores the final stats as
        //   long as the path's refcount is not 0
    ...
    /// Closes this path.
    ///
    /// The path is immediately considered closed by the local endpoint. Once
    /// the state is removed, after a short period of time for any in-flight
    /// packets, a [`PathEvent::Abandoned`] is returned.
    ...
    ///
    /// If the address-discovery extension is not negotiated, the stream will
    /// never return.
    ...
    ///
    /// Returns `Ok(None)` for clients or when the platform does not expose this
    /// information; see
    /// [`noq_udp::RecvMeta::dst_ip`](udp::RecvMeta::dst_ip) for supported
    /// platforms.
    ...
    ///
    /// Returns a [`FourTuple`], combining [`Self::remote_address`] and
    /// [`Self::local_ip`].
...
///
/// As long as a [`WeakPathHandle`] for a path exists, that path's final stats
/// will not be dropped even if the path was abandoned.
///
/// The [`WeakPathHandle`] can be upgraded to a [`Path`] as long as its
...
///
/// Holds the shared [`AtomicUsize`] but does not itself contribute to the
/// count. Hands out [`PathRef`] handles via [`Self::acquire`].
///
...
/// Handle side of a path's reference counter, held by [`Path`] and
/// [`WeakPathHandle`].
///
/// Cloning bumps the counter automatically. When dropping a `PathRef`, holders
/// must call [`Self::on_drop`] to decrement the counter and clear the
/// corresponding entries from the connection state if the counter reaches zero.
///
/// [`WeakPathHandle`]: crate::path::WeakPathHandle
#[derive(Debug)]
...
impl PathRef {
    ...
    ///
    /// This must be called before dropping a [`PathRef`], i.e. in the [`Drop`]
    /// impl of its holder.
...
///
/// This will always return the external address most recently reported by the
/// remote over this path. If the extension is not negotiated, this stream will
/// never return.
...
impl AddressDiscovery {
    pub(super) fn new(
        ...
        // TODO(@divma): check if there's a way to ensure the future ends. AbortHandle
        // is not an option
        Self { watcher }
    }
}
