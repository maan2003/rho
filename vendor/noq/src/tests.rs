...
use proto::{ConnectionError, FourTuple, PathId, PathStatus, RandomConnectionIdGenerator};
...
///
/// Uses environment variables to avoid pulling in `windows-sys` as a
/// dev-dependency.
fn is_wine() -> bool {
...
async fn zero_rtt() {
...

    {
        ...
        let msg = stream.read_to_end(usize::MAX).await.expect("read_to_end");
        assert_eq!(msg, MSG0);
        // Read a 1-RTT message to ensure the handshake completes fully, allowing the
        // server's NewSessionTicket frame to be received.
        let mut stream = connection.accept_uni().await.expect("incoming streams");
        let msg = stream.read_to_end(usize::MAX).await.expect("read_to_end");
        assert_eq!(msg, MSG1);
        drop(connection);
...
fn run_echo(args: EchoArgs) {
    ...
    let handle = {
        ...
        // We don't use the `endpoint` helper here because we want two different
        // endpoints with different addresses.
        let cert = rcgen::generate_simple_self_signed(vec!["localhost".into()]).unwrap();
        ...
        runtime.block_on(
            async move {
                ...
                /// This is just an arbitrary number to generate deterministic
                /// test data
...
async fn stream_id_flow_control() {
    ...
    // If `open_uni` doesn't get unblocked when the previous stream is dropped, this
    // will time out.
    tokio::join!(
        async {
...
async fn test_multipath_observed_address() {
    ...
    let client_task = async move {
        ...
        // TODO(@divma): this is not fixed by removing the early check of remote CIDs,
        // at least not right now. Removing the check makes poll_transmit panic
        // somewhere. So, eval removing this sleep after the poll_transmit
        // unwraps have been addressed
        ...
        // in this instance the test is local and the locally known and remotely
        // observed addresses should coincide
...
async fn dropped_endpoint_cleans_up() {
    ...
    drop(endpoint);
    // Let the driver task run; paused runtimes are guaranteed to drain pending work
    // on sleep.
...
async fn path_clone_stats_after_abandon() {
    ...
    let client_task = async move {
        ...
        // After dropping the path, upgrading fails after the endpoint cleared the
        // connection.
        drop(path_clone);
...
/// connection, then checks that the server's
/// `get_remote_nat_traversal_addresses` returns the client's address within
/// 500ms — proving the REACH_OUT frame was sent and processed promptly.
///
    ...
    for drop_stream in [false, true] {
        ...
        let server_task = tokio::spawn(async move {
            ...
            if !drop_stream {
                ...
                // We have a blocked reader, closing the connection should wake it. We use this
                // as a proxy to assert that the stream is in
...
async fn recv_stream_cancel_stop_drop() {
    ...
    tokio::join!(
        async {
            ...
            // Create a future to read from the stream, poll it once, then immediately drop
            // it
            {
...
/// Regression test for an `active_connections` underflow panic in the endpoint
/// driver.
///
/// `ConnectionSet::insert` used to only increment `active_connections` when the
/// endpoint isn't already closed, but `State::handle_events` decremented it
/// unconditionally when a connection reports that it started draining.
/// Accepting an `Incoming` *after* `Endpoint::close` therefore inserted a
/// connection that is never counted, yet still drains and emits a `Draining`
/// endpoint event, causing `active_connections -= 1` to underflow.
///
...
async fn regression_incoming_accept_after_endpoint_close_panic() -> TestResult {
    ...
    // self-connect, but immediately close the client side, to ensure it doesn't
    // linger
    ...
    // advance the time to allow for draining connections (we can be generous due to
    // simulated time)
...
///
/// Provides [`PanicPropagatingRuntime::assert_tasks_ok`] to assert all tasks
...
impl PanicPropagatingRuntime {
    ...
    /// Awaits all tracked tasks, panicking if any of them were cancelled or
    /// panicked.
