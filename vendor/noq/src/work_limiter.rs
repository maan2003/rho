...
/// The estimates are smoothed over all cycles where the exact duration is
/// measured.
///
/// In cycles where no measurement is performed the previously determined work
/// limit is used.
///
...
pub(crate) struct WorkLimiter {
    /// Whether to measure the required work time, or to use the previous
    /// estimates
    mode: Mode,
...
impl WorkLimiter {
    ...
    /// Returns whether more work can be performed inside the
    /// `desired_cycle_time`
    ///
    ...
    /// Records that `work` additional work items have been completed inside the
    /// cycle
    ///
    ...
    ///
    /// For cycles where the exact duration is measured this will update the
    /// estimates for the time per work item and the limit of allowed work
    /// items per cycle. The estimate is updated using the same exponential
    /// averaging (smoothing) mechanism which is used for determining QUIC
    /// path rtts: The last value is weighted by 1/8, and the previous
    /// average by 7/8.
