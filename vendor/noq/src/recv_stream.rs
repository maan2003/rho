...
impl RecvStream {
    ...
    /// Attempts to read from the stream into the provided buffer, which may be
    /// uninitialized
    ///
    /// On success, returns `Poll::Ready(Ok(()))` and places data into the
    /// unfilled portion of `buf`. If this does not write any bytes to `buf`
    /// (and `buf.remaining()` is non-zero), that indicates that the remote
    /// side has [`finish`]ed the stream and the local side has already read
    /// all bytes.
    ///
    /// If no data is available for reading, this returns `Poll::Pending` and
    /// arranges for the current task (via `cx.waker()`) to be notified when
    ...
    ///
    /// Yields `None` if the stream was finished. Otherwise, yields the next
    /// segment of data. The chunk's offset will be immediately after the
    /// last data yielded by [`RecvStream::read`] or
    /// [`RecvStream::read_chunk`]; use [`bytes_read()`](Self::bytes_read) to
    /// query that offset explicitly.
    ///
    /// For unordered reads, convert the stream into an unordered stream using
    /// [`Self::into_unordered`].
    ///
    /// Slightly more efficient than [`RecvStream::read`] due to not copying.
    /// Chunk boundaries do not correspond to peer writes, and hence cannot
    /// be used as framing.
    ///
    /// This operation is cancel-safe.
    ...
    ///
    /// On success, returns `Poll::Ready(Ok(Some(chunk)))`. If
    /// `Poll::Ready(Ok(None))` is returned, it implies that EOF has been
    /// reached.
    ///
    /// If no data is available for reading, the method returns `Poll::Pending`
    /// and arranges for the current task (via cx.waker()) to receive a
    /// notification when the stream becomes readable or is closed.
    fn poll_read_chunk(
        &mut self,
        cx: &mut Context<'_>,
        max_length: usize,
    ...
    /// last data yielded by [`read`](Self::read),
    /// [`read_chunk`](Self::read_chunk), or
    /// [`read_many_chunks`](Self::read_many_chunks), or `None` if the stream
    /// was finished.
    ///
    /// Slightly more efficient than [`read`](Self::read) due to not copying.
    /// Chunk boundaries do not correspond to peer writes, and hence cannot
    /// be used as framing.
    ///
    /// This operation is cancel-safe.
    ...
    ///
    /// Fails with [`ReadToEndError::TooLong`] on reading more than `size_limit`
    ...
    ///
    /// This is the offset of the next byte to be read, i.e. the length of the
    /// contiguous prefix of the stream consumed by the application.
...
///
/// The stream state related fns like [`Self::id`], [`Self::is_0rtt`],
/// [`Self::stop`], and [`Self::received_reset`] behave exactly as on
/// [`RecvStream`].
#[derive(Debug)]
...
impl UnorderedRecvStream {
    ...
    /// data and its offset in the stream. Segments may be received in any
    /// order, and the `Chunk`'s `offset` field can be used to determine
    /// ordering in the caller. Unordered reads are less prone
    /// to head-of-line blocking within a stream, but require the application to
    /// manage reassembling the original data.
    ///
    /// This operation is cancel-safe.
...
/// Future produced by [`RecvStream::read_chunk()`] or
/// [`UnorderedRecvStream::read_chunk()`].
///
