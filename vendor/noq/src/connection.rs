...
use crate::endpoint::ensure_ipv6;
use crate::mutex::{Mutex, MutexGuard};
use crate::path::{OpenPath, PathRef, PathRefOwner};
use crate::recv_stream::RecvStream;
use crate::runtime::{AsyncTimer, Runtime, UdpSender};
use crate::send_stream::SendStream;
use crate::{ConnectionEvent, Duration, Instant, Path, VarInt, udp_transmit};
...
impl Connecting {
    ...
    /// sending 0/0.5-RTT data. If so, the returned [`Connection`] can be
    /// used to send application data without waiting for the rest of the
    /// handshake to complete, at the cost of weakened cryptographic security
    /// guarantees. The returned [`ZeroRttAccepted`] future resolves when the
    /// handshake does complete, at which point subsequently opened streams
    ...
    /// [`crypto::ClientConfig`][crate::crypto::ClientConfig] attempts to
    /// resume a previous TLS session. However, **the remote endpoint may not
    /// actually _accept_ the 0-RTT data**--yet still accept the connection
    /// attempt in general. This possibility is conveyed through the
    /// [`ZeroRttAccepted`] future--when the handshake completes, it
    /// resolves to true if the 0-RTT data was accepted and false if it was
    /// rejected. If it was rejected, the existence of streams opened and
    /// other application data sent prior to the handshake completing will
    /// not be conveyed to the remote application, and local operations on
    /// them will return `ZeroRttRejected` errors.
    ///
    /// A server may reject 0-RTT data at its discretion, but accepting 0-RTT
    /// data requires the relevant resumption state to be stored in the
    /// server, which servers may limit or lose for various reasons
    /// including not persisting resumption state across server restarts.
    ///
    /// If manually providing a
    /// [`crypto::ClientConfig`][crate::crypto::ClientConfig], check your
    /// implementation's docs for 0-RTT pitfalls.
    ///
    /// ## Incoming
    ///
    /// For incoming connections, conversion to 0.5-RTT will always fully
    /// succeed. `into_0rtt` will always return `Ok` and the
    /// [`ZeroRttAccepted`] will always resolve to true.
    ///
    /// If manually providing a
    /// [`crypto::ServerConfig`][crate::crypto::ServerConfig], check your
    /// implementation's docs for 0-RTT pitfalls.
    ///
    /// ## Security
    ///
    ...
    pub fn into_0rtt(mut self) -> Result<(Connection, ZeroRttAccepted), Self> {
        // This lock borrows `self` and would normally be dropped at the end of this
        // scope, so we'll have to release it explicitly before returning `self`
        // by value.
    ...
    /// [`Session`](proto::crypto::Session). For the default `rustls` session,
    /// the return value can be [`downcast`](Box::downcast) to a
    /// [`crypto::rustls::HandshakeData`](crate::crypto::rustls::HandshakeData).
    pub async fn handshake_data(&mut self) -> Result<Box<dyn Any>, ConnectionError> {
        // Taking &mut self allows us to use a single oneshot channel rather than
        // dealing with potentially many tasks waiting on the same event. It's a
    ...
    /// expose this information. See
    /// [`noq_udp::RecvMeta::dst_ip`](udp::RecvMeta::dst_ip) for a list of
    /// supported platforms when using [`noq_udp`](udp) for I/O, which is the
    /// default.
    ///
...
///
/// This future handles the protocol logic for a single connection, routing
/// events from the `Connection` API object to the `Endpoint` task and the
/// related stream-related interfaces. It also keeps track of outstanding
/// timeouts for the `Connection`.
///
/// If the connection encounters an error condition, this future will yield an
/// error. It will terminate (yielding `Ok(())`) if the connection was closed
/// without error. Unlike other connection-related futures, this waits for the
/// draining period to complete to ensure that packets still in flight from the
/// peer are handled gracefully.
...
impl Future for ConnectionDriver {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        // something, we might need to reset a timer. Hence, we must loop until
        // neither happens.
...
/// handle, streams of incoming streams, and the various stream types) have been
/// dropped, then the connection will be automatically closed with an
/// `error_code` of 0 and an empty `reason`. You can also close the connection
/// explicitly by calling [`Connection::close()`].
///
/// Closing the connection immediately abandons efforts to deliver data to the
/// peer.  Upon receiving CONNECTION_CLOSE the peer *may* drop any stream data
/// not yet delivered to the application. [`Connection::close()`] describes in
/// more detail how to gracefully close a connection without losing application
/// data.
///
...
impl Connection {
    ...
    ///
    /// Streams are cheap and instantaneous to open unless blocked by flow
    /// control. As a consequence, the peer won't be notified that a stream
    /// has been opened until the stream is actually used.
    ...
    /// Streams are cheap and instantaneous to open unless blocked by flow
    /// control. As a consequence, the peer won't be notified that a stream
    /// has been opened until the stream is actually used. Calling
    /// [`open_bi()`] then waiting on the [`RecvStream`] without writing
    ...
    /// `accept_bi()`. Calling [`open_bi()`] then waiting on the
    /// [`RecvStream`] without writing anything to [`SendStream`] will never
    /// succeed.
    ///
    ...
    ///
    /// This function takes a [`FourTuple`], which contains the remote address
    /// and an optional local IP. If the local IP is set, the path will be
    /// opened with this source address, and the endpoint must support
    /// sending from that IP address. You can also pass a [`SocketAddr`] to
    /// only set the remote address and leave the local IP interface
    /// unspecified.
    ///
    /// The returned future completes once the path is either fully opened and
    /// ready to carry application data, or if there was an error.
    ///
    /// Dropping the returned future does not cancel the opening of the path,
    /// the [`PathEvent::Established`] event will still be emitted from
    /// [`Self::path_events`] if the path opens.  The [`PathId`] for the
    /// events can be extracted from [`OpenPath::path_id`].
    ///
    /// Failure to open a path can either occur immediately, before polling the
    /// returned future, or at a later time.  If the failure is immediate
    /// [`OpenPath::path_id`] will return `None` and the future will be
    /// ready immediately.  If the failure happens later, a [`PathEvent`]
    /// will be emitted.
    pub fn open_path(
        &self,
        network_path: impl Into<FourTuple>,
    ...
    ///
    /// The stream will yield a [`PathEvent`] whenever there is a change in the
    /// state of any path in this connection. The events need to be
    /// processed immediately, since there isn't an unbounded buffer for them.
    ///
    /// If processing of events lags behind too much, you will get an error of
    /// type [`crate::Lagged`] indicating how many events were lost. The
    /// stream continues after a lag, delivering the oldest retained message
    /// next.
    ...
    ///
    /// The events need to be processed immediately, since there isn't an
    ...
    ///
    /// Despite the return type's name, closed connections are often not an
    /// error condition at the application layer. Cases that might be
    /// routine include [`ConnectionError::LocallyClosed`]
    ...
    pub async fn closed(&self) -> ConnectionError {
        {
            ...
            // Construct the future while the lock is held to ensure we can't miss a wakeup
            // if the `Notify` is signaled immediately after we release the
            // lock. `await` it after the lock guard is out of scope.
    ...
    /// Wait for the connection to be closed without keeping a strong reference
    /// to the connection
    ///
    /// Returns a future that resolves, once the connection is closed, to a
    /// [`Closed`] struct describing the close reason and final connection
    /// and per-path statistics.
    ///
    /// Calling [`Self::closed`] keeps the connection alive until it is either
    /// closed locally via [`Connection::close`] or closed by the remote
    /// peer. This function instead does not keep the connection itself alive,
    /// so if all *other* clones of the connection are dropped, the connection
    /// will be closed implicitly even if there are futures returned from
    /// this function still being awaited.
    ...
    ///
    /// The close_reason is always set to `Some(ConnectionError)` when a socket
    /// is closed; whether it was closed manually by calling
    /// [`Connection::close()`] or due to an internal error (such as an idle
    /// timeout or the peer closing the connection).
    ///
    /// Note: when the connection is closed,
    /// `connection.close_reason().is_some()` will always be true.
    ...
    /// endpoints stay online long enough, and [`Endpoint::wait_idle()`] can
    ...
    /// or, if client authentication is not required, accepted our lack of
    /// authentication.
    ...
    ///
    /// Previously queued datagrams which are still unsent may be discarded to
    /// make space for this datagram, in order of oldest to newest.
    ...
    /// in the path MTU estimate. The peer can also enforce an arbitrarily
    /// small fixed limit, but if the peer's limit is large this is
    /// guaranteed to be a little over a kilobyte at minimum.
    ///
    ...
    /// with a datagram of at most this size is guaranteed not to cause
    /// older datagrams to be dropped.
    ...
    /// [`Connecting::handshake_data()`] succeeds. See that method's
    /// documentations for details on the returned value.
    ///
    ...
    // TODO(flub): not a useful API, once we do real things with multipath we can
    // remove this again.
    ...
    ///
    /// When the NAT traversal extension is negotiated, servers send these
    /// addresses to clients in `ADD_ADDRESS` frames. This allows clients to
    /// obtain server address candidates to initiate NAT traversal attempts.
    /// Clients provide their own reachable addresses in `REACH_OUT` frames
    ...
    /// Removes one or more addresses from the set of addresses at which this
    /// endpoint is reachable
    ///
    /// When the NAT traversal extension is negotiated, servers send address
    /// removals to clients in `REMOVE_ADDRESS` frames. This allows clients
    /// to stop using outdated server address candidates that are no longer
    /// valid for NAT traversal.
    ///
    /// For clients, removed addresses will no longer be advertised in
    /// `REACH_OUT` frames.
    ///
    ...
    /// cancelled, and paths that have not been opened are closed.
    ///
...
/// [`ensure_ipv6`]. If it uses IPv4 and the requested remote is IPv6, this
/// returns [`PathError::InvalidRemoteAddress`].
fn normalize_network_path(
    network_path: FourTuple,
    conn: &proto::Connection,
) -> Result<FourTuple, PathError> {
    // If endpoint::State::ipv6 is true we want to keep all our IP addresses as
    // IPv6. If not, we do not support IPv6.  We can not access endpoint::State
    // from here however, but either all our paths use an IPv6 address, or all
    // our paths use an IPv4 address.  So we can use that information.
    let ipv6 = conn
        .paths()
        .iter()
        .filter_map(|id| {
...
fn poll_accept<'a>(
    ...
    // Check for incoming streams before checking `state.error` so that
    // already-received streams, which are necessarily finite, can be drained
...
impl Future for ReadDatagram<'_> {
    ...
    fn poll(self: Pin<&mut Self>, ctx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        // Check for buffered datagrams before checking `state.error` so that
        // already-received datagrams, which are necessarily finite, can be
        // drained from a closed connection.
...
pub struct Closed {
    ...
    /// Per-path statistics for every path the connection knew about at close
    /// time.
    ///
    /// This includes paths that haven't been discarded at close time, plus any
    /// already-discarded paths whose final stats had been retained because a
    /// [`Path`] or [`WeakPathHandle`] handle was kept alive.
    ///
...
impl Future for OnClosed {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        // The `expect` is safe because `State::drop` ensures that all senders are
        // triggered before being dropped.
        Pin::new(&mut this.rx)
            .poll(cx)
...
impl Drop for ConnectionRef {
    fn drop(&mut self) {
        ...
        if !conn.inner.is_closed() {
            ...
            // it's not, we can't do any harm. If there were any streams being
            // opened, then either the connection will be closed for an
            // unrelated reason or a fresh reference will be constructed for the
            // newly opened stream.
...
impl ConnectionInner {
    /// Lock the state and return a guard that wakes the connection driver on
    /// drop.
    ///
    /// Use this for operations that may queue frames. The wake ensures the
    /// driver sends queued frames. If that's not needed, use
    /// [`Self::lock_without_waking`].
    ...
    ///
    /// Use this for operations that don't require any action from the
    /// connection driver. Otherwise, use [`Self::lock_and_wake`] instead.
...
///
/// This contains a weak reference to the connection so will not itself keep the
/// connection alive.
#[derive(Debug, Clone)]
pub struct WeakConnectionHandle(Weak<Arc<ConnectionInner>>);

impl WeakConnectionHandle {
    /// Returns `true` if the [`Connection`] associated with this handle is
    /// still alive.
    ...
    /// Returns `true` if the two [`WeakConnectionHandle`] point at the same
    /// connection.
...
pub(crate) struct Shared {
    ...
    /// Notified when new streams may be locally initiated due to an increase in
    /// stream ID flow control budget
    ...
    closed: Notify,
    /// Number of live handles that can be used to initiate or handle I/O;
    /// excludes the driver
...
pub(crate) struct State {
    ...
    ///
    /// I.e. how many [`Path`] and [`WeakPathHandle`] structs are alive for a
    /// path. Each entry's [`PathRefOwner`] holds an [`AtomicUsize`] so that
    /// cloning or dropping a [`PathRef`] (held by [`Path`] or
    /// [`WeakPathHandle`]) does not need to lock this state.
    ///
    ...
    ///
    /// We only insert entries if the discarded path has a non-zero reference
    /// count in [`Self::path_refs`]. When the last reference to a path is
    /// dropped its entry is removed from both maps.
    ...
    /// Our last external address reported by the peer. When multipath is
    /// enabled, this will be the last report across all paths.
...
impl State {
    ...
    /// If this returns `Err`, the endpoint is dead, so the driver should exit
    /// immediately.
    ...
    fn drive_timer(&mut self, cx: &mut Context<'_>) -> bool {
        // Check whether we need to (re)set the timer. If so, we must poll again to
        // ensure the timer is registered with the runtime (and check whether
    ...
    /// Used to wake up all blocked futures when the connection becomes closed
    /// for any reason
    ...
    /// Acquire a new [`PathRef`] for a path id, bumping its reference counter
    /// by 1.
    ///
    /// The returned [`PathRef`] is intended to be stored on a [`Path`] or
    /// [`WeakPathHandle`]. Its reference count is automatically increased
    /// when cloned. When its owner is dropped, [`PathRef::on_drop`] must be
    /// called to decrement the refcount.
    ///
    /// [`WeakPathHandle`]: crate::path::WeakPathHandle
