...
use crate::runtime::{AsyncUdpSocket, Runtime, UdpSender};
use crate::work_limiter::WorkLimiter;
use crate::{
    ConnectionEvent, EndpointConfig, IO_LOOP_BOUND, Instant, RECV_TIME_BOUND, VarInt, udp_transmit,
};
...
impl Endpoint {
    ...
    ///
    /// If an IPv6 address is provided, attempts to make the socket dual-stack
    /// so as to allow communication with both IPv4 and IPv6 addresses. As
    /// such, calling `Endpoint::client` with the address `[::]:0` is a
    ...
    /// ```
    ///
    /// Some environments may not allow creation of dual-stack sockets, in which
    /// case an IPv6 client will only be able to connect to IPv6 servers. An
    ...
    ///
    /// If an IPv6 address is provided, attempts to make the socket dual-stack
    /// so as to allow communication with both IPv4 and IPv6 clients. As
    /// such, calling `Endpoint::server` with the address `[::]:0` is a
    /// reasonable default to maximize the ability to accept connections
    /// from any address.
    ///
    /// Some environments may not allow creation of dual-stack sockets, in which
    /// case an IPv6 server will only be able to accept connections from
    /// IPv6 clients. An IPv4 server is never dual-stack.
    #[cfg(all(
        not(wasm_browser),
        any(feature = "runtime-tokio", feature = "runtime-smol"),
        any(feature = "aws-lc-rs", feature = "ring"), // `EndpointConfig::default()` is only available with these
    ...
    /// Construct an endpoint with arbitrary configuration and pre-constructed
    /// abstract socket
    ///
    /// Useful when `socket` has additional state (e.g. sidechannels) attached
    /// for which shared ownership is needed.
    ...
    /// final [`Connection`](crate::Connection), or used to e.g.
    /// filter connection attempts or force address validation, or converted
    /// into an intermediate `Connecting` future which can be used to e.g.
    /// send 0.5-RTT data.
    ...
    ///
    /// `server_name` must be covered by the certificate presented by the
    /// server. This prevents a connection from being intercepted by an
    /// attacker with a valid certificate for some other server.
    ///
    /// May fail immediately due to configuration errors, or in the future if
    ...
    ///
    /// This informs all active connections that the local address may have
    /// changed (e.g., due to a network interface change), triggering
    /// liveness checks and recovery procedures on each connection without
    /// requiring a socket rebind.
    ///
    /// Unlike [`Self::rebind`], this does not change the underlying socket. Use
    /// this when the network topology changes but the socket remains valid
    /// (e.g., if bound to the unspecified address and switching from WiFi
    /// to cellular, or when the local IP address changes).
    ///
    /// The optional `hint` allows callers to indicate which paths may still be
    /// recoverable after the network change. If `None`, all paths are
    /// assumed to be non-recoverable. For client-side
    /// multipath connections, unrecoverable paths will be closed and replaced
    /// with new paths to the same remote addresses.
    ...
    pub fn close(&self, error_code: VarInt, reason: &[u8]) {
        ...
        // Wake the endpoint driver so that it can shutdown if the endpoint is already
        // drained.
    ...
    /// Waits for all connections on the endpoint to be cleanly shut down and
    /// drained.
    ///
    /// This is equivalent to [`wait_all_draining()`] with additionally waiting
    /// for the connections to be drained. Please see its documentation for
    /// more information.
    ///
    /// Use `wait_idle()` in favor of `wait_all_draining()` if you care about
    /// waiting for the [`Connection`] structs to be dropped.
    ///
    ...
    /// period.
    ///
    /// Does not proactively close existing connections or cause incoming
    /// connections to be rejected. Consider calling [`close()`] if that is
    /// desired.
    ///
    /// Unlike [`wait_idle()`], this doesn't wait for the full draining period,
    /// so it can't be used to wait for all now-idle [`Connection`]s to be
    /// dropped.
    ///
...
///
/// This task functions as the switch point between the UDP socket object and
/// the `Endpoint` responsible for routing datagrams to their owning
/// `Connection`. In order to do so, it also facilitates the exchange of
/// different types of events flowing between the `Endpoint` and the tasks
/// managing `Connection`s. As such, running this task is necessary to keep the
/// endpoint's connections running.
///
/// `EndpointDriver` futures terminate when all clones of the `Endpoint` have
/// been dropped, or when an I/O error occurs.
...
impl Drop for EndpointDriver {
    fn drop(&mut self) {
        ...
        self.0.shared.incoming.notify_waiters();
        // Drop all outgoing channels, signaling the termination of the endpoint to the
...
pub(crate) struct Shared {
    ...
    incoming: Notify,
    /// Notifies subscribers when *all* connections have entered the draining
    /// state.
    ///
...
impl State {
    fn drive_recv(&mut self, cx: &mut Context<'_>, now: Instant) -> Result<bool, io::Error> {
        ...
        if poll_res.received_connection_packet {
            // Traffic has arrived on self.socket, therefore there is no need for the
            // abandoned one anymore. TODO: Account for multiple outgoing
            // connections.
            self.prev_socket = None;
        }
...
fn respond(
    ...
    // Copied from rust 1.85's std::task::Waker::noop() implementation for backwards
    // compatibility
    const NOOP: RawWaker = {
    ...
    // SAFETY: Copied from rust stdlib, the NOOP waker is thread-safe and doesn't
    // violate the RawWakerVTable contract, it doesn't access the data pointer
    // at all.
...
struct ConnectionSet {
    ...
    ///
    /// This is directly related to the QUIC connection states "Initial",
    /// "Handshake", "Established", "Closed", "Draining" and "Drained" (see
    /// also `proto/src/connection/state.rs`).
    ///
    /// Any connection state that is not "Draining" or "Drained" is considered
    /// active.
    ///
    /// This counter is updated when new connections are added
    /// ([`ConnectionSet::insert`]) and when a connection informs us about
    /// entering the draining state ([`State::handle_events`]).
...
struct PollProgress {
    ...
    /// Whether datagram handling was interrupted early by the work limiter for
    /// fairness
    keep_going: bool,
}
