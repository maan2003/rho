...
- `rho-agent2`: `wait` is a core-owned tool (`{"seconds": N}`, 1..3600), answered at the next drain, and sets `ModelAsked::Wait`; the default check-in is 120 s (was 10 s) and partial output waits 300 s (was 60 s).
- `rho-agent2-tools` (new crate): the real tools as agent2 sources. `exec_command` stays attached to its process for life (silent until exit; `Soon` on crash/test-verdict/server-up/prompt lines; later output arrives as `ToolUpdate`s on the same call); `write_stdin` only types, never polls; `apply_patch`, `view_image`, `web__run` and the collaboration tools are one-shot futures; code mode's `exec` cell is a session too (`notify`/`yield_control` mark it `Soon`, completion `Ended`), and code mode's own cell `wait` is not offered.
- `rho-agent` runtime `AgentRuntime::Rho2 { core }` + `RunningAgent::Rho2`: same records, workdirs, roles, prompt and tools; agent2 loop underneath. Opt in with `RHO_AGENT2=1` in the daemon environment. Not yet on that runtime: Iris, presentation (titles/activity), usage accounting, rewind, role/profile changes. Flip the default in `crates/rho-agent/src/agent2.rs::enabled` once those are in.
