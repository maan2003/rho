...
async fn shell_end_to_end_over_registry(
    ...
    let mut first_state = rho_shell_view::protocol::ShellState::default();
    ...
    assert!(rho_shell_view::protocol::command_fits(&long_line));
    ...
    let too_long = format!(
        ...
        "x".repeat(rho_shell_view::protocol::MAX_COMMAND_BYTES)
    );
    assert!(!rho_shell_view::protocol::command_fits(&too_long));
    ...
    let mut second_state = rho_shell_view::protocol::ShellState::default();
    ...
    assert_eq!(
        ...
        rho_shell_view::protocol::ShellExecutionState::Finished { status: 7 }
    );
    drop(exit);
    assert_eq!(
...
async fn wait_for_text(
    ...
    state: &mut rho_shell_view::protocol::ShellState,
    needle: &str,
) {
...
async fn wait_for_running(
    ...
    let running = |state: &rho_shell_view::protocol::ShellState| {
        state.executions.iter().any(|block| {
            block.execution == execution
                && matches!(
                    block.state,
                    rho_shell_view::protocol::ShellExecutionState::Running
                )
        })
    };
    if running(state) {
...
async fn wait_for_finished(
    ...
    let finished = |state: &rho_shell_view::protocol::ShellState| {
        state.executions.iter().any(|block| {
            block.execution == execution
                && matches!(
                    block.state,
                    rho_shell_view::protocol::ShellExecutionState::Finished { .. }
                )
        })
    };
    if finished(state) {
...
fn apply_test_frame(state: &mut rho_shell_view::protocol::ShellState, frame: ShellServerFrame) {
    use rho_shell_view::protocol::ShellExecutionState;
    match frame {
...
fn render_state(state: &rho_shell_view::protocol::ShellState) -> String {
    let mut text = state.terminal_output.clone();
    for execution in &state.executions {
        if matches!(
            execution.state,
            rho_shell_view::protocol::ShellExecutionState::Queued
        ) {
            continue;
        }
