//! Retained sessions and workset control, independent of any agent runtime.
...
use crate::transport::{Port, Sender};
...
pub fn encode<T: senax_encoder::Encoder>(value: &T) -> anyhow::Result<bytes::Bytes> {
...
pub fn decode<T: senax_encoder::Decoder>(bytes: &[u8]) -> anyhow::Result<T> {
...
pub type Clients = Arc<Mutex<HashMap<Port, mpsc::Sender<bytes::Bytes>>>>;

/// Root-owned retained execution, independent of loaded agent handles.
pub struct Execution {
    pub base: Arc<rho_fs_view::Namespace>,
...
impl Execution {
    ...
    pub async fn attach(
        ...
        reply: impl Fn(u64, Reply) -> anyhow::Result<bytes::Bytes>,
    ) -> anyhow::Result<()> {
        match attach {
            Attach::Terminal {
                agent,
